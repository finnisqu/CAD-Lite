    ...
    delete state.overlay;
      ...
      function enablePieceDimEdit(textEl, piece, prop, promptLabel){
        ...
        textEl.addEventListener('dblclick', e => {
          ...
          if (!(next > 0)) { window.alert('Enter a positive dimension, such as 96, 94.5, or 94 1/2.'); return; }
      ...
      function enableSinkCenterlineEdit(textEl, sink, piece){
        ...
        textEl.addEventListener('dblclick', e => {
          e.preventDefault(); e.stopPropagation();
          const raw = window.prompt('Sink centerline (inches)', fmt3(sink.centerline ?? 0));
          if (raw == null) return;
          const next = parseDimInches(raw);
          const axisMax = (sink.side === 'left' || sink.side === 'right') ? (piece.h || 0) : (piece.w || 0);
          if (next == null || next < 0 || next > axisMax) {
            window.alert('Enter a centerline between 0 and ' + fmt3(axisMax) + ' inches.');
            return;
          }
