   (function(){
    function init(){
      ...
      function syncCanvasToPlan(plan=currentPlan()){
        ...
        const size=planCanvasSize(plan);
        const L=activeLayout?.();
        if(!L)return false;
        L.cw=size.w;
        L.ch=size.h;
        ...
        Object.entries(handles).forEach(([side,h])=>{
          ...
          handle.addEventListener('pointerdown',e=>{
            ...
            const move=ev=>{
              ...
              // Recompute center shift from the final clamped/snapped size so
              // dragging through the opposite edge never throws the piece away.
              shiftU=0;shiftV=0;
