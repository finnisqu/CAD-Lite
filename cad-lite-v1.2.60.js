  ...
  "pieces": [
    ...
    { "id": "ycid73l", "name": "Backsplash", "w": 4, "h": 22.5, "x": 159, "y": 10, "rotation": 0, "color": "#d5f0f0", "layer": 8, "rTL": false, "rTR": false, "rBL": false, "rBR": false, "sinks": [] }
      ...
      // Pieces uses a dedicated toggle because its header also contains Add Piece.
      ...
      if(piecesCard&&piecesHead&&!piecesHead.dataset.toggleReady){
        ...
        const piecesArrow=document.createElement('button');
        piecesArrow.type='button';piecesArrow.className='lc-btn ghost';piecesArrow.textContent='▸';piecesArrow.title='Show/hide pieces';
        piecesArrow.style.padding='2px 8px';piecesArrow.style.minWidth='32px';piecesArrow.style.flex='0 0 auto';
        if(piecesTitle)piecesTitle.insertAdjacentElement('afterend',piecesArrow);else piecesHead.insertBefore(piecesArrow,piecesHead.firstChild);
        let piecesOpen=false;
        const syncPieces=()=>{list.style.display=piecesOpen?'':'none';piecesArrow.textContent=piecesOpen?'▾':'▸';piecesArrow.setAttribute('aria-expanded',String(piecesOpen));};
        piecesArrow.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();piecesOpen=!piecesOpen;syncPieces();});
        syncPieces();
      }
