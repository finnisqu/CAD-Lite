   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if(toggleSpacer) toggleSpacer.replaceWith(btnNoteTool);
      else if(toggleGrid) toggleGrid.appendChild(btnNoteTool);
      else btnDimTool?.parentElement?.appendChild(btnNoteTool);
      function syncNoteToolUI(){btnNoteTool.textContent=state.noteTool?'Note Tool: On':'Note Tool: Off';btnNoteTool.classList.toggle('alt',!!state.noteTool);btnNoteTool.classList.toggle('ghost',!state.noteTool);}
      btnNoteTool.onclick=()=>{state.noteTool=!state.noteTool;if(state.noteTool&&state.dimTool){state.dimTool=false;dimTempStart=null;syncDimToolUI?.();}syncNoteToolUI();}; syncNoteToolUI();
