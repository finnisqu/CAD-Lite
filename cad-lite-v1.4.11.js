   (function(){
    function init(){
      ...
      function renderPlanNavigator(){
        ...
        const eye=planButton(plan.visible?'Hide':'Show',()=>{
          plan.visible=!plan.visible;
          renderPlanNavigator();draw();scheduleSave();pushHistory();
        },'ghost');
        eye.title=plan.visible?'Hide floor plan':'Show floor plan';
  ...
  if (state.overlay && (state.overlay.dataURL || state.overlay.visible)){
    ...
    delete state.overlay;
      ...
      function pieceLabelTextColor(piece,fallback){
        const custom=String(piece?.labelColor||'').trim();
        return /^#[0-9a-f]{6}$/i.test(custom)?custom:(fallback||'#111111');
      }
      ...
      function mirrorPieceLocalY(piece){
        if(!piece)return;
        migratePieceGeometry(piece);
        ...
        piece.sinks.forEach(sink=>{
          const oldSide=['front','back','left','right'].includes(sink.side)?sink.side:'front';
          const oldLocalAngle=sinkReferenceAngle(oldSide)+(Number(sink.rotation)||0);
          const nextSide=(oldSide==='front'||oldSide==='back')?swapFrontBack(oldSide):oldSide;
          if(oldSide==='left'||oldSide==='right'){
            sink.centerline=round3(Math.max(0,(Number(piece.h)||0)-(Number(sink.centerline)||0)));
          }
          sink.side=nextSide;
          sink.rotation=normDeg(-oldLocalAngle-sinkReferenceAngle(nextSide));
        });
      ...
      function mirrorSelectedPieces(axis){
        ...
        // Preserve linked assemblies: selected parent brings snapped children;
        // selected linked child brings its parent and sibling snapped splashes.
        let changed=true;
        while(changed){
          changed=false;
          state.pieces.forEach(piece=>{
        ...
        poses.forEach(item=>{
          ...
          piece.rotation=axis==='h'?normDeg(180-(Number(piece.rotation)||0)):normDeg(-(Number(piece.rotation)||0));
          mirrorPieceLocalY(piece);
          const rs=realSize(piece);
  ...
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      function updateInspector(){
        ...
        const visualHint=document.createElement('span');
        visualHint.className='lc-piece-visual-hint';
        visualHead.append(visualTitle,visualHint);
        ...
        const visualFillInput=document.createElement('input');
        visualFillInput.type='color';
        visualFillInput.className='lc-piece-visual-hidden-color';
        visualFillInput.value=p.color||'#ffffff';

        const visualLabelInput=document.createElement('input');
        visualLabelInput.type='color';
        visualLabelInput.className='lc-piece-visual-hidden-color';
        visualLabelInput.value=pieceLabelTextColor(p,pickTextColor(p.color||'#ffffff'));

        visualFillInput.oninput=()=>{
          p.color=visualFillInput.value;
          renderList();draw();pieceInspectorPreviewSync?.();
        };
        visualFillInput.onchange=()=>{scheduleSave();pushHistory();};

        visualLabelInput.oninput=()=>{
          p.labelColor=visualLabelInput.value;
          draw();pieceInspectorPreviewSync?.();
        };
        visualLabelInput.onchange=()=>{scheduleSave();pushHistory();};
        ...
        const renderPieceVisual=()=>{
          ...
          visualHint.textContent=mode==='appearance'?'Click fill or label to edit':'';
          ...
          rect.setAttribute('fill-opacity',String(p.noFill?0:Math.max(.08,getFillOpacity(p))));
          rect.setAttribute('stroke','#374151');
          rect.setAttribute('stroke-width','1.2');
          if(mode==='appearance'){
            rect.classList.add('is-editable');
            rect.addEventListener('click',()=>visualFillInput.click());
          }
          ...
          if(mode==='sinks'&&Array.isArray(p.sinks)){
            p.sinks.forEach(sink=>{
              ...
              let el;
              if(sink.shape==='oval'){
                el=svgEl('ellipse',{cx,cy,rx:sw/2,ry:sh/2,fill:'none',stroke:'#111','stroke-width':1});
              }else{
                el=svgEl('rect',{x:cx-sw/2,y:cy-sh/2,width:sw,height:sh,rx:Math.min(5,Number(sink.cornerR)||0)*Math.min(sx,sy),fill:'none',stroke:'#111','stroke-width':1});
              }
              const angle=sinkReferenceAngle(sink.side||'front')+(Number(sink.rotation)||0);
              if(angle)el.setAttribute('transform',`rotate(${angle} ${cx} ${cy})`);
              visualSvg.appendChild(el);
            });
          }
          ...
          if(mode==='edgeOptions'&&p.edgeProfiles){
            const edgeText=(side,tx,ty,rot=0)=>{
              ...
              if(!value)return;
              const t=svgEl('text',{x:tx,y:ty,'text-anchor':'middle','dominant-baseline':'middle','font-size':'10',fill:'#174ea6','font-weight':'700'});
              if(rot)t.setAttribute('transform',`rotate(${rot} ${tx} ${ty})`);
              t.textContent=value;
              visualSvg.appendChild(t);
            };
            edgeText('top',x+w/2,y+9);
            edgeText('bottom',x+w/2,y+h-9,180);
            edgeText('left',x+9,y+h/2,-90);
            edgeText('right',x+w-9,y+h/2,90);
          }
          ...
          if(mode==='appearance'){
            label.classList.add('lc-piece-visual-label-edit');
            label.addEventListener('click',e=>{e.stopPropagation();visualLabelInput.click();});
          }
        ...
        renderPieceVisual();
        pieceVisual.append(visualHead,visualSvg,visualFillInput,visualLabelInput);
        ...
        // Label color is secondary to piece appearance, so keep it after
        // color + opacity/fill and use the same compact swatch interaction.
        const labelColorRow=document.createElement('label');
        labelColorRow.className='lc-label lc-piece-label-color-row lc-piece-label-color-row-subtle';
        const labelColorText=document.createElement('span');
        labelColorText.textContent='Label Color';

        const labelColorControls=document.createElement('div');
        labelColorControls.className='lc-piece-label-color-controls';
        const customLabel=/^#[0-9a-f]{6}$/i.test(String(p.labelColor||''));
        const labelColorStack=colorStack(
          customLabel?p.labelColor:pickTextColor(p.color||'#ffffff'),
          val=>{
            p.labelColor=val;
            draw();
            pieceInspectorPreviewSync?.();
        ...
        labelColorStack.wrap.classList.add('lc-piece-label-color-stack');

        const labelColorAuto=document.createElement('button');
        labelColorAuto.type='button';
        labelColorAuto.className='lc-btn ghost sm lc-piece-label-color-auto';
        labelColorAuto.textContent='Auto';
        labelColorAuto.title='Use automatic label contrast';
        const syncLabelColorAuto=()=>{
          const custom=/^#[0-9a-f]{6}$/i.test(String(p.labelColor||''));
          labelColorAuto.classList.toggle('is-active',!custom);
          labelColorAuto.setAttribute('aria-pressed',String(!custom));
          if(!custom){
            const fallback=pickTextColor(p.color||'#ffffff');
            labelColorStack.input.value=fallback;
            labelColorStack.sw.style.setProperty('--c',fallback);
          }
        };
        labelColorAuto.onclick=e=>{
          e.preventDefault();
          delete p.labelColor;
          syncLabelColorAuto();
          draw();
        ...
        syncLabelColorAuto();
        labelColorControls.append(labelColorStack.wrap,labelColorAuto);
        labelColorRow.append(labelColorText,labelColorControls);
        appBody.appendChild(labelColorRow);
        ...
        if(seamHead&&seamArrow){
          ...
          seamVisibilityInspectorEye.className='lc-btn ghost lc-iconbtn lc-seams-eye';
    ...
    del.innerHTML='<svg class="lc-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l1 2h4v2H3V5h4l1-2Zm-2 6h12l-1 11H7L6 9Zm3 2v7h2v-7H9Zm4 0v7h2v-7h-2Z" fill="currentColor"/></svg>';
        ...
        edgeBody.appendChild(inspectorEdgeMode);
        ...
        edgeDesigner.append(edgeDiagram,manualRadiusButton,radiusLabelButton,edgePainterRow,edgeHint);
        syncMomentaryToolUI();
