        ...
        if (L && Array.isArray(L.dims) && L.dims.length && state.showManualDims) {
          ...
          const offPx = 12; // perpendicular offset in px from the measured segment
            ...
            L.dims.forEach(d => {
              ...
              t.textContent=label;t.style.cursor='grab';t.setAttribute('pointer-events','all');
              t.addEventListener('pointerdown',ev=>{
                ev.preventDefault();ev.stopPropagation();state.selectedDimId=d.id;state.selectedId=null;renderDimList();updateInspector();
                const start=svgPoint(ev),startOff=(typeof d.offsetPx==='number'?d.offsetPx:12);let moved=false;
                const move=mv=>{const q=svgPoint(mv);if(Math.hypot(q.x-start.x,q.y-start.y)>3)moved=true;if(!moved)return;const delta=(q.x-start.x)*nx+(q.y-start.y)*ny;d.offsetPx=Math.max(-300,Math.min(300,startOff+delta));draw();};
                const up=()=>{svg.removeEventListener('pointermove',move);svg.removeEventListener('pointerup',up);svg.removeEventListener('pointercancel',up);if(moved){scheduleSave();pushHistory();}};
                svg.addEventListener('pointermove',move);svg.addEventListener('pointerup',up);svg.addEventListener('pointercancel',up);
              });
              t.addEventListener('dblclick',ev=>{ev.preventDefault();ev.stopPropagation();openInlineDimEditor(t,distIn.toFixed(2),raw=>{const next=parseDimInches(raw);if(!(next>0))return false;const vx=d.x2-d.x1,vy=d.y2-d.y1,oldLen=Math.sqrt(vx*vx+vy*vy)||1;d.x2=round3(d.x1+(vx/oldLen)*next);d.y2=round3(d.y1+(vy/oldLen)*next);draw();renderDimList();scheduleSave();pushHistory();return true;});});
