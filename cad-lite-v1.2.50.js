        ...
        if (L && Array.isArray(L.dims) && L.dims.length && state.showManualDims) {
          ...
          const offPx = 12; // perpendicular offset in px from the measured segment
            ...
            L.dims.forEach(d => {
              ...
              t.textContent=label;
              t.style.cursor='grab';
              t.setAttribute('pointer-events','stroke');
              t.setAttribute('stroke','rgba(0,0,0,0.001)');
              t.setAttribute('stroke-width','14');
              t.setAttribute('paint-order','stroke fill');
              t.addEventListener('pointerdown',ev=>{
                ev.preventDefault();ev.stopPropagation();
                state.selectedDimId=d.id;state.selectedId=null;renderDimList();updateInspector();
                ...
                const up=()=>{document.removeEventListener('pointermove',move,true);document.removeEventListener('pointerup',up,true);document.removeEventListener('pointercancel',up,true);if(moved){scheduleSave();pushHistory();}};
