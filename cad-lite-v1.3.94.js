   (function(){
    function init(){
      ...
      function syncMomentaryToolUI(){
        ...
        root.classList.toggle('lc-momentary-radius',!slabWorkspace&&tool==='radius');
        ...
        document.querySelectorAll('.lc-radius-label-action').forEach(btn=>{
          ...
          btn.title=slabWorkspace
            ...
            : 'Highlight rounded corners on the canvas and click one to create a linked Radius Note + Leader. Enter or blank canvas exits.';
        });
      }

      function setMomentaryTool(tool,key){
        if(state.workspace==='slab'&&(tool==='splash'||tool==='splashAll'||tool==='radius'))return;
  ...
  async function addRasterPNG() {
    ...
    sinksUI = initSinksCard({
      ...
      function setCanvasWorkspace(mode){
        ...
        toolPreview=null;
        if(next==='slab'){
          if(momentaryToolState.held==='splash'||momentaryToolState.held==='splashAll'||momentaryToolState.held==='radius'){
            momentaryToolState.held=null;
            momentaryToolState.key=null;
          }
          if(momentaryToolState.locked==='splash'||momentaryToolState.locked==='splashAll'||momentaryToolState.locked==='radius'){
            momentaryToolState.locked=null;
          }
          state.dimTool=false;
          state.noteTool=false;
  ...
  if (state.overlay && (state.overlay.dataURL || state.overlay.visible)){
    ...
    delete state.overlay;
      ...
      function drawRadiusInteractionLayer(){
        if(activeMomentaryTool()!=='radius'||state.workspace==='slab')return;
        ...
        if(rot)master.setAttribute('transform',`rotate(${rot} ${cx} ${cy})`);
...
function syncRadiusAnnotations(L=cur()){
  ...
  L.notes.forEach(note=>{
    ...
    const target=piece?radiusCornerTarget(piece,ref.corner):null;
    if(!piece||!target){
    ...
    noteLeadersFor(note.id,L).forEach(lineObj=>{
      ...
      lineObj.radiusRef={pieceId:ref.pieceId,corner:ref.corner};
      if(lineObj.attachedEnd==='end'){
...
function createRadiusAnnotation(piece,corner){
  ...
  const note={
    ...
    radiusRef:{pieceId:piece.id,corner,autoText:true}
  };
  const line={
    id:uid(),
    name:'Radius Leader',
    ...
    attachedEnd:'start',
    annotationType:'radius',
    radiusRef:{pieceId:piece.id,corner}
  };
  L.notes.push(note);
  L.lines.push(line);
    ...
    del.innerHTML='<svg class="lc-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l1 2h4v2H3V5h4l1-2Zm-2 6h12l-1 11H7L6 9Zm3 2v7h2v-7H9Zm4 0v7h2v-7h-2Z" fill="currentColor"/></svg>';
        ...
        const edgeProfileIcon=value=>{
          ...
          const paths={
            ...
            miter:'<path d="M3 4h13l5 5v3H3M16 4v5h5"/>',
        ...
        function makeEdgeSelect(side,labelText){
          ...
          button.onclick=e=>{
            ...
            edgeDiagram.querySelectorAll('.lc-edge-profile-picker-menu').forEach(other=>{
              if(other!==menu)other.hidden=true;
            });
            edgeDiagram.querySelectorAll('.lc-edge-profile-picker-button').forEach(other=>{
      ...
      const blockNonMomentaryInteraction=e=>{
        ...
        if(target instanceof Element){
          ...
          if(tool==='radius' && target.closest('[data-radius-corner-hit="1"]'))return;
      ...
      function activeToolCursor(){
        ...
        if(activeMomentaryTool()==='splash'||activeMomentaryTool()==='splashAll'||activeMomentaryTool()==='radius')return 'default';
