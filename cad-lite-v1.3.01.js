   (function(){
    function init(){
      ...
      function selectOnly(id){ state.selectedDimId=null; state.selectedSeamId=null; state.selectedNoteId=null; setSelection([id]); }
      function toggleSelect(id){
        state.selectedDimId=null; state.selectedSeamId=null; state.selectedNoteId=null;
      ...
      window.addEventListener('keydown', (e) => {
        ...
        L.notes=L.notes.filter(n=>n.id!==state.selectedNoteId);
        if(L.notes.length===before)return;
      ...
      window.addEventListener('keydown', (e) => {
        ...
        if(tag==='input'||tag==='textarea'||tag==='select'||(ael&&ael.isContentEditable))return;
        if(e.key!=='Delete'&&e.key!=='Backspace')return;
        // Let selected dimensions/seams/notes use their own delete handlers.
        if(state.selectedDimId||state.selectedSeamId||state.selectedNoteId)return;
      ...
      window.addEventListener('keydown', (e) => {
        ...
        if(e.key==='Escape'){
          ...
          if(!hadTool){
            clearSelection();state.selectedDimId=null;state.selectedSeamId=null;state.selectedNoteId=null;
...
function renderLayouts(){
  ...
  state.layouts.forEach((L, idx)=>{
    ...
    row.addEventListener('click', (e)=>{
      if(e.target.closest('button, input, textarea')) return;
      state.active = idx; state.selectedId = null; state.selectedIds=[]; state.selectedDimId=null; state.selectedSeamId=null; state.selectedNoteId=null;
  ...
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      function updateInspector(){
        ...
        const seamHint=document.createElement('div');
        seamHint.className='lc-small';
        seamHint.textContent='Piece-based seams use exact offsets and move with the piece.';
        seamHint.style.marginBottom='8px';
        seamBody.appendChild(seamHint);
      ...
      btnClearSel && (btnClearSel.onclick = ()=>{
        clearSelection();
        state.selectedDimId=null; state.selectedSeamId=null; state.selectedNoteId=null;
        renderList(); renderDimList(); renderSeamList(); renderNoteList(); updateInspector(); sinksUI?.refresh(); draw();
      });
