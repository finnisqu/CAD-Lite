    ...
    pushHistory();
      ...
      function drawSeams(){
        ...
        L.seams.forEach(seam=>{
          ...
          const line=svgEl('line',{x1:i2p(seam.x1),y1:i2p(seam.y1),x2:i2p(seam.x2),y2:i2p(seam.y2),stroke:'#111','stroke-width':state.selectedSeamId===seam.id?3:2,'stroke-dasharray':'8 4','vector-effect':'non-scaling-stroke'});
          g.appendChild(line);
          g.addEventListener('pointerdown',ev=>{ev.preventDefault();ev.stopPropagation();state.selectedSeamId=seam.id;state.selectedDimId=null;clearSelection();draw();});
          svg.appendChild(g);
        });
      }
      ...
      svg.addEventListener('click',e=>{
        ...
        const raw={x:p2i(pt.x),y:p2i(pt.y)}; const snapped=snapDimPoint(raw)||raw;
        if(!seamTempStart){seamTempStart=snapped;state.selectedSeamId=null;}
          ...
          L.seams.push(seam);state.selectedSeamId=seam.id;seamTempStart=null;draw();scheduleSave();pushHistory();
        }
        e.stopPropagation();
      });
      ...
      window.addEventListener('keydown',e=>{
        ...
        L.seams.splice(idx,1);state.selectedSeamId=null;draw();scheduleSave();pushHistory();e.preventDefault();
      });
