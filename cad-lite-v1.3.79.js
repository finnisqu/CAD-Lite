   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      const addViewSubmenu=(label,build)=>{
        ...
        if(label==='Annotations')wrap.classList.add('lc-design-only');
        if(label==='Overlays')wrap.classList.add('lc-slab-only');
      ...
      addViewToggle('Slab Show','showSlabMaterial');
      ...
      [insertNoteAction,insertDimAction,insertLineAction].forEach(item=>item.classList.add('lc-design-only'));

      const insertSlabSep=addToolbarMenuSeparator(insertDropdown.menu);
      insertSlabSep.classList.add('lc-slab-only');
      const insertUploadSlab=addToolbarMenuAction(insertDropdown.menu,'Slab Overlay: Upload Image',()=>document.getElementById('ov-add-photo')?.click(),{icon:ICON_MENU.upload});
      const insertLibrarySlab=addToolbarMenuAction(insertDropdown.menu,'Slab Overlay: Choose From Library',()=>document.getElementById('ov-lib-photo')?.click(),{icon:ICON_MENU.library});
      [insertUploadSlab,insertLibrarySlab].forEach(item=>item.classList.add('lc-slab-only'));
      ...
      const quickLayoutsMenu=addToolbarSubmenu(importDropdown.menu,'Quick Layouts',{icon:ICON_MENU.quickLayout});
     ...
     // ===== General helpers (also used for rounding/inputs) =====
      ...
      if(piecesCard&&piecesHead&&!piecesHead.dataset.toggleReady){
        ...
        pieceFillBtn.onclick=e=>{
          ...
          const mode=state.showSlabMaterial?'slab':(state.showPieceFills!==false?'color':'empty');
          if(mode==='color'){
            state.showPieceFills=false;
            state.showSlabMaterial=true;
          }else if(mode==='slab'){
            state.showPieceFills=false;
            state.showSlabMaterial=false;
          }else{
            state.showPieceFills=true;
            state.showSlabMaterial=false;
          }
  ...
  async function addRasterPNG() {
    ...
    sinksUI = initSinksCard({
      ...
      function appendMappedSlabMaterial(group,p,pathD,layoutCx,layoutCy){
        ...
        if(!L||L.showOverlays===false||!arr.some(o=>o?.visible&&o?.dataURL))return;
      ...
      function drawSlabWorkspace(){
        ...
        // The full slab image belongs only to the SLAB workspace.
        if(state.showSlabMaterial)drawOverlays();
        ...
        [...state.pieces].sort((a,b)=>(a.layer||0)-(b.layer||0)).forEach((p,index)=>{
          ...
          if(state.showEdgeProfiles&&p.edgeProfiles){
            ...
            const addEdgeLabel=(value,xPos,yPos,anchor='middle',rotation=0)=>{
              ...
              const t=svgEl('text',{x:xPos,y:yPos,'text-anchor':anchor,'dominant-baseline':'middle','font-size':'11',fill:'#111','paint-order':'stroke',stroke:'#fff','stroke-width':'2'});
  ...
  if (state.overlay && (state.overlay.dataURL || state.overlay.visible)){
    ...
    delete state.overlay;
      ...
      function drawOverlays(){
        ...
        if (!state.showSlabMaterial || !L || L.showOverlays===false || !arr.length) {
