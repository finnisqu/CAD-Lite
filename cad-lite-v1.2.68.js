  ...
  async function addRasterPNG() {
    ...
    sinksUI = initSinksCard({
      ...
      function constrainDrawPoint(start,end,force=false){
        ...
        if(ax>0&&ay/ax<=AUTO_RATIO)return {x:end.x,y:start.y};
        if(ay>0&&ax/ay<=AUTO_RATIO)return {x:start.x,y:end.y};
        return end;
      }
    ...
    pushHistory();
      ...
      svg.addEventListener('click',e=>{
        ...
        if(!seamTempStart){seamTempStart=snapped;state.selectedSeamId=null;}
          ...
          const end=constrainDrawPoint(seamTempStart,snapped,!!e.shiftKey);
          const cdx=end.x-seamTempStart.x,cdy=end.y-seamTempStart.y;
          if(Math.hypot(cdx,cdy)<0.01){e.stopPropagation();return;}
      ...
      svg.addEventListener('click', (e) => {
        ...
        if (!dimTempStart) {
          ...
          snapped=constrainDrawPoint(dimTempStart,snapped,!!e.shiftKey);
