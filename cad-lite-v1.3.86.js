   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      const insertQuickLayout=(key)=>{
        ...
        const spawnInset=grid*3;
        const left=Math.max(0,Math.min(spawnInset,state.cw-groupW));
        const top=Math.max(0,Math.min(spawnInset,state.ch-groupH));
        const originX=snap(left-minX,grid);
        const originY=snap(top-minY,grid);
  ...
  btnAddLayout.onclick = ()=>{
    ...
    del.innerHTML='<svg class="lc-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l1 2h4v2H3V5h4l1-2Zm-2 6h12l-1 11H7L6 9Zm3 2v7h2v-7H9Zm4 0v7h2v-7h-2Z" fill="currentColor"/></svg>';
        ...
        function syncCornerZone(key){
          const input=cornerInputs[key];
          if(input)input.classList.toggle('active',(Number(p.cornerRadii[key])||0)>0);
        }
        ...
        function makeCornerInput(key,titleText){
          ...
          input.addEventListener('focus',()=>{
            if((Number(input.value)||0)===0&&maxRadius>0){
              input.value=String(Math.min(1,maxRadius));
              preview();
              scheduleSave();
              pushHistory();
      ...
      btnAdd.onclick = () => {
        ...
        const grid=Math.max(.001,Math.abs(Number(state.grid)||1));
        const spawnInset=snap(grid*3,grid);
        const p = { 
          id: uid(), 
          name: `Piece ${idx+1}`, 
          w:40, 
          h:25.5, 
          x:Math.min(spawnInset,Math.max(0,state.cw-40)), 
          y:Math.min(spawnInset,Math.max(0,state.ch-25.5)), 
          rotation:0, 
          color:'#ffffff', 
          layer: top, 
          rTL:false, 
