   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      edgeModeRow.className='lc-edge-label-mode-row';
      const edgeModeLabel=document.createElement('span');
      edgeModeLabel.textContent='Edge Style';
      const edgeModeChoices=document.createElement('div');
      edgeModeChoices.className='lc-edge-label-mode-choices';
      [
        ['text','Text','<svg viewBox="0 0 18 14" aria-hidden="true"><path d="M3 3h12M5 7h8M6 11h6" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>'],
        ['symbol','Symbols','<svg viewBox="0 0 18 14" aria-hidden="true"><path d="M4 3l4 8M8 3l-4 8M11 4h4M13 2v4M11 10h4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>']
      ].forEach(([mode,label,icon])=>{
        const btn=document.createElement('button');
        btn.type='button';
        btn.className='lc-edge-label-mode-btn';
        btn.title=label+' edge labels';
      ...
      edgeModeRow.append(edgeModeLabel,edgeModeChoices);
     ...
     // ===== General helpers (also used for rounding/inputs) =====
      ...
      function drawEdgeProfileLegend(){
        if(!state.showEdgeProfiles||state.edgeLabelMode!=='symbol'||!state.showEdgeLegend)return;
        const used=new Set();
        ...
        const rows=[...EDGE_PROFILE_VALUES].filter(profile=>used.has(profile));
        const rowH=16;
        const boxW=142;
        ...
        g.appendChild(svgEl('rect',{
          ...
          fill:'#fff','fill-opacity':'.9',stroke:'#9ca3af','stroke-width':'1',
          'vector-effect':'non-scaling-stroke'
        }));
