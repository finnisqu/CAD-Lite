...
function renderSeamList(){
  ...
  L.seams.forEach((seam,idx)=>{
    const li=document.createElement('div');
    li.className='lc-item nav'+(state.selectedSeamId===seam.id?' selected':'');
    li.style.display='flex';
    li.style.alignItems='center';
    li.style.gap='8px';
    li.style.cursor='pointer';

    const dx=seam.x2-seam.x1,dy=seam.y2-seam.y1;
    ...
    const title=document.createElement('div');
    title.textContent=(String(seam.name||'').trim()||`Seam ${idx+1}`);
    title.style.whiteSpace='nowrap';
    title.style.overflow='hidden';
    title.style.textOverflow='ellipsis';
    title.title=title.textContent;
    ...
    del.onclick=e=>{
      e.stopPropagation();
      L.seams=L.seams.filter(x=>x.id!==seam.id);
      if(state.selectedSeamId===seam.id)state.selectedSeamId=null;
      renderSeamList();
      draw();
      scheduleSave();
      pushHistory();
    ...
    li.append(label,rename,del);
    seamList.appendChild(li);
  });

  renderEmptyState(seamList,'No seams');
  updateSidebarCounts();
}
...
function renderNoteList(){
  ...
  L.notes.forEach((n,idx)=>{
    ...
    label.onclick=()=>{
      const el=svg.querySelector(`g[data-note-id="${n.id}"] text`);
      if(el)openInlineNoteEditor(el,n.text||'',raw=>{
    ...
    del.onclick=e=>{
      e.stopPropagation();
      L.notes=L.notes.filter(x=>x.id!==n.id);
      draw();
      renderNoteList();
      scheduleSave();
      pushHistory();
