   (function(){
    function init(){
      ...
      function findOpenSlabPosition(w,h,ignoreId=null){
        ...
        if(!found){
          ...
          for(let y=SLAB_GUTTER;y+height<=ch&&!found;y+=SLAB_GUTTER){
            for(let x=SLAB_GUTTER;x+width<=cw;x+=SLAB_GUTTER){
              const pos={x,y};
    ...
    window.addEventListener('keydown', (e) => {
      ...
      const startGuideMenuItem=addToolbarMenuAction(helpDropdown.menu,'Start Guided Workflow',()=>startGuidedWorkflow(),{});
      ...
      const guidedWorkflowSteps=[
        {
          title:'Add a slab',
          body:'Start in SLAB. Expand SLABS and choose + Add Slab. Upload a photo or choose one from the library, crop it, then enter the real slab dimensions.',
          workspace:'slab',
        ...
        },
        {
          ...
          body:'Switch to DESIGN and add pieces. For a fast starting point, INSERT → Quick Layouts can generate a common kitchen or vanity arrangement.',
          workspace:'layout',
          target:()=>document.querySelector('.lc-add-piece-top')||document.getElementById('lc-add')
        },
        {
          title:'Build the installed layout',
          body:'Move, size, rotate, and snap the countertop pieces into their installed positions. DESIGN placement and SLAB placement stay independent.',
          workspace:'layout',
        ...
        },
        {
          ...
          body:'Return to SLAB. Move and rotate the same pieces over the slab images. These coordinates do not disturb the installed DESIGN layout.',
          workspace:'slab',
          target:()=>document.getElementById('lc-svg')
        },
        {
          title:'Preview Color, Slab, or Empty',
          body:'Use the Pieces header display button—or VIEW → Pieces → Piece Display—to switch between color fill, mapped slab imagery, and empty outlines.',
          workspace:'layout',
          target:()=>document.querySelector('.lc-piece-fill-toggle')||document.getElementById('lc-view-menu-btn')
        },
        {
          title:'Review and export',
          body:'When the job is ready, EXPORT can create the current-layout PDF, all-layout PDF, PNG, SVG, project JSON, or a share link.',
          workspace:'layout',
      ...
      function clearGuideHighlight(){
        guideHighlight?.classList.remove('lc-guide-highlight');
        guideHighlight=null;
      }
      ...
      function renderGuidedWorkflowStep(){
        ...
        requestAnimationFrame(()=>{
          ...
          if(!target)return;
          guideHighlight=target;
          target.classList.add('lc-guide-highlight');
          if(target.id!=='lc-svg'){
