    ...
    pushHistory();
      ...
      function drawSeams(){
        ...
        L.seams.forEach(seam=>{
          ...
          g.append(hit,line);
          g.addEventListener('pointerdown',ev=>{if(state.dimTool||state.seamTool)return;ev.preventDefault();ev.stopPropagation();state.selectedSeamId=seam.id;state.selectedDimId=null;clearSelection();renderSeamList();draw();});
