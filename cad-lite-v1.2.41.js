   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      const noteToolCtl=document.createElement('div');
      noteToolCtl.className='lc-ctl'; noteToolCtl.style.marginTop='6px'; noteToolCtl.appendChild(btnNoteTool);
      const dimToolCtl0=btnDimTool?.closest('.lc-ctl');
      if(dimToolCtl0?.parentElement) dimToolCtl0.parentElement.insertBefore(noteToolCtl,dimToolCtl0.nextSibling);
...
if(btnAddLayout){
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      // Note Tool gets first chance at any canvas click, including clicks on countertop pieces.
      svg.addEventListener('pointerdown', (e)=>{
        if(!state.noteTool) return;
        if(e.target.closest && e.target.closest('g[data-note-id]')) return;
        e.preventDefault(); e.stopPropagation();
        const pt=svgPoint(e);
        ...
        requestAnimationFrame(()=>{
          ...
          if(el) openInlineDimEditor(el,'',raw=>{
            const v=String(raw??'').trim();
            if(!v){cur().notes=cur().notes.filter(n=>n.id!==note.id);draw();scheduleSave();return true;}
            note.text=v;draw();scheduleSave();pushHistory();return true;
          });
        });
        blankDown=null;
      }, true);
