   (function(){
    function init(){
      ...
      function selectOnly(id){ setSelection([id]); state.selectedDimId=null; }
    ...
    window.addEventListener('keydown', (e) => {
      ...
      btnNoteTool.onclick=()=>{state.noteTool=!state.noteTool;if(state.noteTool){state.dimTool=false;syncDimToolUI?.();}syncNoteToolUI();}; syncNoteToolUI();
         ...
         // append rotated geometry group to a piece container g (for pointerdown)
          ...
          if (state.showEdgeProfiles && p.edgeProfiles) {
            ...
            function addEdgeLabel(text, x, y, anchor, baseline, rotation) {
              ...
              t.setAttribute('x',lx);t.setAttribute('y',ly);t.setAttribute('dominant-baseline','middle');t.setAttribute('text-anchor','middle');t.setAttribute('font-size','14');t.textContent=label;
              const hit=document.createElementNS(svgNS,'rect');
              const hitW=Math.max(52,label.length*8+18),hitH=28;
              hit.setAttribute('x',lx-hitW/2);hit.setAttribute('y',ly-hitH/2);hit.setAttribute('width',hitW);hit.setAttribute('height',hitH);hit.setAttribute('rx','5');hit.setAttribute('fill','transparent');hit.setAttribute('pointer-events','all');hit.style.cursor='grab';
              let suppressDimClick=false;
              const beginOffsetDrag=ev=>{
                ev.preventDefault();ev.stopPropagation();state.selectedDimId=d.id;state.selectedId=null;renderDimList();updateInspector();
                const startPt=svgPoint(ev),startOff=(typeof d.offsetPx==='number'?d.offsetPx:12);let moved=false;
                const move=mv=>{const q=svgPoint(mv);if(Math.hypot(q.x-startPt.x,q.y-startPt.y)>3)moved=true;if(!moved)return;const delta=(q.x-startPt.x)*nx+(q.y-startPt.y)*ny;d.offsetPx=Math.max(-300,Math.min(300,startOff+delta));draw();};
                const up=()=>{svg.removeEventListener('pointermove',move);svg.removeEventListener('pointerup',up);svg.removeEventListener('pointercancel',up);if(moved){suppressDimClick=true;scheduleSave();pushHistory();setTimeout(()=>suppressDimClick=false,0);}};
                svg.addEventListener('pointermove',move);svg.addEventListener('pointerup',up);svg.addEventListener('pointercancel',up);
              };
              hit.addEventListener('pointerdown',beginOffsetDrag);
              hit.addEventListener('dblclick',ev=>{ev.preventDefault();ev.stopPropagation();if(suppressDimClick)return;openInlineDimEditor(t,distIn.toFixed(2),raw=>{const next=parseDimInches(raw);if(!(next>0))return false;const vx=d.x2-d.x1,vy=d.y2-d.y1,oldLen=Math.sqrt(vx*vx+vy*vy)||1;d.x2=round3(d.x1+(vx/oldLen)*next);d.y2=round3(d.y1+(vy/oldLen)*next);draw();renderDimList();scheduleSave();pushHistory();return true;});});
              ...
              hit.setAttribute('transform', `rotate(${angleDeg}, ${lx}, ${ly})`);

              g.append(ext1, ext2, dl, hit, t);
...
if(btnAddLayout){
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      btnDimTool && (btnDimTool.onclick=()=>{state.dimTool=!state.dimTool;if(state.dimTool){state.noteTool=false;syncNoteToolUI();}if(!state.dimTool)dimTempStart=null;syncDimToolUI();});
