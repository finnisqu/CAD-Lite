   (function(){
    function init(){
      ...
      async function openPlanImageEditor(dataURL,name){
        ...
        let previewZoom=1;
        let snapCalibration=true;

        const modal=planDialogShell('Prepare Floor Plan');
        modal.dialog.classList.add('lc-plan-import-dialog');

        const toolsRow=document.createElement('div');
        toolsRow.className='lc-plan-editor-tools';
        const cropBtn=planButton('Crop',()=>{mode='crop';syncMode();},'ghost');
        const levelBtn=planButton('Level Line',()=>{mode='level';syncMode();},'ghost');
        const calibrateBtn=planButton('Calibrate',()=>{mode='calibrate';syncMode();},'ghost');
        const resetCropBtn=planButton('Reset Crop',()=>{
          crop={x:0,y:0,w:work.width,h:work.height};
          levelLine=null;
          calibrationLine=null;
          renderPreview();
        },'ghost');
        const rotateLeftBtn=planButton('Rotate Left',()=>{
          work=rotatePlanCanvas(work,-90);
          crop={x:0,y:0,w:work.width,h:work.height};
          levelLine=null;calibrationLine=null;transformed=true;renderPreview();
        },'ghost');
        const rotateRightBtn=planButton('Rotate Right',()=>{
          work=rotatePlanCanvas(work,90);
          crop={x:0,y:0,w:work.width,h:work.height};
          levelLine=null;calibrationLine=null;transformed=true;renderPreview();
        },'ghost');
        const rotateFineLeftBtn=planButton('↺ 0.25°',()=>{
          work=rotatePlanCanvas(work,-.25);
          crop={x:0,y:0,w:work.width,h:work.height};
          levelLine=null;calibrationLine=null;transformed=true;renderPreview();
        },'ghost');
        const rotateFineRightBtn=planButton('↻ 0.25°',()=>{
          work=rotatePlanCanvas(work,.25);
          crop={x:0,y:0,w:work.width,h:work.height};
          levelLine=null;calibrationLine=null;transformed=true;renderPreview();
        },'ghost');
        const flipHBtn=planButton('Flip Horizontal',()=>{
          work=flipPlanCanvas(work,true,false);
          crop={x:0,y:0,w:work.width,h:work.height};
          levelLine=null;calibrationLine=null;transformed=true;renderPreview();
        },'ghost');
        const flipVBtn=planButton('Flip Vertical',()=>{
          work=flipPlanCanvas(work,false,true);
          crop={x:0,y:0,w:work.width,h:work.height};
          levelLine=null;calibrationLine=null;transformed=true;renderPreview();
        },'ghost');
        toolsRow.append(cropBtn,levelBtn,calibrateBtn,resetCropBtn,rotateLeftBtn,rotateRightBtn,rotateFineLeftBtn,rotateFineRightBtn,flipHBtn,flipVBtn);

        const zoomRow=document.createElement('div');
        zoomRow.className='lc-plan-preview-controls';
        const zoomOut=planButton('Zoom −',()=>{previewZoom=Math.max(.5,previewZoom-.25);renderPreview();},'ghost');
        const zoomIn=planButton('Zoom +',()=>{previewZoom=Math.min(4,previewZoom+.25);renderPreview();},'ghost');
        const zoomFit=planButton('Fit',()=>{previewZoom=1;renderPreview();},'ghost');
        const zoomRead=document.createElement('span');
        zoomRead.className='lc-small lc-plan-muted lc-plan-zoom-read';
        zoomRow.append(zoomOut,zoomIn,zoomFit,zoomRead);

        const help=document.createElement('div');
        help.className='lc-plan-help lc-plan-editor-help';

        const calibrationRow=document.createElement('div');
        calibrationRow.className='lc-plan-calibration-controls';
        const distanceLabel=document.createElement('label');
        distanceLabel.className='lc-plan-calibration-distance';
        const distanceText=document.createElement('span');
        distanceText.textContent='Known Distance';
        const distanceInput=document.createElement('input');
        distanceInput.type='text';
        distanceInput.className='lc-input';
        distanceInput.placeholder='8\' 6" or 102.5';
        distanceLabel.append(distanceText,distanceInput);
        const snapLabel=document.createElement('label');
        snapLabel.className='lc-plan-check-row';
        const snapCheck=document.createElement('input');
        snapCheck.type='checkbox';
        snapCheck.checked=true;
        snapCheck.onchange=()=>{snapCalibration=snapCheck.checked;};
        const snapText=document.createElement('span');
        snapText.textContent='Snap calibration points to crop edges/corners';
        snapLabel.append(snapCheck,snapText);
        const calStatus=document.createElement('div');
        calStatus.className='lc-small lc-plan-muted';
        calibrationRow.append(distanceLabel,snapLabel,calStatus);
        ...
        resultTitle.className='lc-small lc-plan-section-title';
        ...
        resultPreview.width=420;
        resultPreview.height=240;
        resultWrap.append(resultTitle,resultPreview);

        const metaRow=document.createElement('div');
        metaRow.className='lc-plan-editor-meta';
        modal.body.append(toolsRow,zoomRow,help,calibrationRow,previewWrap,resultWrap,metaRow);

        let map={scale:1,ox:0,oy:0};
        const ctx=preview.getContext('2d');

        const pointInCrop=p=>p&&p.x>=crop.x&&p.y>=crop.y&&p.x<=crop.x+crop.w&&p.y<=crop.y+crop.h;
        const calibrationPixelDistance=()=>calibrationLine
          ? Math.hypot(calibrationLine.x2-calibrationLine.x1,calibrationLine.y2-calibrationLine.y1)
          : 0;

        const syncMode=()=>{
          cropBtn.classList.toggle('alt',mode==='crop');
          levelBtn.classList.toggle('alt',mode==='level');
          calibrateBtn.classList.toggle('alt',mode==='calibrate');
          calibrationRow.classList.toggle('is-active',mode==='calibrate');
          ...
          help.textContent=mode==='crop'
            ...
            : (mode==='level'
              ...
              : 'Drag Point A to Point B across a known plan dimension, then enter the real distance below.');
          renderPreview();
        };
        ...
        const renderPreview=()=>{
          const baseW=980,baseH=620;
          preview.width=Math.max(490,Math.round(baseW*previewZoom));
          preview.height=Math.max(310,Math.round(baseH*previewZoom));
          ctx.clearRect(0,0,preview.width,preview.height);
          ctx.fillStyle='#f3f4f6';
          ctx.fillRect(0,0,preview.width,preview.height);
          const pad=18*previewZoom;
          ...
          if(calibrationLine){
            ...
            ctx.lineWidth=Math.max(2,3*previewZoom);
            ctx.beginPath();
            ctx.moveTo(ox+calibrationLine.x1*sc,oy+calibrationLine.y1*sc);
            ctx.lineTo(ox+calibrationLine.x2*sc,oy+calibrationLine.y2*sc);
            ctx.stroke();
            [ [calibrationLine.x1,calibrationLine.y1], [calibrationLine.x2,calibrationLine.y2] ].forEach((pt,idx)=>{
              ctx.beginPath();
              ctx.arc(ox+pt[0]*sc,oy+pt[1]*sc,Math.max(4,5*previewZoom),0,Math.PI*2);
              ctx.fillStyle=idx===0?'#16a34a':'#fff';
              ctx.fill();
              ctx.strokeStyle='#16a34a';
              ctx.stroke();
            });
          ...
          zoomRead.textContent=Math.round(previewZoom*100)+'%';
          metaRow.textContent=Math.round(crop.w)+' × '+Math.round(crop.h)+' px selected'+(transformed?' · image adjusted':'');
          const px=calibrationPixelDistance();
          const known=parsePlanDistance(distanceInput.value);
          if(calibrationLine&&px>0){
            calStatus.textContent=(known>0)
              ? 'Calibration ready · '+known.toFixed(3)+' in over '+px.toFixed(1)+' px'
              : 'Calibration line ready · enter the known distance.';
          }else{
            calStatus.textContent='Calibration optional.';
          }
          renderCropPreview();
        };
        ...
        const snapCalibrationPoint=p=>{
          if(!snapCalibration)return p;
          const threshold=Math.max(2,12/Math.max(.01,map.scale));
          const xs=[crop.x,crop.x+crop.w];
          const ys=[crop.y,crop.y+crop.h];
          let x=p.x,y=p.y;
          xs.forEach(v=>{if(Math.abs(x-v)<=threshold)x=v;});
          ys.forEach(v=>{if(Math.abs(y-v)<=threshold)y=v;});
          return {x,y};
        };

        preview.addEventListener('pointerdown',e=>{
          let p=localPoint(e);
          if(mode==='calibrate')p=snapCalibrationPoint(p);
          drag={start:p,current:p};
          preview.setPointerCapture?.(e.pointerId);
          if(mode==='level')levelLine={x1:p.x,y1:p.y,x2:p.x,y2:p.y};
          if(mode==='calibrate')calibrationLine={x1:p.x,y1:p.y,x2:p.x,y2:p.y};
        });
        ...
        preview.addEventListener('pointermove',e=>{
          ...
          drag.current=p;
          if(mode==='crop'){
            const x=Math.min(drag.start.x,drag.current.x),y=Math.min(drag.start.y,drag.current.y);
            crop={x,y,w:Math.max(1,Math.abs(drag.current.x-drag.start.x)),h:Math.max(1,Math.abs(drag.current.y-drag.start.y))};
            calibrationLine=null;
          }else if(mode==='level'){
            levelLine={x1:drag.start.x,y1:drag.start.y,x2:drag.current.x,y2:drag.current.y};
          }else{
            calibrationLine={x1:drag.start.x,y1:drag.start.y,x2:drag.current.x,y2:drag.current.y};
          }
          renderPreview();
        });
        ...
        const finishDrag=e=>{
          ...
          if(mode==='calibrate'){
            if(Math.hypot(end.x-start.x,end.y-start.y)<6)calibrationLine=null;
            renderPreview();
            return;
          }
        ...
        distanceInput.addEventListener('input',renderPreview);
        ...
        const use=planButton('Import Plan',()=>{
          ...
          const calPx=calibrationPixelDistance();
          const actual=parsePlanDistance(distanceInput.value);
          const wantsCalibration=!!calibrationLine||String(distanceInput.value||'').trim().length>0;
          const calInside=calibrationLine&&pointInCrop({x:calibrationLine.x1,y:calibrationLine.y1})&&pointInCrop({x:calibrationLine.x2,y:calibrationLine.y2});
          if(wantsCalibration&&(!(calPx>1)||!(actual>0)||!calInside)){
            alert('Finish the calibration line inside the crop and enter a valid known distance, or clear the calibration distance to import without calibration.');
            return;
          }
          ...
          let w,h,calibrated=false,calibration=null;
          if(calPx>1&&actual>0&&calInside){
            const inchesPerSourcePixel=actual/calPx;
            w=sw*inchesPerSourcePixel;
            h=sh*inchesPerSourcePixel;
            calibrated=true;
            calibration={
              knownDistance:actual,
              sourcePixelDistance:calPx,
              inchesPerSourcePixel:inchesPerSourcePixel,
              updatedAt:new Date().toISOString(),
