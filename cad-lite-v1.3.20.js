  ...
  "pieces": [
    ...
    { "id": "ycid73l", "name": "Backsplash", "w": 4, "h": 22.5, "x": 159, "y": 10, "rotation": 0, "color": "#d5f0f0", "layer": 8, "rTL": false, "rTR": false, "rBL": false, "rBR": false, "sinks": [] }
      ...
      function makeListCardToggle(listEl,label,visibilityKey,shortcut,onAdd){
        ...
        head.classList.add('lc-two-row-head');
        ...
        titleRow.title=`Press ${shortcut} to add a new ${addThing}`;
        ...
        actionsRow.className='lc-head-actions-row';
        ...
        actionsRow.append(addBtn,eye);
        head.replaceChildren(titleRow,actionsRow);
      ...
      syncListVisibilityEyes();
      // Pieces: title/chevron on row one, Add Piece on row two.
      ...
      if(piecesCard&&piecesHead&&!piecesHead.dataset.toggleReady){
        ...
        piecesHead.classList.add('lc-two-row-head');
        ...
        titleRow.append(piecesArrow);

        const actionsRow=document.createElement('div');
        actionsRow.className='lc-head-actions-row';
        if(addPieceBtn){
          addPieceBtn.classList.add('lc-sidebar-add');
          addPieceBtn.textContent='+ Add Piece';
          actionsRow.append(addPieceBtn);
        }

        piecesHead.replaceChildren(titleRow,actionsRow);
      ...
      function makeSidebarCardCollapsible(card,{defaultOpen=true,twoRow=false,actionButton=null}={}){
        ...
        if(twoRow&&actionButton){
          const actionsRow=document.createElement('div');
          actionsRow.className='lc-head-actions-row';
          actionButton.classList.add('lc-sidebar-add');
      ...
      if(overlayAccordion&&piecesCard?.parentElement){
        overlayAccordion.classList.remove('lc-span-full');
        overlayAccordion.classList.add('lc-card','lc-sidebar-overlay-card');
        piecesCard.insertAdjacentElement('afterend',overlayAccordion);
      }
      ...
      makeSidebarCardCollapsible(layoutsCard,{defaultOpen:true,twoRow:true,actionButton:btnAddLayout});
  ...
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      function updateInspector(){
        ...
        r3.style.gridTemplateColumns='minmax(0,1fr)';
        r3.appendChild(rotField);
    ...
    del.innerHTML='<svg class="lc-icon" viewBox="0 0 24 24"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2m-1 0v14a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2V6h10z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
        ...
        edgeDiagram.className='lc-edge-diagram';

        const piecePreview=document.createElement('div');
        piecePreview.className='lc-edge-piece-preview';
        const piecePreviewLabel=document.createElement('span');
        piecePreviewLabel.textContent='PIECE';
        piecePreview.appendChild(piecePreviewLabel);
        ...
        edgeDiagram.append(
          makeSpatialEdgeSelect('top','Top'),
          makeSpatialEdgeSelect('right','Right'),
          makeSpatialEdgeSelect('bottom','Bottom'),
