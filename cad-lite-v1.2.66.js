   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      function syncNoteToolUI(){const on=!!state.noteTool;btnNoteTool.textContent=on?'Note Tool: On':'Note Tool: Off';btnNoteTool.classList.toggle('alt',on);btnNoteTool.classList.toggle('ghost',!on);btnNoteTool.style.background=on?'#e5e7eb':'#fff';btnNoteTool.style.color='#111';if(svg)svg.style.cursor=on?'text':(state.dimTool?'crosshair':'');}
      ...
      function syncSeamToolUI(){const on=!!state.seamTool;btnSeamTool.textContent=on?'Seam Tool: On':'Seam Tool: Off';btnSeamTool.classList.toggle('alt',on);btnSeamTool.classList.toggle('ghost',!on);btnSeamTool.style.background=on?'#e5e7eb':'#fff';btnSeamTool.style.color='#111';if(svg)svg.style.cursor=on?'crosshair':(state.noteTool?'text':(state.dimTool?'crosshair':''));}
  ...
  if (state.overlay && (state.overlay.dataURL || state.overlay.visible)){
    ...
    delete state.overlay;
        ...
        sorted.forEach((p)=>{
         ...
         // append rotated geometry group to a piece container g (for pointerdown)
          ...
          gPiece.addEventListener('pointerdown', (e)=>{
            if(state.seamTool) return; // seam placement owns clicks while active
        ...
        if (L && Array.isArray(L.dims) && L.dims.length && state.showManualDims) {
          ...
          const offPx = 12; // perpendicular offset in px from the measured segment
            ...
            L.dims.forEach(d => {
              ...
              g.addEventListener('pointerdown',(ev)=>{ev.stopPropagation();state.selectedDimId=d.id;state.selectedId=null;renderDimList();updateInspector();draw();});
      ...
      function pieceItem(p, i){
        ...
        // Piece name editing is handled by the edit button below so row selection cannot rerender it.
        ...
        btnEdit.className='lc-btn ghost lc-iconbtn';btnEdit.type='button';btnEdit.title='Rename piece';btnEdit.textContent='✎';
        btnEdit.addEventListener('click',(e)=>{
          e.preventDefault();e.stopPropagation();
          const old=p.name||'Piece',input=document.createElement('input');
          input.className='lc-input';input.value=old;input.style.flex='1';input.style.minWidth='0';
          line.replaceWith(input);input.focus();input.select();
          let done=false;
          const finish=save=>{if(done)return;done=true;if(save){const v=input.value.trim();if(v&&v!==old){p.name=v;scheduleSave();pushHistory();}}renderList();draw();updateInspector();};
...
function addListRenameButton(parent,getValue,onSave,title='Rename'){
  const edit=document.createElement('button');
  edit.type='button';edit.className='lc-btn ghost lc-iconbtn';edit.title=title;edit.textContent='✎';
  edit.onclick=e=>{
    e.preventDefault();e.stopPropagation();
    const old=String(getValue()||'');
    const input=document.createElement('input');
    input.className='lc-input';input.value=old;input.style.flex='1';input.style.minWidth='0';
    parent.insertBefore(input,edit);edit.style.display='none';input.focus();input.select();
    let done=false;
    const finish=save=>{
      if(done)return;done=true;
      if(save){const v=input.value.trim();if(v&&v!==old){onSave(v);scheduleSave();pushHistory();}}
      input.remove();edit.style.display='';
      renderList();renderDimList();renderNoteList();renderSeamList();draw();updateInspector();
    };
...
function renderDimList(){
  if(!dimList)return;const L=cur();dimList.innerHTML='';
  if(!L||!Array.isArray(L.dims)){renderEmptyState(dimList,'No dimensions');updateSidebarCounts();return;}
  L.dims.forEach((d,idx)=>{
    const li=document.createElement('div');li.className='lc-item nav'+(state.selectedDimId===d.id?' selected':'');li.style.display='flex';li.style.alignItems='center';li.style.gap='8px';
    const dx=d.x2-d.x1,dy=d.y2-d.y1,dist=Math.sqrt(dx*dx+dy*dy),ang=(Math.atan2(dy,dx)*180/Math.PI+360)%180;
    const label=document.createElement('div');label.style.flex='1';label.textContent=`${d.name||('Dim '+(idx+1))}: ${dist.toFixed(2)}" (${ang<45||ang>135?'Horiz':'Vert'})`;
    label.onclick=()=>{state.selectedDimId=d.id;state.selectedId=null;renderDimList();updateInspector();draw();};
    addListRenameButton(li,()=>d.name||('Dim '+(idx+1)),v=>{d.name=v;},'Rename dimension');
    const del=document.createElement('button');del.type='button';del.className='lc-btn ghost';del.title='Delete dimension';del.textContent='🗑';del.onclick=e=>{e.stopPropagation();L.dims=L.dims.filter(x=>x.id!==d.id);if(state.selectedDimId===d.id)state.selectedDimId=null;renderDimList();draw();scheduleSave();pushHistory();};
    li.insertBefore(label,li.firstChild);li.appendChild(del);dimList.appendChild(li);
  });
  renderEmptyState(dimList,'No dimensions');updateSidebarCounts();
}

function renderSeamList(){
  if(!seamList)return;const L=cur();seamList.innerHTML='';
  if(!L||!Array.isArray(L.seams)){renderEmptyState(seamList,'No seams');updateSidebarCounts();return;}
  L.seams.forEach((s,idx)=>{
    const li=document.createElement('div');li.className='lc-item nav'+(state.selectedSeamId===s.id?' selected':'');li.style.display='flex';li.style.alignItems='center';li.style.gap='8px';
    const dx=s.x2-s.x1,dy=s.y2-s.y1,len=Math.sqrt(dx*dx+dy*dy);
    const label=document.createElement('div');label.style.flex='1';label.textContent=`${s.name||('Seam '+(idx+1))}: ${len.toFixed(2)}"`;label.onclick=()=>{state.selectedSeamId=s.id;state.selectedDimId=null;clearSelection();renderSeamList();draw();};
    li.appendChild(label);addListRenameButton(li,()=>s.name||('Seam '+(idx+1)),v=>{s.name=v;},'Rename seam');
    const del=document.createElement('button');del.type='button';del.className='lc-btn ghost';del.title='Delete seam';del.textContent='🗑';del.onclick=e=>{e.stopPropagation();L.seams=L.seams.filter(x=>x.id!==s.id);if(state.selectedSeamId===s.id)state.selectedSeamId=null;renderSeamList();draw();scheduleSave();pushHistory();};li.appendChild(del);seamList.appendChild(li);
  });
  renderEmptyState(seamList,'No seams');updateSidebarCounts();
}

function renderNoteList(){
  if(!noteList)return;const L=cur();noteList.innerHTML='';
  if(!L||!Array.isArray(L.notes)){renderEmptyState(noteList,'No notes');updateSidebarCounts();return;}
  L.notes.forEach((n,idx)=>{
    const li=document.createElement('div');li.className='lc-item nav';li.style.display='flex';li.style.alignItems='center';li.style.gap='8px';
    const label=document.createElement('div');label.style.flex='1';label.style.whiteSpace='pre-wrap';label.textContent=n.listName||n.text||`Note ${idx+1}`;
    label.onclick=()=>{const el=svg.querySelector(`g[data-note-id="${n.id}"] text`);if(el)openInlineNoteEditor(el,n.text||'',raw=>{const v=String(raw??'').trim();if(!v)return false;n.text=v;draw();renderNoteList();scheduleSave();pushHistory();return true;});};
    li.appendChild(label);addListRenameButton(li,()=>n.listName||n.text||('Note '+(idx+1)),v=>{n.listName=v;},'Rename note in list');
    const del=document.createElement('button');del.type='button';del.className='lc-btn ghost';del.title='Delete note';del.textContent='🗑';del.onclick=e=>{e.stopPropagation();L.notes=L.notes.filter(x=>x.id!==n.id);draw();renderNoteList();scheduleSave();pushHistory();};li.appendChild(del);noteList.appendChild(li);
  });
  renderEmptyState(noteList,'No notes');updateSidebarCounts();
}
    ...
    pushHistory();
      ...
      function drawSeams(){
        ...
        L.seams.forEach(seam=>{
          const g=document.createElementNS(svgNS,'g');g.setAttribute('data-seam-id',seam.id);g.style.cursor='pointer';
          ...
          g.append(hit,line);
          g.addEventListener('pointerdown',ev=>{ev.preventDefault();ev.stopPropagation();state.selectedSeamId=seam.id;state.selectedDimId=null;clearSelection();renderSeamList();draw();});
          if(state.selectedSeamId===seam.id){
            const handle=(which,x,y)=>{const h=svgEl('circle',{cx:i2p(x),cy:i2p(y),r:5,fill:'#fff',stroke:'#2563eb','stroke-width':2,'vector-effect':'non-scaling-stroke'});h.style.cursor='crosshair';h.addEventListener('pointerdown',ev=>{ev.preventDefault();ev.stopPropagation();const move=mv=>{const q=svgPoint(mv),sn=snapSeamPoint({x:p2i(q.x),y:p2i(q.y)});if(which===1){seam.x1=round3(sn.x);seam.y1=round3(sn.y);}else{seam.x2=round3(sn.x);seam.y2=round3(sn.y);}draw();};const up=()=>{document.removeEventListener('pointermove',move,true);document.removeEventListener('pointerup',up,true);document.removeEventListener('pointercancel',up,true);renderSeamList();scheduleSave();pushHistory();};document.addEventListener('pointermove',move,true);document.addEventListener('pointerup',up,true);document.addEventListener('pointercancel',up,true);});g.appendChild(h);};
      ...
      function syncDimToolUI(){
        ...
        if(svg)svg.style.cursor=on?'crosshair':(state.noteTool?'text':'');
      }
      ...
      svg.addEventListener('click',e=>{
        ...
        if(e.target.closest&&e.target.closest('g[data-seam-id]')) return;
        ...
        if(!seamTempStart){seamTempStart=snapped;state.selectedSeamId=null;}
          ...
          const seam={id:uid(),x1:seamTempStart.x,y1:seamTempStart.y,x2:snapped.x,y2:snapped.y};
          L.seams.push(seam);state.selectedSeamId=seam.id;seamTempStart=null;state.seamTool=false;syncSeamToolUI();draw();renderSeamList();scheduleSave();pushHistory();
        }
        e.stopPropagation();
      });
