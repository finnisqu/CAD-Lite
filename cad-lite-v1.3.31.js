    ...
    doc.addImage(dataURL, 'PNG', imgX, imgY, imgW, imgH, undefined, 'FAST');
      ...
      function initSinksCard({ uiMountEl, getSelectedPiece, onStateChange }) {
        ...
        function buildSinkEditor(sink, idx, piece){
          ...
          // Row 3: faucet holes — full-width rack.
          // Use a non-label wrapper so clicks in empty rack space cannot
          // implicitly toggle the first checkbox.
          const row3 = el('div','row faucet-hole-row');
          const faucetField = el('div','lc-label faucet-hole-field');
          faucetField.appendChild(el('div','lc-small','Faucet holes'));
          const rack = el('div', 'holes-row');
          for(let i=0;i<9;i++){
            const cb=document.createElement('input');
            cb.type='checkbox';
            cb.style.margin='0';
            cb.setAttribute('aria-label','Faucet hole '+(i+1));
          ...
          faucetField.appendChild(rack);
          row3.appendChild(faucetField);
