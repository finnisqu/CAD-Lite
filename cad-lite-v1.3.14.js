   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if(btnUndoTop){
        ...
        btnUndoTop.classList.add('lc-toolbar-icon');
      }
      if(btnRedoTop){
        ...
        btnRedoTop.classList.add('lc-toolbar-icon');
      }
      if(btnClearSel){
      ...
      clipOverlayViewCheck=addLayoutViewToggle(
        'Clip Slab Overlays to Pieces',
        ()=>!!ensureOverlaysOnLayout(activeLayout())?.overlayClip,
        checked=>{
          const L=ensureOverlaysOnLayout(activeLayout());if(!L)return;
          L.overlayClip=checked;
          draw();scheduleSave();pushHistory();syncClipTop?.();syncOverlayUI?.();
        }
      );
      });
      ...
      function syncViewMenuUI(){
        ...
        if(clipOverlayViewCheck){
          clipOverlayViewCheck.checked=!!ensureOverlaysOnLayout(activeLayout())?.overlayClip;
        }
        if(btnZoomOut){
      ...
      if(showNotesHost)showNotesHost.appendChild(zoomHost);
      ...
      btnResetTop.className='lc-btn ghost lc-iconbtn lc-toolbar-icon';
      ...
      document.addEventListener('pointerdown',e=>{
        toolbarMenus.forEach(item=>{
          if(!item.host.contains(e.target))item.setOpen(false);
        });
      });
  ...
  "pieces": [
    ...
    { "id": "ycid73l", "name": "Backsplash", "w": 4, "h": 22.5, "x": 159, "y": 10, "rotation": 0, "color": "#d5f0f0", "layer": 8, "rTL": false, "rTR": false, "rBL": false, "rBR": false, "sinks": [] }
      ...
      function makeListCardToggle(listEl,label,visibilityKey,shortcut,onAdd){
        ...
        addBtn.type='button';
        addBtn.className='lc-btn ghost lc-list-add';
        addBtn.textContent='+';
        addBtn.title=`Add ${label==='Dimensions'?'Dimension':label.slice(0,-1)} (${shortcut})`;
      ...
      // --- Main workspace order: lists | canvas | inspector ---
      ...
      const canvasCard    = col3?.querySelector('.lc-card');
      const inspectorPanel= col2?.querySelector('.lc-card:nth-child(1)');
      const importCard    = col2?.querySelector('.lc-card:nth-child(2)');
      const workspace=col2?.parentElement;
      if(workspace&&col2&&col3&&col2.parentElement===workspace&&col3.parentElement===workspace){
        workspace.insertBefore(col3,col2);
      }
