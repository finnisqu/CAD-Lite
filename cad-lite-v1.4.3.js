   (function(){
    function init(){
      ...
      async function renderPdfPlanPage(pdf,pageNumber,maxDim){
        const page=await pdf.getPage(pageNumber);
        const base=page.getViewport({scale:1});
        const scale=Math.max(.25,Math.min(5,(maxDim||1000)/Math.max(base.width,base.height)));
      ...
      async function openPlanPdfPicker(file){
        ...
        for(let i=1;i<=pdf.numPages;i++){
          ...
          opt.value=String(i);opt.textContent='Page '+i;
        ...
        wrap.className='lc-plan-pdf-preview';
        ...
        const previewPage=async()=>{
          ...
          busy.hidden=false;
          try{
            const c=await renderPdfPlanPage(pdf,Number(select.value)||1,900);
        ...
        const use=planButton('Use Page',async()=>{
          ...
          try{
            ...
            const c=await renderPdfPlanPage(pdf,n,3000);
      ...
      async function openPlanImageEditor(dataURL,name){
        ...
        const resetCropBtn=planButton('Reset Crop',()=>{crop={x:0,y:0,w:work.width,h:work.height};levelLine=null;renderPreview();},'ghost');
        const flipHBtn=planButton('Flip Horizontal',()=>{
          work=flipPlanCanvas(work,true,false);
          crop={x:0,y:0,w:work.width,h:work.height};
          levelLine=null;transformed=true;renderPreview();
        },'ghost');
        ...
        toolsRow.append(cropBtn,levelBtn,resetCropBtn,flipHBtn,flipVBtn);
        ...
        preview.width=980;preview.height=620;
        const previewWrap=document.createElement('div');
        previewWrap.className='lc-plan-editor-preview';
        previewWrap.appendChild(preview);

        const metaRow=document.createElement('div');
        metaRow.className='lc-plan-editor-meta';
        modal.body.append(toolsRow,help,previewWrap,metaRow);
        ...
        const syncMode=()=>{
          ...
          help.textContent=mode==='crop'
            ? 'Drag a box around the portion of the sheet you want to keep. The plan will be downscaled when imported.'
        ...
        const renderPreview=()=>{
          ...
          const pad=18;
          const sc=Math.min((preview.width-pad*2)/work.width,(preview.height-pad*2)/work.height);
