   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      const shortcutGroups=[
        ...
        },
        {
          ...
          rows:[
            ...
            {keys:[['Drag rotate handle']],action:'Rotate freely; soft-snap near 90° increments'},
     ...
     // ===== General helpers (also used for rounding/inputs) =====
      ...
      function workspacePiecePose(piece){
      ...
      function setPieceWorkspaceRotation(piece,angle,center){
        if(!piece||!center)return;
        const normalized=round3(((Number(angle)||0)%360+360)%360);
        if(state.workspace==='slab'){
          const sp=ensureSlabPlacement(piece);
          sp.rotation=normalized;
          const rs=slabRealSize(piece);
          sp.x=round3(center.x-rs.w/2);
          sp.y=round3(center.y-rs.h/2);
          clampSlabPlacement(piece);
        }else{
          piece.rotation=normalized;
          const rs=realSize(piece);
          piece.x=round3(center.x-rs.w/2);
          piece.y=round3(center.y-rs.h/2);
          clampToCanvas(piece);
        }
      }
      ...
      function drawPieceRotateHandle(){
        ...
        const piece=selectedSinglePiece();
        if(!piece)return;
        if(state.workspace!=='slab'&&isLinkedSplash(piece)&&isSplashSnappedToParent(piece))return;

        const pose=workspacePiecePose(piece);
        if(!pose)return;
        const cx=i2p(pose.cx),cy=i2p(pose.cy);
        const W0=i2p(Math.max(.25,Number(piece.w)||.25));
        const H0=i2p(Math.max(.25,Number(piece.h)||.25));
        const rad=(Number(pose.rotation)||0)*Math.PI/180;

        const rotatePoint=(lx,ly)=>({
          x:cx+lx*Math.cos(rad)-ly*Math.sin(rad),
          y:cy+lx*Math.sin(rad)+ly*Math.cos(rad)
        });
        const anchor=rotatePoint(W0/2,-H0/2);
        const handle=rotatePoint(W0/2+18,-H0/2-18);
        ...
        circle.setAttribute('aria-label','Rotate piece');

        const glyph=svgEl('text',{
          x:handle.x,y:handle.y+.5,
        ...
        circle.addEventListener('pointerdown',e=>{
          ...
          const livePose=workspacePiecePose(piece);
          if(!livePose)return;
          const center={x:livePose.cx,y:livePose.cy};
          const centerPx={x:i2p(center.x),y:i2p(center.y)};
          const pt=svgPoint(e);
          const startPointer=Math.atan2(pt.y-centerPx.y,pt.x-centerPx.x)*180/Math.PI;
          const startRotation=Number(livePose.rotation)||0;
          ...
          const move=ev=>{
            if(!pieceRotateDrag||pieceRotateDrag.pieceId!==piece.id)return;
            const q=svgPoint(ev);
            const current=Math.atan2(q.y-centerPx.y,q.x-centerPx.x)*180/Math.PI;
            const delta=((current-startPointer+540)%360)-180;
            ...
            if(ev.shiftKey){
              ...
              if(Math.abs(angle-snapAngle)<=5)angle=snapAngle;
            }

            angle=((angle%360)+360)%360;
            if(Math.abs(angle-pieceRotateDrag.angle)>.001)pieceRotateDrag.moved=true;
            pieceRotateDrag.angle=angle;
            setPieceWorkspaceRotation(piece,angle,center);
            draw();
          };
        ...
        circle.addEventListener('dblclick',e=>{
          ...
          if(!livePose)return;
          const center={x:livePose.cx,y:livePose.cy};
          setPieceWorkspaceRotation(piece,(Number(livePose.rotation)||0)+90,center);
          draw();
          updateInspector();
          scheduleSave();
          pushHistory();
        ...
        if(pieceRotateDrag&&pieceRotateDrag.pieceId===piece.id&&pieceRotateDrag.workspace===state.workspace){
          const label=svgEl('text',{
            x:handle.x+14,y:handle.y-12,
...
function createSinkRadiusAnnotation(piece,sink,corner){
  ...
  return note;
      ...
      function syncEdgePainterHUD(tool=activeMomentaryTool()){
        const canvasCol=document.querySelector('.lite-cad .lc-canvas-col');
        if(!canvasCol)return;
        let hud=canvasCol.querySelector('.lc-edge-painter-hud');
        ...
        if(!hud){
          ...
          select.onchange=()=>{
            edgePainterProfile=normalizeEdgeProfile(select.value);
...
if(btnAddLayout){
  btnAddLayout.onclick = ()=>{
    ...
    del.innerHTML='<svg class="lc-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l1 2h4v2H3V5h4l1-2Zm-2 6h12l-1 11H7L6 9Zm3 2v7h2v-7H9Zm4 0v7h2v-7h-2Z" fill="currentColor"/></svg>';
        ...
        edgeDesigner.append(edgeDiagram,manualRadiusButton,radiusLabelButton,edgePainterButton,edgeHint);
        syncMomentaryToolUI();
