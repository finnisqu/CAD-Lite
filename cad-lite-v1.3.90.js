    ...
    window.addEventListener('keydown', (e) => {
      ...
      const guidedWorkflowSteps=[
        ...
        },
        {
          ...
          workspace:'slab',
          targets:()=>[
            document.getElementById('lc-svg'),
            layoutWorkspaceTab||document.querySelector('.lc-canvas-tab')
          ],
          complete:()=>state.workspace!=='slab'
        },
        {
      ...
      const blockNonMomentaryInteraction=e=>{
        ...
        if(target instanceof Element){
          ...
          const onCanvasEntity=target.closest(
            'g[data-id], g[data-note-id], [data-overlay-hit="1"], [data-splash-edge-hit="1"], [data-dim-id], [data-line-id]'
          );
          if(!onCanvasEntity){
            cancelPlacementTools({redraw:false});
            return;
