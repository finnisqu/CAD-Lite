   (function(){
    function init(){
      ...
      async function openPlanImageEditor(dataURL,name){
        ...
        toolsRow.append(cropBtn,levelBtn,calibrateBtn,resetCropBtn,rotateLeftBtn,rotateRightBtn,flipHBtn,flipVBtn);
      ...
      function ensurePlanNavigatorCard(){
        ...
        layoutCard.insertAdjacentElement('afterend',card);
      ...
      function renderPlanNavigator(){
        ...
        const rotL=planButton('↺ 0.25°',()=>{
          plan.rotation=(Number(plan.rotation)||0)-.25;syncCanvasToPlan(plan);draw();scheduleSave();pushHistory();renderPlanNavigator();syncToolbarFromLayout?.();
        },'ghost');
        const rotR=planButton('↻ 0.25°',()=>{
          plan.rotation=(Number(plan.rotation)||0)+.25;syncCanvasToPlan(plan);draw();scheduleSave();pushHistory();renderPlanNavigator();syncToolbarFromLayout?.();
        },'ghost');

        [calibrate,crop,flipH,flipV,rotL,rotR].forEach(b=>b.disabled=!!plan.locked);
        buttons.append(calibrate,crop,flipH,flipV,rotL,rotR);
        ...
        nudgeGrid.append(blank,up,document.createElement('span'),left,down,right,document.createElement('span'),reset,document.createElement('span'));
        ...
        Object.entries(handles).forEach(([side,h])=>{
          ...
          handle.addEventListener('pointerdown',e=>{
            ...
            const move=ev=>{
              ...
              if(ev.shiftKey){
                newW=Math.max(.25,Math.round(newW*16)/16);
                newH=Math.max(.25,Math.round(newH*16)/16);
              }
