    ...
    function init(){
      ...
      async function openPlanImageEditor(dataURL,name){
        ...
        const resetCropBtn=iconify(
          planButton('Reset Crop',()=>{crop={x:0,y:0,w:work.width,h:work.height};levelLine=null;renderPreview();},'ghost'),
          'Reset',
        ...
        const rotateFineLeftBtn=iconify(
          planButton('Fine Rotate Left',()=>transformWork(src=>rotatePlanCanvas(src,-.25)),'ghost'),
          '0.25° L',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M6 5H2V1M3 5a7 7 0 1 1 0 8" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );
        const rotateFineRightBtn=iconify(
          planButton('Fine Rotate Right',()=>transformWork(src=>rotatePlanCanvas(src,.25)),'ghost'),
          '0.25° R',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M14 5h4V1M17 5a7 7 0 1 0 0 8" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );
        ...
        const zoomFit=iconify(
          planButton('Fit',()=>{previewZoom=1;renderPreview();},'ghost'),
          'Fit',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M2 7V2h5M13 2h5v5M18 11v5h-5M7 16H2v-5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>'
        );

        const eraseBtn=iconify(
          planButton('Eraser',()=>{mode='erase';syncMode();},'ghost'),
          'Eraser',
        ...
        controls.append(
          ...
          group('Rotate',rotateLeftBtn,rotateRightBtn,rotateFineLeftBtn,rotateFineRightBtn),
          group('Flip',flipHBtn,flipVBtn),
          group('Zoom',zoomOut,zoomIn,zoomFit),
        ...
        const renderPreview=()=>{
          ...
          if(levelLine){
            ...
            ctx.beginPath();
            ctx.moveTo(ox+levelLine.x1*sc,oy+levelLine.y1*sc);
            ctx.lineTo(ox+levelLine.x2*sc,oy+levelLine.y2*sc);
            ctx.stroke();
          }
          ...
          if(mode==='erase'){
            ...
            if(hover){
              ctx.beginPath();ctx.arc(ox+hover.x*sc,oy+hover.y*sc,r,0,Math.PI*2);ctx.stroke();
            }
            ctx.setLineDash([]);
          }
          ctx.restore();
        ...
        const localPoint=e=>{
          ...
          return {
            x:Math.max(0,Math.min(work.width,(px-map.ox)/map.scale)),
            y:Math.max(0,Math.min(work.height,(py-map.oy)/map.scale))
          };
        };
        ...
        const finishDrag=e=>{
          ...
          if(mode==='crop'){
            if(crop.w<8||crop.h<8)crop={x:0,y:0,w:work.width,h:work.height};
            renderPreview();return;
          }
          if(mode==='erase'){renderPreview();return;}
        ...
        const use=planButton('Import Plan',()=>{
          ...
          let compressed=out.toDataURL('image/webp',.78);
          if(!compressed.startsWith('data:image/webp'))compressed=out.toDataURL('image/jpeg',.82);
