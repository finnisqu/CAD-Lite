    ...
    function init(){
      ...
      async function exportAllLayoutsToPDF(){
        ...
        for (let i = 0; i < layouts.length; i++){
          ...
          const src = serializer.serializeToString(svgForExport(svg));
      ...
      // ===== v1.4 Floor Plan Underlay ==========================================
      // One plan belongs to one layout. It behaves as DESIGN canvas context,
      // not as another selectable drawing entity.
      let planFileInput=null;
      let planCalibration=null;
      let planModalCleanup=null;

      function normalizePlan(plan){
        if(!plan||typeof plan!=='object')return null;
        if(typeof plan.visible!=='boolean')plan.visible=true;
        if(typeof plan.locked!=='boolean')plan.locked=!!plan.calibrated;
        if(typeof plan.calibrated!=='boolean')plan.calibrated=false;
        if(typeof plan.includeInExport!=='boolean')plan.includeInExport=false;
        if(typeof plan.grayscale!=='boolean')plan.grayscale=false;
        if(typeof plan.flipX!=='boolean')plan.flipX=false;
        if(typeof plan.flipY!=='boolean')plan.flipY=false;
        if(!Number.isFinite(Number(plan.opacity)))plan.opacity=.4;
        plan.opacity=Math.max(.08,Math.min(1,Number(plan.opacity)||.4));
        if(!Number.isFinite(Number(plan.rotation)))plan.rotation=0;
        if(!Number.isFinite(Number(plan.margin)))plan.margin=0;
        plan.margin=Math.max(0,Number(plan.margin)||0);
        if(!Number.isFinite(Number(plan.offsetX)))plan.offsetX=0;
        if(!Number.isFinite(Number(plan.offsetY)))plan.offsetY=0;
        plan.offsetX=Number(plan.offsetX)||0;
        plan.offsetY=Number(plan.offsetY)||0;
        if(!Number.isFinite(Number(plan.natW)))plan.natW=0;
        if(!Number.isFinite(Number(plan.natH)))plan.natH=0;
        if(!Number.isFinite(Number(plan.w))||Number(plan.w)<=0)plan.w=300;
        if(!Number.isFinite(Number(plan.h))||Number(plan.h)<=0)plan.h=200;
        plan.name=String(plan.name||'Floor Plan');
        plan.dataURL=String(plan.dataURL||'');
        return plan;
      }
      ...
      function currentPlan(){
        ...
        if(!L)return null;
        return normalizePlan(L.plan);
      }

      function planCanvasSize(plan){
        plan=normalizePlan(plan);
        if(!plan)return {w:state.cw,h:state.ch};
        const rad=(Number(plan.rotation)||0)*Math.PI/180;
        const c=Math.abs(Math.cos(rad)),s=Math.abs(Math.sin(rad));
        const margin=Math.max(0,Number(plan.margin)||0);
        return {
          w:Math.max(12,Math.ceil(((plan.w*c+plan.h*s)+margin*2)*1000)/1000),
          h:Math.max(12,Math.ceil(((plan.w*s+plan.h*c)+margin*2)*1000)/1000)
        };
      }

      function syncCanvasToPlan(plan=currentPlan()){
        if(!plan)return false;
        const planSize=planCanvasSize(plan);
        const L=activeLayout?.();
        if(!L)return false;

        const oldW=Math.max(12,Number(L.cw)||Number(state.cw)||planSize.w);
        const oldH=Math.max(12,Number(L.ch)||Number(state.ch)||planSize.h);
        ...
        if(Math.abs(dx)>.0001||Math.abs(dy)>.0001){
          (L.pieces||[]).forEach(piece=>{
            piece.x=round3((Number(piece.x)||0)+dx);
            piece.y=round3((Number(piece.y)||0)+dy);
          });
          (L.dims||[]).forEach(dim=>{
            ['x1','x2'].forEach(k=>{if(Number.isFinite(Number(dim[k])))dim[k]=round3(Number(dim[k])+dx);});
            ['y1','y2'].forEach(k=>{if(Number.isFinite(Number(dim[k])))dim[k]=round3(Number(dim[k])+dy);});
          });
          (L.lines||[]).forEach(line=>{
            ['x1','x2'].forEach(k=>{if(Number.isFinite(Number(line[k])))line[k]=round3(Number(line[k])+dx);});
            ['y1','y2'].forEach(k=>{if(Number.isFinite(Number(line[k])))line[k]=round3(Number(line[k])+dy);});
          });
          (L.notes||[]).forEach(note=>{
            if(Number.isFinite(Number(note.x)))note.x=round3(Number(note.x)+dx);
            if(Number.isFinite(Number(note.y)))note.y=round3(Number(note.y)+dy);
          });
        }
        ...
        if(inCW)inCW.value=String(size.w);
        if(inCH)inCH.value=String(size.h);
        if(typeof syncViewMenuUI==='function')syncViewMenuUI();
        return true;
      }

      function svgForExport(srcSvg){
        const cl=srcSvg.cloneNode(true);
        const plan=currentPlan();
        if(plan&&!plan.includeInExport){
          cl.querySelectorAll('[data-plan-underlay="1"]').forEach(n=>n.remove());
        }
        cl.querySelectorAll('[data-plan-calibration="1"]').forEach(n=>n.remove());

        // Export the drawing, never the editor state.
        cl.querySelectorAll(
          '.lc-selection-outline,[data-piece-resize-layer],[data-piece-rotate-layer],'+
          '.lc-smart-guide,.lc-piece-resize-layer,.lc-piece-rotate-layer'
        ).forEach(n=>n.remove());
        cl.querySelectorAll('.dim-line.selected').forEach(g=>{
          g.classList.remove('selected');
          g.querySelectorAll('line').forEach(line=>line.removeAttribute('stroke-width'));
        });
        cl.querySelectorAll('.lc-free-line-visible[data-base-width]').forEach(line=>{
          line.setAttribute('stroke-width',line.getAttribute('data-base-width')||'1');
        });
        cl.querySelectorAll('.selected').forEach(n=>n.classList.remove('selected'));
        cl.querySelectorAll('.lc-hitbox').forEach(n=>n.remove());

        cl.querySelectorAll('[data-edge-profile-legend="1"]').forEach(n=>n.remove());
        if(state.showEdgeProfiles&&state.edgeLabelMode==='symbol'){
          const legend=makeEdgeProfileLegendSvgNode?.();
          if(legend)cl.appendChild(legend);
        }
        return cl;
      }

      function drawPlanUnderlay(){
        const plan=currentPlan();
        if(!plan||!plan.visible||!plan.dataURL)return;

        const ns='http://www.w3.org/2000/svg';
        const g=document.createElementNS(ns,'g');
        g.setAttribute('data-plan-underlay','1');
        g.setAttribute('pointer-events','none');

        const image=document.createElementNS(ns,'image');
        const x=(state.cw-plan.w)/2+(Number(plan.offsetX)||0);
        const y=(state.ch-plan.h)/2+(Number(plan.offsetY)||0);
        image.setAttribute('x',String(i2p(x)));
        image.setAttribute('y',String(i2p(y)));
        image.setAttribute('width',String(i2p(plan.w)));
        image.setAttribute('height',String(i2p(plan.h)));
        image.setAttribute('preserveAspectRatio','none');
        image.setAttribute('opacity',String(plan.opacity));
        image.setAttribute('href',plan.dataURL);
        try{image.setAttributeNS('http://www.w3.org/1999/xlink','href',plan.dataURL);}catch(_){}
        if(plan.grayscale)image.style.filter='grayscale(1)';

        const cx=i2p(state.cw/2),cy=i2p(state.ch/2);
        const sx=plan.flipX?-1:1,sy=plan.flipY?-1:1;
        const rot=Number(plan.rotation)||0;
        if(rot||sx<0||sy<0){
          g.setAttribute('transform','translate('+cx+' '+cy+') rotate('+rot+') scale('+sx+' '+sy+') translate('+-cx+' '+-cy+')');
        }
        g.appendChild(image);
        svg.appendChild(g);
      }

      function drawPlanCalibrationOverlay(){
        if(!planCalibration||state.workspace==='slab')return;
        const L=activeLayout?.();
        if(!L||L.id!==planCalibration.layoutId)return;
        const ns='http://www.w3.org/2000/svg';
        const g=document.createElementNS(ns,'g');
        g.setAttribute('data-plan-calibration','1');
        g.setAttribute('pointer-events','none');

        const pts=planCalibration.points||[];
        pts.forEach((p,idx)=>{
          const c=document.createElementNS(ns,'circle');
          c.setAttribute('cx',String(i2p(p.x)));
          c.setAttribute('cy',String(i2p(p.y)));
          c.setAttribute('r','6');
          c.setAttribute('fill',idx===0?'#2563eb':'#fff');
        ...
        if(pts.length===1){
          ...
          t.setAttribute('x',String(i2p(pts[0].x)+10));
          t.setAttribute('y',String(i2p(pts[0].y)-10));
          t.setAttribute('fill','#2563eb');
          t.setAttribute('font-size','13');
          t.setAttribute('font-weight','700');
          t.textContent='Point A';
          g.appendChild(t);
        }
        svg.appendChild(g);
      }

      function planDialogShell(titleText){
        if(planModalCleanup)planModalCleanup();
        const overlay=document.createElement('div');
        overlay.className='lc-plan-modal-overlay';
        if(document.querySelector('.lite-cad')?.classList.contains('lc-theme-dark'))overlay.classList.add('lc-theme-dark');

        const dialog=document.createElement('div');
        dialog.className='lc-plan-dialog';
        ...
        const host=(document.fullscreenElement instanceof Element&&document.fullscreenElement.contains(svg))
          ? document.fullscreenElement
          : document.body;
        host.appendChild(overlay);

        let closed=false;
        const blockCanvasKeys=e=>{
        ...
        const cleanup=()=>{
          ...
          window.removeEventListener('keydown',blockCanvasKeys,true);
          overlay.remove();
          if(planModalCleanup===cleanup)planModalCleanup=null;
        };
        planModalCleanup=cleanup;
        close.onclick=cleanup;
        overlay.addEventListener('pointerdown',e=>{if(e.target===overlay)cleanup();});
        document.addEventListener('keydown',onKey,true);
        return {overlay,dialog,body,foot,close:cleanup};
      }

      function planButton(label,handler,cls){
        const b=document.createElement('button');
        b.type='button';
        b.className='lc-btn sm '+(cls||'ghost');
        b.textContent=label;
        b.onclick=handler;
        return b;
      }

      function parsePlanDistance(raw){
        let s=String(raw??'').trim().toLowerCase();
        if(!s)return null;
        s=s.replace(/feet|foot|ft\.?/g,"'").replace(/inches|inch|in\.?/g,'"');
        const parseInchesPart=value=>{
          const v=String(value||'').trim().replace(/"/g,'');
          if(!v)return 0;
          const mixed=v.match(/^(-?\d+)\s+(\d+)\/(\d+)$/);
          if(mixed){
            const den=Number(mixed[3]);if(!den)return null;
            const whole=Number(mixed[1]),frac=Number(mixed[2])/den;
            return whole<0?whole-frac:whole+frac;
          }
          const frac=v.match(/^(-?)(\d+)\/(\d+)$/);
          if(frac){
            const den=Number(frac[3]);if(!den)return null;
        ...
        if(s.includes("'")){
          const parts=s.split("'");
          const feet=Number(parts.shift().trim());
          if(!Number.isFinite(feet))return null;
          const inches=parseInchesPart(parts.join("'"));
      ...
      function openPlanCalibrationDialog(points){
        const plan=currentPlan();
        if(!plan||!Array.isArray(points)||points.length!==2)return;
        const measured=Math.hypot(points[1].x-points[0].x,points[1].y-points[0].y);
        if(!(measured>0.0001))return;

        const modal=planDialogShell('Calibrate Floor Plan');
        const intro=document.createElement('p');
        intro.className='lc-plan-help';
        intro.textContent='Enter the real-world distance between the two points. Scaling is uniform, so the plan aspect ratio will not be stretched.';
        const input=document.createElement('input');
        input.className='lc-input lc-plan-distance-input';
        input.type='text';
        input.placeholder='Example: 8\' 6" or 102.5';
        input.setAttribute('aria-label','Known distance');
        const hint=document.createElement('div');
        hint.className='lc-small lc-plan-muted';
        hint.textContent='Current drawing span: '+measured.toFixed(3)+' in';
        const error=document.createElement('div');
        error.className='lc-plan-error';
        modal.body.append(intro,input,hint,error);

        const cancel=planButton('Cancel',()=>{modal.close();renderPlanNavigator();draw();},'ghost');
        const apply=planButton('Calibrate',()=>{
          const actual=parsePlanDistance(input.value);
          if(!(actual>0)){
            error.textContent='Enter a valid positive distance.';
            input.focus();input.select();return;
          }
          const factor=actual/measured;
          plan.w=Math.max(1,plan.w*factor);
          plan.h=Math.max(1,plan.h*factor);
          plan.calibrated=true;
          plan.locked=true;
          plan.calibration={
            knownDistance:actual,
            measuredDistance:measured,
            factor:factor,
            updatedAt:new Date().toISOString()
          };
          syncCanvasToPlan(plan);
          modal.close();
          renderPlanNavigator();
          draw();
          scheduleSave();
        ...
        modal.foot.append(cancel,apply);
        input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();apply.click();}});
        setTimeout(()=>input.focus(),0);
      }

      function cancelPlanCalibration(){
        if(!planCalibration)return;
        planCalibration=null;
        svg.classList.remove('lc-plan-calibrating');
        renderPlanNavigator();
        draw();
      }

      function startPlanCalibration(){
        const plan=currentPlan(),L=activeLayout?.();
        if(!plan||!L)return;
        if(plan.locked){
          alert('Unlock the floor plan before recalibrating it.');
          return;
        }
        if(state.workspace==='slab')setCanvasWorkspace?.('layout');
        planCalibration={layoutId:L.id,points:[]};
        svg.classList.add('lc-plan-calibrating');
        renderPlanNavigator();
        draw();
      }

      function bindPlanCalibrationPointer(){
        svg.addEventListener('pointerdown',e=>{
          if(!planCalibration||state.workspace==='slab')return;
          const L=activeLayout?.();
          if(!L||L.id!==planCalibration.layoutId){cancelPlanCalibration();return;}
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation?.();
          const pt=svgPoint(e);
          planCalibration.points.push({x:p2i(pt.x),y:p2i(pt.y)});
          if(planCalibration.points.length>=2){
            const pts=planCalibration.points.slice(0,2);
            planCalibration=null;
            svg.classList.remove('lc-plan-calibrating');
            draw();
            renderPlanNavigator();
            openPlanCalibrationDialog(pts);
          }else{
            draw();
            renderPlanNavigator();
          }
      ...
      document.addEventListener('keydown',e=>{
        if(e.key==='Escape'&&planCalibration){
          e.preventDefault();
          e.stopPropagation();
          cancelPlanCalibration();
        }
      },true);
      ...
      function ensurePdfJs(){
        return new Promise((resolve,reject)=>{
          ...
          const existing=document.querySelector('script[data-cadlite-pdfjs="1"]');
          if(existing){
          ...
          const s=document.createElement('script');
          s.dataset.cadlitePdfjs='1';
          s.src='https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/pdf.min.js';
          s.onload=()=>{
            if(!window.pdfjsLib)return reject(new Error('PDF.js unavailable'));
      ...
      function readPlanFileAsDataURL(file){
        return new Promise((resolve,reject)=>{
      ...
      function copyImageToCanvas(img,maxDim){
        const scale=Math.min(1,(maxDim||3200)/Math.max(img.naturalWidth||img.width,img.naturalHeight||img.height));
        const c=document.createElement('canvas');
      ...
      function rotatePlanCanvas(src,degrees){
        const rad=degrees*Math.PI/180;
        const c=Math.abs(Math.cos(rad)),s=Math.abs(Math.sin(rad));
        const out=document.createElement('canvas');
        out.width=Math.max(1,Math.ceil(src.width*c+src.height*s));
        out.height=Math.max(1,Math.ceil(src.width*s+src.height*c));
      ...
      function flipPlanCanvas(src,horizontal,vertical){
        const out=document.createElement('canvas');
      ...
      async function renderPdfPlanPage(pdf,pageNumber,maxDim,rotation=0){
        const page=await pdf.getPage(pageNumber);
        const normalizedRotation=((Number(rotation)||0)%360+360)%360;
        const base=page.getViewport({scale:1,rotation:normalizedRotation});
        const scale=Math.max(.25,Math.min(6,(maxDim||1000)/Math.max(base.width,base.height)));
        const vp=page.getViewport({scale,rotation:normalizedRotation});
        const canvas=document.createElement('canvas');
      ...
      async function openPlanPdfPicker(file){
        let pdfjs,pdf;
        try{
          ...
          console.error(err);
          alert('Could not read this PDF. Try exporting the plan page as PNG or JPG.');
          return;
        }

        const modal=planDialogShell('Choose PDF Page');
        modal.dialog.classList.add('lc-plan-pdf-dialog');

        const top=document.createElement('div');
        top.className='lc-plan-page-row';
        ...
        pageInfo.className='lc-small lc-plan-muted';
        ...
        let previewZoom=1;
        let rotation=0;
        const zoomOut=planButton('Zoom −',()=>{previewZoom=Math.max(.5,previewZoom-.25);previewPage();},'ghost');
        const zoomIn=planButton('Zoom +',()=>{previewZoom=Math.min(4,previewZoom+.25);previewPage();},'ghost');
        const zoomFit=planButton('Fit',()=>{previewZoom=1;previewPage();},'ghost');
        const rotate=planButton('Rotate 90°',()=>{rotation=(rotation+90)%360;previewPage();},'ghost');
        ...
        wrap.className='lc-plan-pdf-preview lc-plan-scroll-preview';
        const preview=document.createElement('canvas');
        preview.className='lc-plan-pdf-canvas';
        wrap.appendChild(preview);
        const busy=document.createElement('div');
        busy.className='lc-plan-preview-busy';
        busy.textContent='Rendering preview…';
        wrap.appendChild(busy);
        modal.body.append(top,controls,wrap);
        ...
        const previewPage=async()=>{
          ...
          try{
            const c=await renderPdfPlanPage(pdf,Number(select.value)||1,Math.round(900*previewZoom),rotation);
        ...
        select.onchange=()=>{previewZoom=1;rotation=0;previewPage();};
        previewPage();

        const cancel=planButton('Cancel',modal.close,'ghost');
        const use=planButton('Use Page',async()=>{
          use.disabled=true;
          use.textContent='Rendering…';
          try{
            const n=Number(select.value)||1;
            const rendered=await renderPdfPlanPage(pdf,n,3000,rotation);
            const dataURL=rendered.toDataURL('image/png');
            modal.close();
            await openPlanImageEditor(dataURL,file.name+' · Page '+n);
          }catch(err){
            console.error(err);
            alert('Could not render that PDF page.');
            use.disabled=false;
            use.textContent='Use Page';
          }
        },'alt');
        modal.foot.append(cancel,use);
      }

      async function openPlanImageEditor(dataURL,name){
        let img;
        try{img=await loadPlanImage(dataURL);}
        catch(err){console.error(err);alert('Could not load that plan image.');return;}

        let work=copyImageToCanvas(img,3200);
        let crop={x:0,y:0,w:work.width,h:work.height};
        let mode='crop';
        let drag=null;
        let levelLine=null;
        ...
        // Local edit history for the Prepare Floor Plan workspace.
        // Snapshots are stored only at discrete edit commits (crop/erase/transform),
        // so pointer movement itself never floods memory.
        const editorHistory=[];
        let editorHistoryIndex=-1;
        let historyBusy=false;
        let undoEditBtn=null;
        let redoEditBtn=null;
        ...
        const syncEditorHistoryButtons=()=>{
          if(undoEditBtn)undoEditBtn.disabled=historyBusy||editorHistoryIndex<=0;
          if(redoEditBtn)redoEditBtn.disabled=historyBusy||editorHistoryIndex>=editorHistory.length-1;
        };
        const pushEditorHistory=()=>{
          if(historyBusy)return;
          const snap=captureEditorState();
          if(editorHistoryIndex<editorHistory.length-1)editorHistory.splice(editorHistoryIndex+1);
          editorHistory.push(snap);
          if(editorHistory.length>10)editorHistory.shift();
          editorHistoryIndex=editorHistory.length-1;
          syncEditorHistoryButtons();
        };
        const restoreEditorHistory=async index=>{
          if(historyBusy||index<0||index>=editorHistory.length)return;
          historyBusy=true;
          syncEditorHistoryButtons();
          try{
            const snap=editorHistory[index];
            const restored=await loadPlanImage(snap.image);
            work=copyImageToCanvas(restored,3200);
            crop={...snap.crop};
            transformed=!!snap.transformed;
            cropFocused=!!snap.cropFocused;
            levelLine=null;
            eraseBase=null;
        ...
        const modal=planDialogShell('Prepare Floor Plan');
        modal.dialog.classList.add('lc-plan-import-dialog');

        const iconify=(btn,label,svg)=>{
          const fullLabel=String(btn.textContent||label||'').trim();
          btn.classList.add('lc-plan-tool-btn');
          btn.title=fullLabel;
        ...
        const group=(label,...children)=>{
          const wrap=document.createElement('div');
          wrap.className='lc-plan-tool-group';
          const title=document.createElement('div');
          title.className='lc-plan-tool-group-title';
        ...
        const resetEraseBaseline=()=>{eraseBase=null;};

        // Crop is normally non-destructive so it can be adjusted freely.
        // Once the user asks for a geometric transform, commit that selected
        // region as the new working image first. This preserves Crop → Erase →
        // Flip/Rotate/Level instead of transforming the hidden full sheet.
        const commitActiveCrop=()=>{
          if(!cropFocused)return false;
          const sx=Math.max(0,Math.floor(Number(crop.x)||0));
          const sy=Math.max(0,Math.floor(Number(crop.y)||0));
          const sw=Math.max(1,Math.min(work.width-sx,Math.round(Number(crop.w)||work.width)));
          const sh=Math.max(1,Math.min(work.height-sy,Math.round(Number(crop.h)||work.height)));

          // No-op if the current crop already represents the full work canvas.
          if(sx===0&&sy===0&&sw===work.width&&sh===work.height){
          ...
          crop={x:0,y:0,w:work.width,h:work.height};
          cropFocused=false;
          preview._eraseHover=null;
          return true;
        };

        const transformWork=transform=>{
          commitActiveCrop();
          work=transform(work);
          crop={x:0,y:0,w:work.width,h:work.height};
          cropFocused=false;
        ...
        const cropBtn=iconify(
          planButton('Crop',()=>{mode='crop';syncMode();},'ghost'),
          'Crop',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M5 2v11a2 2 0 0 0 2 2h11M2 5h11a2 2 0 0 1 2 2v9" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg>'
        );
        const resetCropBtn=iconify(
          planButton('Reset Crop',()=>{crop={x:0,y:0,w:work.width,h:work.height};cropFocused=false;levelLine=null;renderPreview();pushEditorHistory();},'ghost'),
          'Reset',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M4 6a7 7 0 1 1-1 6M4 2v4H0" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );
        const levelBtn=iconify(
          planButton('Level Line',()=>{mode='level';syncMode();},'ghost'),
          'Level',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M2 14 18 4M4 14h12" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg>'
        );

        const rotateLeftBtn=iconify(
          planButton('Rotate Left',()=>transformWork(src=>rotatePlanCanvas(src,-90)),'ghost'),
          '90° L',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M5 5H1V1M2 5a8 8 0 1 1 1 9" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );
        const rotateRightBtn=iconify(
          planButton('Rotate Right',()=>transformWork(src=>rotatePlanCanvas(src,90)),'ghost'),
          '90° R',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M15 5h4V1M18 5a8 8 0 1 0-1 9" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );
        const flipHBtn=iconify(
          planButton('Flip Horizontal',()=>transformWork(src=>flipPlanCanvas(src,true,false)),'ghost'),
          'Flip H',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M10 2v14M8 4 3 9l5 5M12 4l5 5-5 5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );
        const flipVBtn=iconify(
          planButton('Flip Vertical',()=>transformWork(src=>flipPlanCanvas(src,false,true)),'ghost'),
          'Flip V',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M2 9h16M5 7l5-5 5 5M5 11l5 5 5-5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );

        const zoomOut=iconify(
          planButton('Zoom Out',()=>{previewZoom=Math.max(.5,previewZoom-.25);renderPreview();},'ghost'),
          'Zoom −',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><circle cx="8" cy="8" r="5.5" fill="none" stroke="currentColor" stroke-width="1.5"/><path d="m12 12 5 5M5.5 8h5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>'
        );
        const zoomIn=iconify(
          planButton('Zoom In',()=>{previewZoom=Math.min(4,previewZoom+.25);renderPreview();},'ghost'),
          'Zoom +',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><circle cx="8" cy="8" r="5.5" fill="none" stroke="currentColor" stroke-width="1.5"/><path d="m12 12 5 5M5.5 8h5M8 5.5v5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>'
        );
        const eraseBtn=iconify(
          planButton('Eraser',()=>{mode=mode==='erase'?'crop':'erase';syncMode();},'ghost'),
          'Eraser',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="m3 12 8-9 6 5-7 8H5l-2-2 0-2Z" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/><path d="m8 15 6-7" stroke="currentColor" stroke-width="1.5"/></svg>'
        );
        const resetMaskBtn=iconify(
          planButton('Reset Cleanup',()=>{
            if(!eraseBase)return;
            work=copyCanvas(eraseBase);
            eraseBase=null;
            renderPreview();
            pushEditorHistory();
          },'ghost'),
        ...
        undoEditBtn=iconify(
          planButton('Undo Edit',()=>restoreEditorHistory(editorHistoryIndex-1),'ghost'),
          'Undo',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M7 5H2V0M2.5 5A8 8 0 1 1 3 14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );
        redoEditBtn=iconify(
          planButton('Redo Edit',()=>restoreEditorHistory(editorHistoryIndex+1),'ghost'),
          'Redo',
          '<svg viewBox="0 0 20 18" aria-hidden="true"><path d="M13 5h5V0M17.5 5A8 8 0 1 0 17 14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        );

        const brushWrap=document.createElement('label');
        brushWrap.className='lc-plan-brush-control';
        const brushLabel=document.createElement('span');
        brushLabel.textContent='Brush';
        const brushRange=document.createElement('input');
        brushRange.type='range';
        brushRange.min='8';brushRange.max='240';brushRange.step='4';brushRange.value=String(brushSize);
        const brushValue=document.createElement('span');
        brushValue.textContent=brushSize+'px';
        brushRange.oninput=()=>{brushSize=Number(brushRange.value)||48;brushValue.textContent=brushSize+'px';};
        brushWrap.append(brushLabel,brushRange,brushValue);

        const controls=document.createElement('div');
        controls.className='lc-plan-editor-control-panel';
        ...
        actionBar.append(
          group('History',undoEditBtn,redoEditBtn),
          group('Crop',cropBtn,resetCropBtn),
          group('Level',levelBtn),
          group('Rotate',rotateLeftBtn,rotateRightBtn),
          group('Flip',flipHBtn,flipVBtn),
          group('Zoom',zoomOut,zoomIn),
          group('Cleanup',eraseBtn,resetMaskBtn)
        );
        ...
        controls.append(actionBar,utilityBar,help);

        const resultWrap=document.createElement('div');
        resultWrap.className='lc-plan-crop-preview-wrap';
        const resultTitle=document.createElement('div');
        resultTitle.className='lc-plan-section-title';
        resultTitle.textContent='Cropped Preview';
        const resultPreview=document.createElement('canvas');
        resultPreview.className='lc-plan-crop-preview';
        resultPreview.width=480;
        resultPreview.height=300;
        resultWrap.append(resultTitle,resultPreview);

        const topGrid=document.createElement('div');
        topGrid.className='lc-plan-editor-top-grid';
        topGrid.append(controls,resultWrap);

        const preview=document.createElement('canvas');
        preview.className='lc-plan-editor-canvas';
        preview.width=980;
        preview.height=620;
        const previewWrap=document.createElement('div');
        previewWrap.className='lc-plan-editor-preview lc-plan-scroll-preview';
        previewWrap.appendChild(preview);

        const metaRow=document.createElement('div');
        metaRow.className='lc-plan-editor-meta';
        modal.body.append(topGrid,previewWrap,metaRow);

        let map={scale:1,ox:0,oy:0};
        const ctx=preview.getContext('2d');

        const syncMode=()=>{
          cropBtn.classList.toggle('alt',mode==='crop');
          levelBtn.classList.toggle('alt',mode==='level');
          eraseBtn.classList.toggle('alt',mode==='erase');
          preview.style.cursor=mode==='erase'?'none':'crosshair';
          help.textContent=mode==='crop'
            ? 'Drag a box around the portion of the sheet you want to keep.'
            : (mode==='level'
              ? 'Drag along a wall or reference line. CAD Lite rotates it to the nearest horizontal or vertical axis.'
              : 'Paint over unwanted notes or callouts. Reset restores all cleanup strokes since the last rotate/flip/level operation.');
          renderPreview();
        };

        const renderCropPreview=()=>{
          const rctx=resultPreview.getContext('2d');
          rctx.clearRect(0,0,resultPreview.width,resultPreview.height);
          rctx.fillStyle='#f3f4f6';
          rctx.fillRect(0,0,resultPreview.width,resultPreview.height);
          const pad=10;
          const sw=Math.max(1,crop.w),sh=Math.max(1,crop.h);
          const scale=Math.min((resultPreview.width-pad*2)/sw,(resultPreview.height-pad*2)/sh);
          const dw=sw*scale,dh=sh*scale;
          const ox=(resultPreview.width-dw)/2,oy=(resultPreview.height-dh)/2;
          rctx.drawImage(work,crop.x,crop.y,crop.w,crop.h,ox,oy,dw,dh);
        };

        const renderPreview=()=>{
          const baseW=980,baseH=570;
          preview.width=Math.max(490,Math.round(baseW*previewZoom));
          preview.height=Math.max(285,Math.round(baseH*previewZoom));
          ...
          const ox=(preview.width-dw)/2,oy=(preview.height-dh)/2;
          map={scale:sc,ox,oy,srcX:source.x,srcY:source.y,srcW:source.w,srcH:source.h};
          ctx.drawImage(work,source.x,source.y,source.w,source.h,ox,oy,dw,dh);
          ...
          if(levelLine){
            ...
            ctx.lineWidth=Math.max(2,3*previewZoom);
            ctx.beginPath();
            ctx.moveTo(ox+(levelLine.x1-source.x)*sc,oy+(levelLine.y1-source.y)*sc);
            ctx.lineTo(ox+(levelLine.x2-source.x)*sc,oy+(levelLine.y2-source.y)*sc);
            ctx.stroke();
          }
          ...
          if(mode==='erase'){
            ...
            ctx.lineWidth=1.2;
            ctx.setLineDash([4,3]);
            const r=Math.max(3,brushSize*sc/2);
            const hover=preview._eraseHover;
            if(hover){
              ctx.beginPath();ctx.arc(ox+(hover.x-source.x)*sc,oy+(hover.y-source.y)*sc,r,0,Math.PI*2);ctx.stroke();
            }
            ctx.setLineDash([]);
          }
          ctx.restore();
          ...
          metaRow.textContent=Math.round(crop.w)+' × '+Math.round(crop.h)+' px selected'+
            (transformed?' · image adjusted':'')+(eraseBase?' · cleanup applied':'');
          renderCropPreview();
          resetMaskBtn.disabled=!eraseBase;
        };
        ...
        const localPoint=e=>{
          ...
          const srcX=Number(map.srcX)||0,srcY=Number(map.srcY)||0;
          const srcW=Number(map.srcW)||work.width,srcH=Number(map.srcH)||work.height;
          return {
            x:Math.max(srcX,Math.min(srcX+srcW,srcX+(px-map.ox)/map.scale)),
            y:Math.max(srcY,Math.min(srcY+srcH,srcY+(py-map.oy)/map.scale))
          };
        };

        const eraseSegment=(from,to)=>{
          if(!eraseBase)eraseBase=copyCanvas(work);
          ...
          if(from.x===to.x&&from.y===to.y){
            wctx.beginPath();wctx.arc(to.x,to.y,brushSize/2,0,Math.PI*2);wctx.fill();
          }
          wctx.restore();
        };
        ...
        preview.addEventListener('pointermove',e=>{
          const p=localPoint(e);
          preview._eraseHover=p;
          if(!drag){if(mode==='erase')renderPreview();return;}
          drag.current=p;
          if(mode==='crop'){
            const x=Math.min(drag.start.x,p.x),y=Math.min(drag.start.y,p.y);
            crop={x,y,w:Math.max(1,Math.abs(p.x-drag.start.x)),h:Math.max(1,Math.abs(p.y-drag.start.y))};
          }else if(mode==='level'){
            levelLine={x1:drag.start.x,y1:drag.start.y,x2:p.x,y2:p.y};
          }else if(mode==='erase'){
            eraseSegment(drag.last,p);
            drag.last=p;
          }
          renderPreview();
        });
        preview.addEventListener('pointerleave',()=>{preview._eraseHover=null;if(mode==='erase'&&!drag)renderPreview();});
        ...
        const finishDrag=e=>{
          ...
          if(Math.hypot(dx,dy)<12){levelLine=null;renderPreview();return;}
          const angle=Math.atan2(dy,dx)*180/Math.PI;
          const target=Math.round(angle/90)*90;
          const delta=target-angle;
          commitActiveCrop();
          work=rotatePlanCanvas(work,delta);
          crop={x:0,y:0,w:work.width,h:work.height};
          cropFocused=false;
          levelLine=null;
          resetEraseBaseline();
        ...
        const cancel=planButton('Cancel',modal.close,'ghost');
        const use=planButton('Import Plan',()=>{
          const sx=Math.max(0,Math.floor(crop.x)),sy=Math.max(0,Math.floor(crop.y));
          const sw=Math.max(1,Math.min(work.width-sx,Math.round(crop.w)));
          const sh=Math.max(1,Math.min(work.height-sy,Math.round(crop.h)));

          const maxDim=3000;
          const sc=Math.min(1,maxDim/Math.max(sw,sh));
          const out=document.createElement('canvas');
          out.width=Math.max(1,Math.round(sw*sc));
          out.height=Math.max(1,Math.round(sh*sc));
          const octx=out.getContext('2d');
          octx.imageSmoothingEnabled=true;
          octx.imageSmoothingQuality='high';
          octx.fillStyle='#fff';
          ...
          let compressed=out.toDataURL('image/webp',.92);
          if(!compressed.startsWith('data:image/webp'))compressed=out.toDataURL('image/jpeg',.94);

          const aspect=out.width/out.height;
          let w,h;
          if(aspect>=1){w=300;h=300/aspect;}
          else{h=300;w=300*aspect;}

          const L=activeLayout?.();
          if(!L)return;
          L.plan={
            id:'plan_'+uid(),
            name:String(name||'Floor Plan'),
            dataURL:compressed,
            natW:out.width,
            natH:out.height,
            w:Math.max(12,w),
      ...
      async function handlePlanFile(file){
        ...
        if(existing&&!confirm('Replace the floor plan on this layout?'))return;
        try{
          if(file.type==='application/pdf'||/\.pdf$/i.test(file.name)){
            await openPlanPdfPicker(file);
          }else{
            const dataURL=await readPlanFileAsDataURL(file);
            await openPlanImageEditor(dataURL,file.name);
          }
        }catch(err){
          console.error(err);
          alert('Could not import this floor plan.');
        }
      }

      function ensurePlanFileInput(){
        if(planFileInput)return planFileInput;
        const input=document.createElement('input');
        input.type='file';
        input.accept='.pdf,.png,.jpg,.jpeg,image/png,image/jpeg,application/pdf';
        input.hidden=true;
        input.id='lc-plan-file-input';
        input.className='lc-plan-file-input';
        input.style.setProperty('display','none','important');
        input.setAttribute('aria-hidden','true');
        input.onchange=async e=>{
          const file=e.target.files?.[0]||null;
          e.target.value='';
          if(file)await handlePlanFile(file);
        };
        (document.querySelector('.lite-cad')||document.body).appendChild(input);
        planFileInput=input;
        return input;
      }

      function openPlanFilePicker(){
        if(state.workspace==='slab')setCanvasWorkspace?.('layout');
        ensurePlanFileInput().click();
      }

      function cropCurrentPlan(){
        const plan=currentPlan();
        if(!plan)return;
        if(plan.locked){alert('Unlock the floor plan before cropping it.');return;}
        if(plan.calibrated&&!confirm('Cropping this calibrated plan will reset calibration. Continue?'))return;
        openPlanImageEditor(plan.dataURL,plan.name);
      }

      const planHeaderEyeMarkup=visible=>visible
      ...
      function ensurePlanNavigatorCard(){
        if(!layoutsEl)return null;
        let card=document.getElementById('lc-plan-card');
        const layoutCard=layoutsEl.closest('.lc-card');
        ...
        card.id='lc-plan-card';
        card.className='lc-card lc-left-section lc-plan-card lc-design-only';

        const head=document.createElement('div');
        head.className='lc-card-head lc-single-row-head';
        ...
        const planEye=document.createElement('button');
        planEye.type='button';
        planEye.className='lc-btn ghost lc-iconbtn lc-head-eye lc-plan-head-eye';
        planEye.onclick=e=>{
        ...
        inlineActions.append(planEye,arrow);
        titleRow.append(title,inlineActions);
        head.appendChild(titleRow);

        const body=document.createElement('div');
        body.id='lc-plan-body';
        body.className='lc-plan-body';
        card.append(head,body);
        (lineCardNow||layoutCard).insertAdjacentElement('afterend',card);

        let open=true;
        try{open=localStorage.getItem('litecad:planCardOpen')!=='0';}catch(_){}
        const sync=()=>{
          body.hidden=!open;
        ...
        const toggle=()=>{
          open=!open;sync();
          try{localStorage.setItem('litecad:planCardOpen',open?'1':'0');}catch(_){}
        };
        head.onclick=e=>{if(e.target.closest('button,input,label'))return;toggle();};
      ...
      function renderPlanNavigator(){
        const card=ensurePlanNavigatorCard();
        const body=card?.querySelector('#lc-plan-body');
        if(!body)return;
        body.innerHTML='';
        const plan=currentPlan();
        const headEye=card?.querySelector('.lc-plan-head-eye');
        const headTitle=card?.querySelector('.lc-card-head h3');
        if(headEye){
          const visible=!!plan?.visible;
          headEye.disabled=!plan;
          headEye.innerHTML=planHeaderEyeMarkup(visible);
          headEye.title=!plan?'No floor plan on this layout':(visible?'Hide floor plan':'Show floor plan');
          headEye.setAttribute('aria-label',headEye.title);
          headEye.setAttribute('aria-pressed',String(visible));
          headEye.style.opacity=!plan?'.28':(visible?'1':'.55');
          if(headTitle)headTitle.style.opacity=!plan?'1':(visible?'1':'.6');
        }
        ...
        if(!plan){
          ...
          text.className='lc-small lc-plan-muted';
          text.textContent='No floor plan on this layout.';
          const add=planButton('Import Floor Plan',openPlanFilePicker,'ghost');
        ...
        const top=document.createElement('div');
        top.className='lc-plan-summary';
        const info=document.createElement('div');
        info.className='lc-plan-summary-text';
        const name=document.createElement('div');
        name.className='lc-plan-name lc-plan-file-name';
        name.textContent=plan.name;
        name.title=plan.name;
        const status=document.createElement('div');
        status.className='lc-plan-status '+(plan.calibrated?'is-calibrated':'is-uncalibrated');
        status.textContent=plan.calibrated?'Calibrated'+(plan.locked?' · Locked':' · Unlocked'):'Not calibrated'+(plan.locked?' · Locked':'');
        info.append(name,status);

        const topActions=document.createElement('div');
        topActions.className='lc-plan-top-actions';
        const lock=planButton(plan.locked?'Unlock':'Lock',()=>{
          plan.locked=!plan.locked;
          if(plan.locked)cancelPlanCalibration();
          renderPlanNavigator();scheduleSave();pushHistory();
        },plan.locked?'ghost':'alt');
        lock.title=plan.locked?'Unlock plan settings':'Lock plan settings';
        topActions.append(lock);
        top.append(info,topActions);
        body.appendChild(top);

        if(planCalibration&&planCalibration.layoutId===activeLayout()?.id){
          const cal=document.createElement('div');
          cal.className='lc-plan-calibration-banner';
          cal.textContent=(planCalibration.points?.length||0)===0
            ? 'Calibration: click Point A on the plan.'
            : 'Calibration: click Point B.';
          const cancel=planButton('Cancel',cancelPlanCalibration,'ghost');
          cal.appendChild(cancel);
          body.appendChild(cal);
        }

        const appearance=document.createElement('div');
        appearance.className='lc-plan-section';
        const apTitle=document.createElement('div');
        apTitle.className='lc-small lc-plan-section-title';
        apTitle.textContent='Appearance';

        const opacityRow=document.createElement('label');
        opacityRow.className='lc-plan-control-row';
        const opacityLabel=document.createElement('span');
        opacityLabel.textContent='Opacity';
        const opacity=document.createElement('input');
        opacity.type='range';opacity.min='8';opacity.max='100';opacity.step='1';
        opacity.value=String(Math.round(plan.opacity*100));
        const opacityValue=document.createElement('span');
        opacityValue.className='lc-plan-value';
        opacityValue.textContent=opacity.value+'%';
        opacity.oninput=()=>{
          plan.opacity=Math.max(.08,Math.min(1,Number(opacity.value)/100));
          opacityValue.textContent=opacity.value+'%';
          draw();
          syncViewMenuUI?.();
        };
        opacity.onchange=()=>{scheduleSave();pushHistory();syncViewMenuUI?.();};
        opacityRow.append(opacityLabel,opacity,opacityValue);

        const grayRow=document.createElement('label');
        grayRow.className='lc-plan-check-row';
        const gray=document.createElement('input');
        gray.type='checkbox';gray.checked=!!plan.grayscale;
        gray.onchange=()=>{plan.grayscale=gray.checked;draw();scheduleSave();pushHistory();syncViewMenuUI?.();};
        const grayText=document.createElement('span');grayText.textContent='Grayscale plan';
        grayRow.append(gray,grayText);

        const exportRow=document.createElement('label');
        exportRow.className='lc-plan-check-row';
        const include=document.createElement('input');
        include.type='checkbox';include.checked=!!plan.includeInExport;
        include.onchange=()=>{plan.includeInExport=include.checked;scheduleSave();pushHistory();};
        const includeText=document.createElement('span');includeText.textContent='Include plan in export';
        exportRow.append(include,includeText);

        appearance.append(apTitle,opacityRow,grayRow,exportRow);
        body.appendChild(appearance);

        const geometry=document.createElement('div');
        geometry.className='lc-plan-section';
        const geoTitle=document.createElement('div');
        geoTitle.className='lc-small lc-plan-section-title';
        geoTitle.textContent='Plan Setup';

        const size=document.createElement('div');
        size.className='lc-small lc-plan-muted';
        size.textContent='Canvas: '+state.cw.toFixed(2)+'" × '+state.ch.toFixed(2)+'"';

        const buttons=document.createElement('div');
        buttons.className='lc-plan-button-grid';
        const calibrate=planButton(plan.calibrated?'Recalibrate':'Calibrate',startPlanCalibration,'ghost');
        const crop=planButton('Crop / Adjust',cropCurrentPlan,'ghost');
        const flipH=planButton('Flip H',()=>{
          plan.flipX=!plan.flipX;draw();scheduleSave();pushHistory();renderPlanNavigator();
        },'ghost');
        flipH.classList.add('lc-plan-flip-btn');
        flipH.innerHTML='<svg viewBox="0 0 20 16" aria-hidden="true"><path d="M10 2v12M8 3 3 8l5 5M12 3l5 5-5 5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Flip H</span>';
        const flipV=planButton('Flip V',()=>{
          plan.flipY=!plan.flipY;draw();scheduleSave();pushHistory();renderPlanNavigator();
        },'ghost');
        flipV.classList.add('lc-plan-flip-btn');
        flipV.innerHTML='<svg viewBox="0 0 20 16" aria-hidden="true"><path d="M3 8h14M5 6l5-4 5 4M5 10l5 4 5-4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Flip V</span>';

        [calibrate,crop,flipH,flipV].forEach(b=>b.disabled=!!plan.locked);
        buttons.append(calibrate,crop,flipH,flipV);
        ...
        marginInput.min='0';
        marginInput.step='.25';
        marginInput.value=String(Number(plan.margin)||0);
        ...
        marginInput.onchange=()=>{
          plan.margin=Math.max(0,Number(marginInput.value)||0);
          syncCanvasToPlan(plan);draw();renderPlanNavigator();scheduleSave();pushHistory();syncToolbarFromLayout?.();
        };
        ...
        const nudge=(label,dx,dy)=>{
          const b=planButton(label,()=>{
            plan.offsetX=Math.round(((Number(plan.offsetX)||0)+dx)*16)/16;
            plan.offsetY=Math.round(((Number(plan.offsetY)||0)+dy)*16)/16;
            draw();renderPlanNavigator();scheduleSave();pushHistory();
          },'ghost');
          b.disabled=!!plan.locked;
          return b;
        };
        const blank=document.createElement('span');
        blank.className='lc-plan-nudge-blank';
        const up=nudge('↑',0,-1/16);
        const left=nudge('←',-1/16,0);
        const down=nudge('↓',0,1/16);
        const right=nudge('→',1/16,0);
        const reset=planButton('Center',()=>{
          plan.offsetX=0;plan.offsetY=0;draw();renderPlanNavigator();scheduleSave();pushHistory();
        },'ghost');
        reset.disabled=!!plan.locked;
        nudgeGrid.append(blank,up,document.createElement('span'),left,reset,right,document.createElement('span'),down,document.createElement('span'));
        const offsetRead=document.createElement('div');
        offsetRead.className='lc-small lc-plan-muted';
        offsetRead.textContent='Offset X '+(Number(plan.offsetX)||0).toFixed(4)+'" · Y '+(Number(plan.offsetY)||0).toFixed(4)+'"';
        nudgeWrap.append(nudgeLabel,nudgeGrid,offsetRead);

        geometry.append(geoTitle,size,buttons,marginRow,nudgeWrap);
        body.appendChild(geometry);

        const fileSection=document.createElement('div');
        fileSection.className='lc-plan-section lc-plan-file-actions';
        const replace=planButton('Replace Plan',openPlanFilePicker,'ghost');
        const del=planButton('Delete Plan',()=>{
          if(!confirm('Delete the floor plan from this layout?'))return;
          const L=activeLayout?.();
          if(!L)return;
          L.plan=null;
          cancelPlanCalibration();
        ...
        del.disabled=!!plan.locked;
        fileSection.append(replace,del);
        body.appendChild(fileSection);

        if(plan.locked){
          const note=document.createElement('div');
          note.className='lc-small lc-plan-muted lc-plan-lock-note';
          note.textContent='Unlock to crop, recalibrate, mirror, rotate, replace, or delete the plan.';
          body.appendChild(note);
        }
      }

      // ===== End v1.4 Floor Plan Underlay ======================================
      ...
      function ensureOverlaysOnLayout(L){
        ...
        const minSlabH=Math.max(200,Number(state.ch)||0);
      ...
      const slabCanvasH=()=>Number(ensureOverlaysOnLayout(activeLayout())?.slabCH)||Math.max(200,state.ch);
      ...
      let edgePainterViewBefore=null;
      let edgePainterProfile='quarter';
      try{
        const storedProfile=localStorage.getItem('litecad:edgePainterProfile');
        if(storedProfile)edgePainterProfile=storedProfile;
      }catch(_){}

      function setEdgePainterProfile(value,{redraw=true}={}){
        edgePainterProfile=normalizeEdgeProfile(value);
        try{localStorage.setItem('litecad:edgePainterProfile',edgePainterProfile);}catch(_){}
        document.querySelectorAll('.lc-edge-painter-select,.lc-edge-painter-profile-select').forEach(select=>{
          if(select.value!==edgePainterProfile)select.value=edgePainterProfile;
        });
        if(redraw)draw?.();
      }
      ...
      function syncMomentaryToolUI(){
        ...
        root.classList.toggle('lc-edge-painter-active',!slabWorkspace&&tool==='edgePainter');
        root.dataset.momentaryTool=tool||'';

        if(!slabWorkspace&&tool==='edgePainter'){
          if(edgePainterViewBefore===null)edgePainterViewBefore=!!state.showEdgeProfiles;
          state.showEdgeProfiles=true;
        }else if(edgePainterViewBefore!==null){
          state.showEdgeProfiles=!!edgePainterViewBefore;
          edgePainterViewBefore=null;
          saveCurrentWorkspaceViewState?.();
          scheduleSave?.();
        }
        syncEdgePainterHUD?.(tool);
        ...
        document.querySelectorAll('.lc-edge-painter-profile-select').forEach(select=>{
          select.disabled=slabWorkspace;
          select.value=normalizeEdgeProfile(edgePainterProfile);
        });

        document.querySelectorAll('.lc-edge-painter-action').forEach(btn=>{
          const active=!slabWorkspace&&tool==='edgePainter';
          btn.disabled=slabWorkspace;
          btn.classList.toggle('is-active',active);
          btn.setAttribute('aria-pressed',String(active));
          btn.innerHTML=active
            ? '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 16h8l7-7-4-4-7 7v8H4v-4Zm9-9 4 4" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Edge Painter: On · Shift+E</span>'
            : '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 16h8l7-7-4-4-7 7v8H4v-4Zm9-9 4 4" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Paint Edges</span>';
          btn.title=slabWorkspace
            ? 'Edge Painter is available in DESIGN.'
            : 'Paint edge profiles across the whole DESIGN canvas. Shift+E toggles the painter.';
        });
      ...
      function setMomentaryTool(tool,key){
        ...
        // One canvas mode at a time. A held shortcut takes ownership from any
        // locked mode (including the same tool), so releasing the key leaves
        // CAD Lite in neutral mode instead of falling back into the old lock.
        if(momentaryToolState.held){
      ...
      function releaseMomentaryTool(key=null){
        ...
        momentaryToolState.key=null;
        if(previous==='overlay')finishActiveOverlayCanvasDrag?.();
        // No locked fallback: setMomentaryTool deliberately clears locks when a
        // momentary shortcut starts.
        syncMomentaryToolUI();
        draw?.();
      }

      function toggleLockedTool(tool){
        if(state.workspace==='slab'&&(tool==='splash'||tool==='splashAll'||tool==='radius'||tool==='radiusAll'||tool==='edgePainter'))return;
        if(state.workspace!=='slab'&&tool==='overlay')return;

        const turningOff=momentaryToolState.locked===tool&&!momentaryToolState.held;
        if(turningOff){
          momentaryToolState.locked=null;
        }else{
          if(momentaryToolState.held){
      ...
      window.addEventListener('keydown',e=>{
        ...
        if(key==='e'&&e.shiftKey){
          e.preventDefault();
          toggleLockedTool('edgePainter');
          return;
        }
        let tool=momentaryKeyMap.get(key);
        if(key==='s'&&state.workspace==='slab')tool='overlay';
        else if(key==='s'&&e.shiftKey)tool='splashAll';
      ...
      function expandIdsWithLinkedSplashChildren(ids,{snappedOnly=false}={}){
        const out=new Set(Array.isArray(ids)?ids:[]);
        let changed=true;
        while(changed){
          changed=false;
          state.pieces.forEach(piece=>{
            if(!isLinkedSplash(piece)||!piece.attachment?.parentPieceId)return;
            if(snappedOnly&&!isSplashSnappedToParent(piece))return;
            if(out.has(piece.attachment.parentPieceId)&&!out.has(piece.id)){
              out.add(piece.id);
              changed=true;
            }
          });
      ...
      window.addEventListener('keydown', (e) => {
        ...
        let ids=state.selectedIds.length?[...state.selectedIds]:(state.selectedId?[state.selectedId]:[]);
        if(!ids.length)return;
        ids=expandIdsWithLinkedSplashChildren(ids);
        const before=state.pieces.length;
        const idSet=new Set(ids);
        // Only detach children that will survive this deletion.
        ids.forEach(parentId=>{
          state.pieces.forEach(child=>{
            if(!isLinkedSplash(child)||child.attachment?.parentPieceId!==parentId||idSet.has(child.id))return;
            delete child.attachment;
          });
        });
        state.pieces=state.pieces.filter(piece=>!idSet.has(piece.id));
      ...
      window.addEventListener('keydown', (e) => {
        ...
        if(k==='d')activateDimTool();
        else if(k==='l')activateLineTool();
        else activateNoteTool();
        e.preventDefault();
      });
      ...
      window.addEventListener('keydown',(e)=>{
        ...
        // Slab overlay (still respects the two-overlay-per-layout limit)
          ...
          if(overlaySelected){
            ...
            // Countertop piece(s)
            const requestedIds=state.selectedIds.length?[...state.selectedIds]:(state.selectedId?[state.selectedId]:[]);
            const ids=expandIdsWithLinkedSplashChildren(requestedIds);
            const idSet=new Set(ids);
            const originals=state.pieces.filter(p=>idSet.has(p.id));
            if(originals.length){
              const bounds=originals.map(p=>{
                const rs=realSize(p);
                return {p,rs,minX:Number(p.x)||0,minY:Number(p.y)||0,maxX:(Number(p.x)||0)+rs.w,maxY:(Number(p.y)||0)+rs.h};
              });
              ...
              let dx=state.grid,dy=state.grid;
              if(maxX+dx>state.cw)dx=-state.grid;
              if(maxY+dy>state.ch)dy=-state.grid;
              if(minX+dx<0||maxX+dx>state.cw)dx=0;
              if(minY+dy<0||maxY+dy>state.ch)dy=0;

              const idMap=new Map();
              originals.forEach(p=>idMap.set(p.id,uid()));
              let nextLayer=Math.max(0,...state.pieces.map(x=>Number(x.layer)||0))+1;
              const copies=originals.map(p=>{
                const np=JSON.parse(JSON.stringify(p));
                np.id=idMap.get(p.id);
                const sourceWasSplash=isBacksplashPiece(p);
                np.name=sourceWasSplash?'Splash':(p.name||'Piece')+' Copy';
                np.x=round3((Number(p.x)||0)+dx);
                np.y=round3((Number(p.y)||0)+dy);
                ...
                if(sourceWasSplash){
                  ...
                  const oldParent=p.attachment?.parentPieceId;
                  if(oldParent&&idMap.has(oldParent)){
                    np.attachment={...np.attachment,parentPieceId:idMap.get(oldParent)};
                  }else{
              ...
              state.pieces.push(...copies);
              syncLinkedSplashRelationships();
              state.selectedDimId=null;
              state.selectedLineId=null;
              state.selectedNoteId=null;
              clearOverlaySelection();
              setSelection(copies.map(p=>p.id));
              state.lastSelIndex=state.pieces.length-1;
              duplicated=true;
            }
          }
      ...
      window.addEventListener('keydown', (e) => {
        ...
        if(state.workspace==='slab'){
          ...
          const moveIds=expandIdsWithLinkedSplashChildren(requestedIds,{snappedOnly:true});
          const moveSet=new Set(moveIds);
          targets.forEach(p=>{
            if(isLinkedSplash(p)&&isSplashSnappedToParent(p)&&!moveSet.has(p.attachment.parentPieceId)){
              p.attachment.snapped=false;
            }
          });
          const moving=state.pieces.filter(p=>moveSet.has(p.id));
          const bounds=moving.map(p=>{
            const rs=realSize(p);
            return {p,rs,minX:Number(p.x)||0,minY:Number(p.y)||0,maxX:(Number(p.x)||0)+rs.w,maxY:(Number(p.y)||0)+rs.h};
          });
          if(bounds.length){
            const minX=Math.min(...bounds.map(b=>b.minX)),maxX=Math.max(...bounds.map(b=>b.maxX));
            const minY=Math.min(...bounds.map(b=>b.minY)),maxY=Math.max(...bounds.map(b=>b.maxY));
            dx=clamp(dx,-minX,state.cw-maxX);
            dy=clamp(dy,-minY,state.ch-maxY);
            moving.forEach(p=>{p.x=round3((Number(p.x)||0)+dx);p.y=round3((Number(p.y)||0)+dy);});
            syncLinkedSplashRelationships();
          }
        }
        draw();
    ...
    window.addEventListener('keydown', (e) => {
      ...
      function makeLayout(name){
        return { 
          ...
          plan: null,       // v1.4: one floor-plan underlay per layout
          showManualDims: true  
        };
      }
      ...
      const cur = () => {
        ...
        if (!Array.isArray(L.lines)) L.lines = [];
        if(!Number.isFinite(Number(L.pieceFillOpacity)))L.pieceFillOpacity=1;
        L.pieceFillOpacity=Math.max(0,Math.min(1,Number(L.pieceFillOpacity)));
        return L;
      };

      // Map old properties to current layout so the rest of the code keeps working
      ['cw','ch','scale','grid','showGrid','pieceFillOpacity','pieces'].forEach(k=>{
      ...
      let viewPieceOpacityRange=null;
      let viewPieceOpacityValue=null;
      const viewEdgeLabelButtons=new Map();

      const zoomMin=1;
      const zoomMax=Math.max(24,Number(inScale?.max)||24);
      const zoomStep=.5;
      let btnZoomOut=null;
      let btnZoomIn=null;
      ...
      let planVisibilityViewCheck=null;
      let planGrayscaleViewCheck=null;
      let planOpacityViewRange=null;
      let planOpacityViewValue=null;
      ...
      const opacityRow=document.createElement('label');
      opacityRow.className='lc-view-range-row';
      const opacityText=document.createElement('span');
      opacityText.textContent='Fill Opacity';
      viewPieceOpacityRange=document.createElement('input');
      viewPieceOpacityRange.type='range';
      viewPieceOpacityRange.min='0';
      viewPieceOpacityRange.max='100';
      viewPieceOpacityRange.step='5';
      viewPieceOpacityRange.value=String(Math.round((Number(state.pieceFillOpacity ?? 1))*100));
      viewPieceOpacityValue=document.createElement('span');
      viewPieceOpacityValue.className='lc-view-range-value';
      viewPieceOpacityValue.textContent=viewPieceOpacityRange.value+'%';
      viewPieceOpacityRange.oninput=()=>{
        state.pieceFillOpacity=Math.max(0,Math.min(1,Number(viewPieceOpacityRange.value)/100));
        viewPieceOpacityValue.textContent=viewPieceOpacityRange.value+'%';
        draw();
      };
      viewPieceOpacityRange.onchange=()=>{scheduleSave?.();pushHistory?.();};
      opacityRow.append(opacityText,viewPieceOpacityRange,viewPieceOpacityValue);
      viewMenuMount.appendChild(opacityRow);
      ...
      addViewToggle('Seams','showSeams');
      addViewToggle('Radius Labels','showRadiusLabels');

      const edgeModeRow=document.createElement('div');
      edgeModeRow.className='lc-edge-label-mode-row lc-menu-toggle-row';
      const edgeModeLeft=document.createElement('label');
      edgeModeLeft.className='lc-edge-label-mode-left';
      const edgeLabelsCheck=document.createElement('input');
      edgeLabelsCheck.type='checkbox';
      edgeLabelsCheck.checked=!!state.showEdgeProfiles;
      const edgeModeLabel=document.createElement('span');
      edgeModeLabel.textContent='Edge Labels';
      edgeModeLeft.append(edgeLabelsCheck,edgeModeLabel);
      edgeLabelsCheck.onchange=()=>{
        state.showEdgeProfiles=!!edgeLabelsCheck.checked;
        if(state.showEdgeProfiles&&state.edgeLabelMode!=='symbol')state.edgeLabelMode='text';
        saveCurrentWorkspaceViewState?.();
        draw();scheduleSave?.();pushHistory?.();syncTopBar?.();syncViewMenuUI?.();
      };
      viewChecks.set('showEdgeProfiles',edgeLabelsCheck);

      const edgeModeChoices=document.createElement('div');
      edgeModeChoices.className='lc-edge-label-mode-choices';
      [
        ['text','Text labels','<svg viewBox="0 0 24 18" aria-hidden="true"><text x="3" y="14" font-size="15" font-weight="800" fill="currentColor">T</text><text x="14" y="14" font-size="9" font-weight="700" fill="currentColor">T</text></svg>'],
        ['symbol','Symbol labels','<svg viewBox="0 0 24 18" aria-hidden="true"><text x="6" y="15" font-size="17" font-weight="800" fill="currentColor">X</text></svg>']
      ].forEach(([mode,label,icon])=>{
        const btn=document.createElement('button');
        btn.type='button';
        btn.className='lc-edge-label-mode-btn';
        btn.title=label;
        btn.setAttribute('aria-label',label);
        btn.innerHTML=icon;
        btn.onclick=e=>{
          e.preventDefault();e.stopPropagation();
          state.edgeLabelMode=mode;
          state.showEdgeProfiles=true;
          edgeLabelsCheck.checked=true;
          saveCurrentWorkspaceViewState?.();
          draw();scheduleSave?.();pushHistory?.();syncViewMenuUI?.();
        };
        viewEdgeLabelButtons.set(mode,btn);
        edgeModeChoices.appendChild(btn);
      });
      edgeModeRow.append(edgeModeLeft,edgeModeChoices);
      viewMenuMount.appendChild(edgeModeRow);
      addViewToggle('Edge Legend','showEdgeLegend');
      });
      ...
      viewCWInput=addViewNumber('Width',{min:12,step:1,suffix:'in'},value=>{
        if(currentPlan()){syncViewMenuUI();alert('Canvas size follows the floor plan. Crop or recalibrate the plan to change it.');return;}
      ...
      planVisibilityViewCheck=addLayoutViewToggle('Show Plan',()=>!!currentPlan()?.visible,visible=>{
        const plan=currentPlan();
        if(!plan){syncViewMenuUI?.();return;}
      ...
      planGrayscaleViewCheck=addLayoutViewToggle('Grayscale',()=>!!currentPlan()?.grayscale,checked=>{
        const plan=currentPlan();
        if(!plan){syncViewMenuUI?.();return;}
        plan.grayscale=checked;
        renderPlanNavigator?.();
        draw();
        scheduleSave?.();
        pushHistory?.();
      ...
      const planOpacityRow=document.createElement('label');
      planOpacityRow.className='lc-view-range-row lc-view-plan-opacity-row';
      const planOpacityText=document.createElement('span');
      planOpacityText.textContent='Opacity';
      planOpacityViewRange=document.createElement('input');
      planOpacityViewRange.type='range';
      planOpacityViewRange.min='8';
      planOpacityViewRange.max='100';
      planOpacityViewRange.step='1';
      planOpacityViewRange.value=String(Math.round((currentPlan()?.opacity??.4)*100));
      planOpacityViewValue=document.createElement('span');
      planOpacityViewValue.className='lc-view-range-value';
      planOpacityViewValue.textContent=planOpacityViewRange.value+'%';
      planOpacityViewRange.oninput=()=>{
        const plan=currentPlan();
        if(!plan){syncViewMenuUI?.();return;}
        plan.opacity=Math.max(.08,Math.min(1,Number(planOpacityViewRange.value)/100));
        planOpacityViewValue.textContent=planOpacityViewRange.value+'%';
        draw();
      };
      planOpacityViewRange.onchange=()=>{
        const plan=currentPlan();
        if(!plan){syncViewMenuUI?.();return;}
        renderPlanNavigator?.();
        scheduleSave?.();
      ...
      planOpacityRow.append(planOpacityText,planOpacityViewRange,planOpacityViewValue);
      viewMenuMount.appendChild(planOpacityRow);
      });
      ...
      const setCanvasZoom=(value,{save=true,history=true}={})=>{
        const next=clamp(Number(value)||state.scale,zoomMin,zoomMax);
        if(next===state.scale){
          syncViewMenuUI?.();
          return false;
      ...
      let zoomWheelZHeld=false;
      window.addEventListener('keydown',e=>{
        const target=e.target;
        const tag=String(target?.tagName||'').toLowerCase();
        if(tag==='input'||tag==='textarea'||tag==='select'||target?.isContentEditable)return;
        if(!e.ctrlKey&&!e.metaKey&&!e.altKey&&String(e.key||'').toLowerCase()==='z')zoomWheelZHeld=true;
      },true);
      window.addEventListener('keyup',e=>{
        if(String(e.key||'').toLowerCase()==='z')zoomWheelZHeld=false;
      },true);
      window.addEventListener('blur',()=>{zoomWheelZHeld=false;});

      window.addEventListener('keydown',e=>{
        if(planModalCleanup)return;
        const target=e.target;
        const tag=String(target?.tagName||'').toLowerCase();
        if(tag==='input'||tag==='textarea'||tag==='select'||target?.isContentEditable)return;
        ...
        if(k==='+'||k==='='){
          e.preventDefault();setCanvasZoom(state.scale+zoomStep);return;
        }
        if(k==='-'||k==='_'){
          e.preventDefault();setCanvasZoom(state.scale-zoomStep);
        }
      });
      ...
      function syncViewMenuUI(){
        ...
        if(viewCWInput){viewCWInput.value=String(state.cw);viewCWInput.disabled=!!currentPlan();viewCWInput.title=currentPlan()?'Canvas size follows the floor plan.':'';}
        if(viewCHInput){viewCHInput.value=String(state.ch);viewCHInput.disabled=!!currentPlan();viewCHInput.title=currentPlan()?'Canvas size follows the floor plan.':'';}
        if(viewGridInput)viewGridInput.value=String(state.grid);
        if(viewZoomInput)viewZoomInput.value=String(state.scale);
        if(viewPieceOpacityRange){viewPieceOpacityRange.value=String(Math.round((Number(state.pieceFillOpacity ?? 1))*100));}
        if(viewPieceOpacityValue&&viewPieceOpacityRange)viewPieceOpacityValue.textContent=viewPieceOpacityRange.value+'%';
        viewEdgeLabelButtons.forEach((btn,mode)=>{const on=mode===(state.edgeLabelMode==='symbol'?'symbol':'text');btn.classList.toggle('is-selected',on);btn.setAttribute('aria-pressed',String(on));});
        document.querySelectorAll('#lc-inspector [data-edge-label-mode]').forEach(btn=>{
          const on=btn.dataset.edgeLabelMode===(state.edgeLabelMode==='symbol'?'symbol':'text');
        ...
        {
          const plan=currentPlan();
          if(planVisibilityViewCheck){
            planVisibilityViewCheck.checked=!!plan?.visible;
            planVisibilityViewCheck.disabled=!plan;
            planVisibilityViewCheck.title=plan?'Show or hide this layout\'s floor plan':'No floor plan on this layout';
          }
          if(planGrayscaleViewCheck){
            planGrayscaleViewCheck.checked=!!plan?.grayscale;
            planGrayscaleViewCheck.disabled=!plan;
            planGrayscaleViewCheck.title=plan?'Display the floor plan in grayscale':'No floor plan on this layout';
          }
          if(planOpacityViewRange){
            const pct=Math.round((Number(plan?.opacity??.4))*100);
            planOpacityViewRange.value=String(pct);
            planOpacityViewRange.disabled=!plan;
            planOpacityViewRange.title=plan?'Floor plan opacity':'No floor plan on this layout';
            if(planOpacityViewValue)planOpacityViewValue.textContent=pct+'%';
          }
        }
        if(btnZoomOut){
      ...
      const ICON_MENU={
        ...
        keyboard:'M3 6h18v12H3zM6 9h2M10 9h2M14 9h2M18 9h1M6 12h2M10 12h2M14 12h5M6 15h10'
      };
      ...
      const importPlanAction=addToolbarMenuAction(importDropdown.menu,'Floor Plan (PDF / PNG / JPG)',()=>openPlanFilePicker(),{icon:ICON_MENU.image});
      markWorkspaceCommand(importPlanAction,'design','Floor Plan');
      addToolbarMenuSeparator(importDropdown.menu);
      ...
      let shortcutSheetOverlay=null;
      let shortcutSheetKeyHandler=null;
      let shortcutSheetReturnFocus=null;

      const shortcutGroups=[
        {
          title:'Create & Tools',
          rows:[
            {keys:[['P']],action:'Add Piece',context:'DESIGN'},
            {keys:[['D']],action:'Start Dimension Tool',context:'DESIGN'},
            {keys:[['N']],action:'Start Note Tool',context:'DESIGN'},
            {keys:[['L']],action:'Start Line Tool',context:'DESIGN'},
            {keys:[['Hold S']],action:'Splash Mode — selected piece',context:'DESIGN'},
            {keys:[['Hold Shift','S']],action:'Splash Mode — all pieces',context:'DESIGN'},
            {keys:[['Hold R']],action:'Radius Label Mode — selected piece',context:'DESIGN'},
            ...
            {keys:[['Hold S']],action:'Grab / move slab',context:'SLAB'}
          ]
        },
        {
          title:'Selection & Movement',
          rows:[
            {keys:[['Ctrl / ⌘','A']],action:'Select all pieces'},
            {keys:[['Ctrl / ⌘','Click']],action:'Add or remove a piece from selection'},
            {keys:[['Shift','Click']],action:'Select a range of pieces',context:'Pieces list'},
            {keys:[['← ↑ ↓ →']],action:'Nudge selected piece(s) one grid step'},
            {keys:[['Shift','← ↑ ↓ →']],action:'Nudge selected piece(s) four grid steps'},
            {keys:[['Drag rotate handle']],action:'Rotate selected piece/group; soft-snap near 90° increments'},
            {keys:[['Shift','Drag handle']],action:'Rotate in hard 90° increments'},
            {keys:[['Alt','Drag handle']],action:'Rotate with snapping disabled'},
            {keys:[['Double-click handle']],action:'Rotate selected piece +90°'},
            {keys:[['Delete'],['Backspace']],action:'Delete selected piece, dimension, note, or line'},
            {keys:[['Esc']],action:'Cancel active tool; otherwise clear selection'},
            {keys:[['Enter']],action:'Exit the active placement / drawing mode'}
          ]
        },
        {
          title:'Editing & History',
          rows:[
            {keys:[['Ctrl / ⌘','D']],action:'Duplicate the active selection'},
            {keys:[['Ctrl / ⌘','Z']],action:'Undo'},
            {keys:[['Ctrl / ⌘','Y'],['Ctrl / ⌘','Shift','Z']],action:'Redo'},
            {keys:[['Hold Shift']],action:'Force horizontal / vertical',context:'Line or Dimension endpoint'}
          ]
        },
        {
          title:'Inline Editing',
          rows:[
            {keys:[['Enter']],action:'Commit an inline value or text edit'},
            {keys:[['Esc']],action:'Cancel an inline edit'},
            {keys:[['Shift','Enter']],action:'Insert a new line',context:'Note editor'}
          ]
        }
      ];

      function closeKeyboardShortcuts(){
        if(!shortcutSheetOverlay)return;
        shortcutSheetOverlay.remove();
        shortcutSheetOverlay=null;
        if(shortcutSheetKeyHandler){
          document.removeEventListener('keydown',shortcutSheetKeyHandler,true);
          shortcutSheetKeyHandler=null;
        }
        shortcutSheetReturnFocus?.focus?.();
        shortcutSheetReturnFocus=null;
      }

      function openKeyboardShortcuts(){
        closeKeyboardShortcuts();
        shortcutSheetReturnFocus=document.activeElement instanceof HTMLElement?document.activeElement:null;

        const overlay=document.createElement('div');
        overlay.className='lc-shortcuts-overlay';
        if(document.querySelector('.lite-cad')?.classList.contains('lc-theme-dark')){
          overlay.classList.add('lc-theme-dark');
        }

        const dialog=document.createElement('div');
        dialog.className='lc-shortcuts-dialog';
        dialog.setAttribute('role','dialog');
        dialog.setAttribute('aria-modal','true');
        dialog.setAttribute('aria-labelledby','lc-shortcuts-title');

        const head=document.createElement('div');
        head.className='lc-shortcuts-head';
        const titleWrap=document.createElement('div');
        const title=document.createElement('h2');
        title.id='lc-shortcuts-title';
        title.textContent='Keyboard Shortcuts';
        const subtitle=document.createElement('div');
        subtitle.className='lc-shortcuts-subtitle';
        subtitle.textContent='CAD Lite shortcuts are ignored while typing in normal input fields.';
        titleWrap.append(title,subtitle);

        const close=document.createElement('button');
        close.type='button';
        close.className='lc-shortcuts-close';
        close.setAttribute('aria-label','Close keyboard shortcuts');
        close.textContent='×';
        close.onclick=closeKeyboardShortcuts;
        head.append(titleWrap,close);

        const grid=document.createElement('div');
        grid.className='lc-shortcuts-grid';

        shortcutGroups.forEach(group=>{
          const section=document.createElement('section');
          section.className='lc-shortcuts-group';
          ...
          group.rows.forEach(rowData=>{
            const row=document.createElement('div');
            row.className='lc-shortcut-row';

            const info=document.createElement('div');
            info.className='lc-shortcut-info';
            const action=document.createElement('div');
            action.className='lc-shortcut-action';
            action.textContent=rowData.action;
            info.appendChild(action);
            if(rowData.context){
              const context=document.createElement('span');
              context.className='lc-shortcut-context';
              context.textContent=rowData.context;
              info.appendChild(context);
            }

            const keys=document.createElement('div');
            keys.className='lc-shortcut-keys';
            rowData.keys.forEach((combo,comboIndex)=>{
              if(comboIndex){
                const or=document.createElement('span');
                or.className='lc-shortcut-or';
                or.textContent='or';
                keys.appendChild(or);
              }
              const comboEl=document.createElement('span');
              comboEl.className='lc-shortcut-combo';
              combo.forEach((key,keyIndex)=>{
                if(keyIndex){
                  const plus=document.createElement('span');
                  plus.className='lc-shortcut-plus';
                  plus.textContent='+';
        ...
        overlay.addEventListener('pointerdown',e=>{
          if(e.target===overlay)closeKeyboardShortcuts();
        });

        shortcutSheetKeyHandler=e=>{
          if(e.key!=='Escape')return;
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation?.();
        ...
        document.addEventListener('keydown',shortcutSheetKeyHandler,true);

        document.body.appendChild(overlay);
        shortcutSheetOverlay=overlay;
        requestAnimationFrame(()=>close.focus());
      }

      const helpDropdown=makeToolbarDropdown('HELP');
      helpDropdown.button.id='lc-help-menu-btn';
      addToolbarMenuAction(helpDropdown.menu,'Keyboard Shortcuts…',()=>openKeyboardShortcuts(),{icon:ICON_MENU.keyboard});
      addToolbarMenuSeparator(helpDropdown.menu);
      ...
      let canvasHoverPoint=null;
      svg.addEventListener('pointermove',e=>{
        if(state.workspace==='slab'){canvasHoverPoint=null;return;}
        const r=svg.getBoundingClientRect();
        if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom){canvasHoverPoint=null;return;}
        const pt=svgPoint(e);
        canvasHoverPoint={x:p2i(pt.x),y:p2i(pt.y)};
      },{passive:true});
      svg.addEventListener('pointerleave',()=>{canvasHoverPoint=null;},{passive:true});
      ...
      const btnGridSnap=document.createElement('button');
      btnGridSnap.className='lc-btn ghost lc-iconbtn lc-toolbar-icon';
      btnGridSnap.type='button';
      btnGridSnap.id='lc-grid-snap';
      btnGridSnap.innerHTML=toolbarIcon('M5 5h2v2H5zM11 5h2v2h-2zM17 5h2v2h-2zM5 11h2v2H5zM11 11h2v2h-2zM17 11h2v2h-2zM5 17h2v2H5zM11 17h2v2h-2zM17 17h2v2h-2z');
      if(primaryToolHost)primaryToolHost.appendChild(btnGridSnap);
      function syncGridSnapUI(){
        btnGridSnap.title=state.gridSnap!==false?'Grid Snap: On — click to disable':'Grid Snap: Off — click to enable';
        btnGridSnap.setAttribute('aria-label',btnGridSnap.title);
        btnGridSnap.classList.add('ghost');
        btnGridSnap.classList.toggle('alt',state.gridSnap!==false);
        btnGridSnap.setAttribute('aria-pressed',String(state.gridSnap!==false));
      }
      btnGridSnap.onclick=()=>{
        state.gridSnap=state.gridSnap===false;
        syncGridSnapUI();
        scheduleSave?.();
        pushHistory?.();
      };
      ...
      const LZString = (function () {
        ...
        function snapshotState(){
          try{
            ...
            return JSON.stringify({
              ...
              selectedDimId: state.selectedDimId ?? null,
        ...
        function applySnapshot(snap){
          ...
          // Selection is interaction state, not geometry. Preserve the user's
          // current selection through ordinary undo/redo whenever those entities
          // still exist in the restored snapshot. History selection is only a
          // fallback when the current entity disappears (e.g. undo Duplicate).
          const selectionBefore={
            pieceIds:Array.isArray(state.selectedIds)&&state.selectedIds.length
              ? [...state.selectedIds]
              : (state.selectedId?[state.selectedId]:[]),
            dimId:state.selectedDimId??null,
            lineId:state.selectedLineId??null,
          ...
          try{
            ...
            state.gridSnap = 'gridSnap' in data ? data.gridSnap!==false : state.gridSnap;
            ...
            state.showSeams = 'showSeams' in data ? !!data.showSeams : state.showSeams;
            state.showEdgeProfiles = 'showEdgeProfiles' in data ? !!data.showEdgeProfiles : state.showEdgeProfiles;
            state.edgeLabelMode = data.edgeLabelMode==='symbol'?'symbol':'text';
            state.showEdgeLegend = 'showEdgeLegend' in data ? !!data.showEdgeLegend : state.showEdgeLegend;
            state.showPieceFills = 'showPieceFills' in data ? !!data.showPieceFills : state.showPieceFills;
            state.pieceFillOpacity = 'pieceFillOpacity' in data ? Math.max(0,Math.min(1,Number(data.pieceFillOpacity))) : state.pieceFillOpacity;
            ...
            const L=cur();
            const pieceIdSet=new Set((state.pieces||[]).map(piece=>piece.id));
            const currentPieceIds=selectionBefore.pieceIds.filter(id=>pieceIdSet.has(id));
            const historyPieceIds=(Array.isArray(data.selectedIds)&&data.selectedIds.length
              ? data.selectedIds
              : (data.selectedId?[data.selectedId]:[]))
              .filter(id=>pieceIdSet.has(id));
            setSelection(currentPieceIds.length?currentPieceIds:historyPieceIds);

            const dimIds=new Set(Array.isArray(L?.dims)?L.dims.map(item=>item.id):[]);
            const lineIds=new Set(Array.isArray(L?.lines)?L.lines.map(item=>item.id):[]);
            const noteIds=new Set(Array.isArray(L?.notes)?L.notes.map(item=>item.id):[]);

            const restoredDim=dimIds.has(selectionBefore.dimId)
              ? selectionBefore.dimId
              : (dimIds.has(data.selectedDimId)?data.selectedDimId:null);
            const restoredLine=lineIds.has(selectionBefore.lineId)
              ? selectionBefore.lineId
              : (lineIds.has(data.selectedLineId)?data.selectedLineId:null);
            const restoredNote=noteIds.has(selectionBefore.noteId)
              ? selectionBefore.noteId
              : (noteIds.has(data.selectedNoteId)?data.selectedNoteId:null);

            state.selectedDimId=restoredDim;
            state.selectedLineId=restoredDim?null:restoredLine;
            state.selectedNoteId=(restoredDim||restoredLine)?null:restoredNote;
            if(state.selectedDimId||state.selectedLineId||state.selectedNoteId){
              setSelection([]);
            }
     ...
     // ===== General helpers (also used for rounding/inputs) =====
      ...
      let pieceRotateDrag = null;
      let pieceResizeDrag = null;
      ...
      function selectedRotationPieces(){
        const ids=new Set(
          (Array.isArray(state.selectedIds)&&state.selectedIds.length)
            ? state.selectedIds
            : (state.selectedId?[state.selectedId]:[])
        );
        if(!ids.size)return [];

        // In DESIGN, a selected countertop carries every still-snapped linked
        // splash as part of the rigid rotation assembly, even if the child is
        // not explicitly selected.
        if(state.workspace!=='slab'){
          [...ids].forEach(parentId=>{
            const parent=state.pieces.find(piece=>piece.id===parentId);
            if(!parent||isBacksplashPiece(parent))return;
            state.pieces.forEach(child=>{
              if(
                isLinkedSplash(child) &&
                child.attachment?.parentPieceId===parentId &&
                isSplashSnappedToParent(child)
              ) ids.add(child.id);
            });
          });
        }

        const pieces=[...ids].map(id=>state.pieces.find(piece=>piece.id===id)).filter(Boolean);
        if(
          pieces.length===1 &&
          state.workspace!=='slab' &&
          isLinkedSplash(pieces[0]) &&
          isSplashSnappedToParent(pieces[0])
        ) return [];
        return pieces;
      }

      function workspacePiecePose(piece,workspace=state.workspace){
        if(!piece)return null;
        if(workspace==='slab'){
          const sp=ensureSlabPlacement(piece);
          const rs=slabRealSize(piece);
          return {
            x:Number(sp.x)||0,y:Number(sp.y)||0,
            w:rs.w,h:rs.h,
            rotation:Number(sp.rotation)||0,
            cx:(Number(sp.x)||0)+rs.w/2,
            cy:(Number(sp.y)||0)+rs.h/2
          };
        }
        const rs=realSize(piece);
        return {
          x:Number(piece.x)||0,y:Number(piece.y)||0,
          w:rs.w,h:rs.h,
          rotation:Number(piece.rotation)||0,
          cx:(Number(piece.x)||0)+rs.w/2,
          cy:(Number(piece.y)||0)+rs.h/2
        };
      }

      function rotationGroupSnapshot(pieces,workspace=state.workspace){
        const items=pieces.map(piece=>{
          const pose=workspacePiecePose(piece,workspace);
          return pose?{
            id:piece.id,
        ...
        const poses=pieces.map(piece=>workspacePiecePose(piece,workspace)).filter(Boolean);
        const minX=Math.min(...poses.map(p=>p.x));
        const minY=Math.min(...poses.map(p=>p.y));
        const maxX=Math.max(...poses.map(p=>p.x+p.w));
        const maxY=Math.max(...poses.map(p=>p.y+p.h));
        return {
          workspace,
          items,
      ...
      function applyRotationGroup(snapshot,deltaDeg){
        if(!snapshot?.items?.length)return;
        const workspace=snapshot.workspace;
        const canvasW=workspace==='slab'?slabCanvasW():state.cw;
        const canvasH=workspace==='slab'?slabCanvasH():state.ch;
        const center=snapshot.center;
        const proposals=[];

        snapshot.items.forEach(item=>{
          const piece=state.pieces.find(candidate=>candidate.id===item.id);
          if(!piece)return;
          const offset=rotateVec(item.cx-center.x,item.cy-center.y,deltaDeg);
          const nextRotation=round3(((item.rotation+deltaDeg)%360+360)%360);
          const rs=realSize({w:Number(piece.w)||0,h:Number(piece.h)||0,rotation:nextRotation});
          const cx=center.x+offset.x,cy=center.y+offset.y;
          proposals.push({
            piece,
        ...
        let minX=Math.min(...proposals.map(p=>p.x));
        let minY=Math.min(...proposals.map(p=>p.y));
        let maxX=Math.max(...proposals.map(p=>p.x+p.w));
        let maxY=Math.max(...proposals.map(p=>p.y+p.h));

        // One correction vector for the entire assembly. Never clamp members
        // independently; that would shear snapped splashes away from parents.
        let shiftX=0,shiftY=0;
        if(maxX-minX<=canvasW){
          if(minX<0)shiftX=-minX;
          else if(maxX>canvasW)shiftX=canvasW-maxX;
        }else{
          shiftX=(canvasW-(minX+maxX))/2;
        }
        if(maxY-minY<=canvasH){
          if(minY<0)shiftY=-minY;
          else if(maxY>canvasH)shiftY=canvasH-maxY;
        }else{
          shiftY=(canvasH-(minY+maxY))/2;
        }

        proposals.forEach(next=>{
          if(workspace==='slab'){
            const sp=ensureSlabPlacement(next.piece);
            sp.rotation=next.rotation;
            sp.x=round3(next.x+shiftX);
            sp.y=round3(next.y+shiftY);
          }else{
            next.piece.rotation=next.rotation;
            next.piece.x=round3(next.x+shiftX);
            next.piece.y=round3(next.y+shiftY);
          }
        });
      }

      function selectedResizePiece(){
        if(state.workspace==='slab'||activeMomentaryTool()||state.dimTool||state.noteTool||state.lineTool)return null;
        const ids=Array.isArray(state.selectedIds)&&state.selectedIds.length
          ? state.selectedIds
          : (state.selectedId?[state.selectedId]:[]);
        if(ids.length!==1)return null;
        const piece=state.pieces.find(p=>p.id===ids[0])||null;
        if(!piece||isBacksplashPiece(piece))return null;
        return piece;
      }

      function pieceProjectedSpan(piece,axis){
        const rs=realSize(piece);
        const cx=(Number(piece.x)||0)+rs.w/2;
        const cy=(Number(piece.y)||0)+rs.h/2;
        const angle=(Number(piece.rotation)||0)*Math.PI/180;
        const u={x:Math.cos(angle),y:Math.sin(angle)};
        const v={x:-Math.sin(angle),y:Math.cos(angle)};
        const hw=(Number(piece.w)||0)/2;
        const hh=(Number(piece.h)||0)/2;
        const corners=[
          {x:cx+u.x*hw+v.x*hh,y:cy+u.y*hw+v.y*hh},
          {x:cx+u.x*hw-v.x*hh,y:cy+u.y*hw-v.y*hh},
          {x:cx-u.x*hw+v.x*hh,y:cy-u.y*hw+v.y*hh},
          {x:cx-u.x*hw-v.x*hh,y:cy-u.y*hw-v.y*hh}
        ];
        const vals=corners.map(pt=>pt.x*axis.x+pt.y*axis.y);
        return {min:Math.min(...vals),max:Math.max(...vals)};
      }

      function resizePieceSnapDelta(live,drag,newW,newH,center){
        if(!state.pieceSnap||!live||!drag)return null;
        const side=drag.side;
        const alongWidth=side==='left'||side==='right';
        const normal=side==='right'
          ? drag.u
          : side==='left'
            ? {x:-drag.u.x,y:-drag.u.y}
            : side==='bottom'
              ? drag.v
              : {x:-drag.v.x,y:-drag.v.y};
        const tangent=alongWidth?drag.v:drag.u;
        const dimension=alongWidth?newW:newH;
        const tangentHalf=(alongWidth?newH:newW)/2;
        const centerNormal=center.x*normal.x+center.y*normal.y;
        const centerTangent=center.x*tangent.x+center.y*tangent.y;
        ...
        state.pieces.forEach(other=>{
          if(!other||other.id===live.id)return;
          if(isLinkedSplash(other)&&other.attachment?.parentPieceId===live.id)return;

          const targetN=pieceProjectedSpan(other,normal);
          const targetT=pieceProjectedSpan(other,tangent);
          const overlap=Math.min(movingTMax,targetT.max)-Math.max(movingTMin,targetT.min);
          if(overlap<-SNAP_TOL)return;

          [targetN.min,targetN.max].forEach(target=>{
            const delta=target-movingEdge;
            const ad=Math.abs(delta);
            if(ad<=SNAP_TOL&&(!best||ad<best.ad)){
              best={delta,ad,target};
            }
          });
        });
        return best;
      ...
      function syncLivePieceInspectorGeometry(piece){
        if(!piece||piece.id!==state.selectedId)return;
        const w=document.getElementById('insp-w');
        const h=document.getElementById('insp-h');
        const sf=document.getElementById('insp-sf');
        if(w&&document.activeElement!==w)w.value=fmt3(piece.w);
        if(h&&document.activeElement!==h)h.value=fmt3(piece.h);
        if(sf)sf.value=pieceSquareFeet(piece).toFixed(2);
        pieceInspectorPreviewSync?.();
      }

      function drawPieceResizeHandles(){
        const piece=selectedResizePiece();
        if(!piece)return;
        migratePieceGeometry(piece);
        const rs=realSize(piece);
        const cx=(Number(piece.x)||0)+rs.w/2;
        const cy=(Number(piece.y)||0)+rs.h/2;
        const cxPx=i2p(cx),cyPx=i2p(cy);
        const W=i2p(Math.max(.25,Number(piece.w)||.25));
        const H=i2p(Math.max(.25,Number(piece.h)||.25));
        const rot=((Number(piece.rotation)||0)%360+360)%360;

        const layer=document.createElementNS(svgNS,'g');
        layer.setAttribute('class','lc-piece-resize-layer');
        layer.setAttribute('data-piece-resize-layer','1');
        if(rot)layer.setAttribute('transform','rotate('+rot+' '+cxPx+' '+cyPx+')');

        const handles={
          top:{x:cxPx-14,y:cyPx-H/2-4,w:28,h:8,cursor:'ns-resize'},
          right:{x:cxPx+W/2-4,y:cyPx-14,w:8,h:28,cursor:'ew-resize'},
          bottom:{x:cxPx-14,y:cyPx+H/2-4,w:28,h:8,cursor:'ns-resize'},
          left:{x:cxPx-W/2-4,y:cyPx-14,w:8,h:28,cursor:'ew-resize'}
        };

        Object.entries(handles).forEach(([side,h])=>{
          const handle=svgEl('rect',{
            x:h.x,y:h.y,width:h.w,height:h.h,rx:3,
            class:'lc-piece-resize-handle',
          ...
          handle.addEventListener('pointerdown',e=>{
            ...
            const live=state.pieces.find(p=>p.id===piece.id);
            if(!live)return;
            const startRs=realSize(live);
            const startCenter={
              x:(Number(live.x)||0)+startRs.w/2,
              y:(Number(live.y)||0)+startRs.h/2
            };
            const pt=svgPoint(e);
            const startPointer={x:p2i(pt.x),y:p2i(pt.y)};
            const angle=(Number(live.rotation)||0)*Math.PI/180;
            const u={x:Math.cos(angle),y:Math.sin(angle)};
            const v={x:-Math.sin(angle),y:Math.cos(angle)};
            pieceResizeDrag={
              pieceId:live.id,side,startPointer,startCenter,u,v,
              startW:Number(live.w)||.25,startH:Number(live.h)||.25,
              moved:false,
              lastValid:{w:Number(live.w)||.25,h:Number(live.h)||.25,x:Number(live.x)||0,y:Number(live.y)||0}
            };

            const move=ev=>{
              const drag=pieceResizeDrag;
              if(!drag||drag.pieceId!==live.id)return;
              const q=svgPoint(ev);
              const pointer={x:p2i(q.x),y:p2i(q.y)};
              const dx=pointer.x-drag.startPointer.x;
              const dy=pointer.y-drag.startPointer.y;
              const du=dx*drag.u.x+dy*drag.u.y;
              const dv=dx*drag.v.x+dy*drag.v.y;
              let newW=drag.startW,newH=drag.startH;
              let shiftU=0,shiftV=0;

              if(drag.side==='right'){newW=drag.startW+du;shiftU=(newW-drag.startW)/2;}
              if(drag.side==='left'){newW=drag.startW-du;shiftU=-(newW-drag.startW)/2;}
              if(drag.side==='bottom'){newH=drag.startH+dv;shiftV=(newH-drag.startH)/2;}
              if(drag.side==='top'){newH=drag.startH-dv;shiftV=-(newH-drag.startH)/2;}

              newW=Math.max(.25,newW);
              newH=Math.max(.25,newH);
              if(!ev.shiftKey){
                newW=Math.max(.25,Math.round(newW*8)/8);
                newH=Math.max(.25,Math.round(newH*8)/8);
              }
              // Default drag is friendly 1/8" sizing; Shift bypasses that
              // quantization for power-user micro adjustments.
              // Piece Snap then gets the final word when the resized edge is
              // within 1" of another piece and the two spans actually overlap.
              shiftU=0;shiftV=0;
              if(drag.side==='right')shiftU=(newW-drag.startW)/2;
              if(drag.side==='left')shiftU=-(newW-drag.startW)/2;
              if(drag.side==='bottom')shiftV=(newH-drag.startH)/2;
              if(drag.side==='top')shiftV=-(newH-drag.startH)/2;

              let center={
                x:drag.startCenter.x+drag.u.x*shiftU+drag.v.x*shiftV,
                y:drag.startCenter.y+drag.u.y*shiftU+drag.v.y*shiftV
              };
              const snapHit=resizePieceSnapDelta(live,drag,newW,newH,center);
              if(snapHit){
                if(drag.side==='left'||drag.side==='right')newW=Math.max(.25,newW+snapHit.delta);
                else newH=Math.max(.25,newH+snapHit.delta);
                shiftU=0;shiftV=0;
                if(drag.side==='right')shiftU=(newW-drag.startW)/2;
                if(drag.side==='left')shiftU=-(newW-drag.startW)/2;
                if(drag.side==='bottom')shiftV=(newH-drag.startH)/2;
              ...
              live.w=round3(newW);
              live.h=round3(newH);
              migratePieceGeometry(live);
              const nextRs=realSize(live);
              live.x=round3(center.x-nextRs.w/2);
              live.y=round3(center.y-nextRs.h/2);
              clampToCanvas(live);

              // The parent's own canvas clamp is not enough when a snapped splash
              // sits outside that edge. Treat the parent + snapped splashes as a
              // single resize envelope and stop at the last valid geometry.
              if(!snappedSplashChildrenFitCanvas(live)){
                const last=drag.lastValid;
                live.w=last.w;live.h=last.h;live.x=last.x;live.y=last.y;
                migratePieceGeometry(live);
                syncLinkedSplashRelationships();
                draw();
                return;
              ...
              drag.lastValid={w:Number(live.w)||.25,h:Number(live.h)||.25,x:Number(live.x)||0,y:Number(live.y)||0};
              syncLinkedSplashRelationships();
              drag.moved=drag.moved||Math.abs(newW-drag.startW)>.001||Math.abs(newH-drag.startH)>.001;
              draw();
              syncLivePieceInspectorGeometry(live);
            };
            ...
            const up=()=>{
              ...
              const moved=!!pieceResizeDrag?.moved;
              pieceResizeDrag=null;
              migratePieceSeams(live);
              syncLinkedSplashRelationships();
              draw();
              renderList();
              updateInspector();
        ...
        if(pieceResizeDrag&&pieceResizeDrag.pieceId===piece.id){
          const label=svgEl('text',{
            x:cxPx,y:cyPx-H/2-12,
            class:'lc-piece-resize-label',
            'text-anchor':'middle',
            'pointer-events':'none'
          });
          label.textContent=fmtCanvasInches(piece.w)+' × '+fmtCanvasInches(piece.h);
      ...
      function edgeLegendProfiles(){
        const used=new Set();
        state.pieces.forEach(piece=>{
          Object.values(piece.edgeProfiles||{}).forEach(value=>{
            const profile=normalizeEdgeProfile(value);
            if(profile!=='none')used.add(profile);
          });
        });
        return EDGE_PROFILE_VALUES.filter(profile=>used.has(profile));
      }

      function makeEdgeProfileLegendSvgNode(){
        if(!state.showEdgeProfiles||state.edgeLabelMode!=='symbol')return null;
        const rows=edgeLegendProfiles();
        if(!rows.length)return null;
        const rowH=16;
        const boxW=142;
        const boxH=10+rows.length*rowH+18;
        const canvasW=i2p(state.workspace==='slab'?slabCanvasW():state.cw);
        const x=Math.max(6,canvasW-boxW-8);
        const y=8;
        const g=document.createElementNS(svgNS,'g');
        g.setAttribute('class','lc-edge-profile-legend');
        g.setAttribute('data-edge-profile-legend','1');
        g.setAttribute('pointer-events','none');
        g.appendChild(svgEl('rect',{
          x,y,width:boxW,height:boxH,rx:5,
          fill:'#fff','fill-opacity':'.92',stroke:'#9ca3af','stroke-width':'1',
          'vector-effect':'non-scaling-stroke'
        }));
        const title=svgEl('text',{x:x+8,y:y+14,'font-size':'10','font-weight':'700',fill:'#111'});
        title.textContent='EDGE LEGEND';
        g.appendChild(title);
        rows.forEach((profile,index)=>{
          const yy=y+30+index*rowH;
          const sym=svgEl('text',{x:x+10,y:yy,'font-size':'11','font-weight':'800',fill:'#111'});
          sym.textContent=EDGE_PROFILE_SYMBOLS[profile]||'?';
          const desc=svgEl('text',{x:x+30,y:yy,'font-size':'10',fill:'#111'});
          desc.textContent=EDGE_PROFILE_LABELS[profile]||profile;
          g.append(sym,desc);
        });
        return g;
      }

      function syncEdgeLegendHUD(){
        if(typeof canvasCard==='undefined'||!canvasCard)return;
        let hud=canvasCard.querySelector(':scope > .lc-edge-legend-hud');
        const rows=edgeLegendProfiles();
        const visible=!!(state.showEdgeProfiles&&state.edgeLabelMode==='symbol'&&state.showEdgeLegend&&rows.length);
        if(!visible){
        ...
        if(!hud){
          ...
          hud.className='lc-edge-legend-hud';
          canvasCard.appendChild(hud);
        }
        hud.hidden=false;
        hud.innerHTML='<div class="lc-edge-legend-hud-title">EDGE LEGEND</div>'+
          rows.map(profile=>'<div class="lc-edge-legend-hud-row"><strong>'+
            (EDGE_PROFILE_SYMBOLS[profile]||'?')+'</strong><span>'+
            (EDGE_PROFILE_LABELS[profile]||profile)+'</span></div>').join('');

        // Anchor the legend inside the visible canvas area. The previous
        // fixed 76px offset could place it above the DESIGN/SLAB tabs.
        const wrap=canvasCard.querySelector('.lc-canvas-wrap');
        if(wrap){
          if(getComputedStyle(canvasCard).position==='static')canvasCard.style.position='relative';
          hud.style.top=Math.round(wrap.offsetTop+8)+'px';
          hud.style.right='12px';
        }
      }

      function drawEdgeProfileLegend(){
        syncEdgeLegendHUD();
      }

      function raiseEdgeProfileLabels(){
        const groups=Array.from(svg.querySelectorAll('.edge-profiles'));
        groups.forEach(labels=>{
          const parent=labels.parentElement;
          if(!parent||parent===svg)return;
          const parentTransform=parent.getAttribute?.('transform');
          labels.remove();
          if(parentTransform)labels.setAttribute('transform',parentTransform);
          svg.appendChild(labels);
        });
      }

      function drawPieceRotateHandle(){
        if(activeMomentaryTool()||state.dimTool||state.noteTool||state.lineTool)return;
        const pieces=selectedRotationPieces();
        if(!pieces.length)return;

        const snapshot=rotationGroupSnapshot(pieces,state.workspace);
        if(!snapshot)return;
        const poses=pieces.map(piece=>workspacePiecePose(piece,state.workspace)).filter(Boolean);
        if(!poses.length)return;
        const minX=Math.min(...poses.map(p=>p.x));
        const minY=Math.min(...poses.map(p=>p.y));
        const maxX=Math.max(...poses.map(p=>p.x+p.w));
        const groupCenter={x:(minX+maxX)/2,y:(minY+Math.max(...poses.map(p=>p.y+p.h)))/2};

        const anchor={x:i2p(maxX),y:i2p(minY)};
        const handle={x:anchor.x+13,y:anchor.y-13};

        const layer=document.createElementNS(svgNS,'g');
        layer.setAttribute('class','lc-piece-rotate-layer');
        layer.setAttribute('data-piece-rotate-layer','1');

        const stem=svgEl('line',{
          x1:anchor.x,y1:anchor.y,x2:handle.x,y2:handle.y,
          class:'lc-piece-rotate-stem',
          'vector-effect':'non-scaling-stroke',
          'pointer-events':'none'
        });
        const circle=svgEl('circle',{
          cx:handle.x,cy:handle.y,r:7,
          class:'lc-piece-rotate-handle',
          'vector-effect':'non-scaling-stroke'
        });
        circle.setAttribute('data-piece-rotate-handle','1');
        circle.setAttribute('aria-label',pieces.length>1?'Rotate selected pieces':'Rotate piece');

        const glyph=svgEl('text',{
          x:handle.x,y:handle.y+.25,
          class:'lc-piece-rotate-glyph',
          'text-anchor':'middle',
        ...
        circle.addEventListener('pointerdown',e=>{
          ...
          const livePieces=selectedRotationPieces();
          const liveSnapshot=rotationGroupSnapshot(livePieces,state.workspace);
          if(!liveSnapshot)return;
          const center=liveSnapshot.center;
          const centerPx={x:i2p(center.x),y:i2p(center.y)};
          const pt=svgPoint(e);
          const startPointer=Math.atan2(pt.y-centerPx.y,pt.x-centerPx.x)*180/Math.PI;
          const primary=liveSnapshot.items[0];

          pieceRotateDrag={
            workspace:state.workspace,
            snapshot:liveSnapshot,
            startPointer,
            startPrimaryRotation:Number(primary?.rotation)||0,
            delta:0,
            angle:Number(primary?.rotation)||0,
            moved:false
          };

          const move=ev=>{
            if(!pieceRotateDrag||pieceRotateDrag.workspace!==state.workspace)return;
            const q=svgPoint(ev);
            const current=Math.atan2(q.y-centerPx.y,q.x-centerPx.x)*180/Math.PI;
            let delta=((current-startPointer+540)%360)-180;
            let angle=pieceRotateDrag.startPrimaryRotation+delta;

            if(ev.shiftKey){
              angle=Math.round(angle/90)*90;
              delta=angle-pieceRotateDrag.startPrimaryRotation;
            }else if(!ev.altKey){
            ...
            if(Math.abs(delta-pieceRotateDrag.delta)>.001)pieceRotateDrag.moved=true;
            pieceRotateDrag.delta=delta;
            pieceRotateDrag.angle=((angle%360)+360)%360;
            applyRotationGroup(pieceRotateDrag.snapshot,delta);
            draw();
          };
          ...
          const up=()=>{
            ...
            window.removeEventListener('pointerup',up,true);
            window.removeEventListener('pointercancel',up,true);
            const moved=!!pieceRotateDrag?.moved;
            pieceRotateDrag=null;
            draw();
            updateInspector();
            if(moved){
              scheduleSave();
        ...
        circle.addEventListener('dblclick',e=>{
          ...
          const liveSnapshot=rotationGroupSnapshot(livePieces,state.workspace);
          if(!liveSnapshot)return;
          applyRotationGroup(liveSnapshot,90);
          draw();
          updateInspector();
          scheduleSave();
          pushHistory();
        ...
        if(pieceRotateDrag&&pieceRotateDrag.workspace===state.workspace){
          const label=svgEl('text',{
            x:handle.x+11,y:handle.y-9,
            class:'lc-piece-rotate-angle',
            'text-anchor':'start',
            'pointer-events':'none'
          });
          label.textContent=`${Math.round(pieceRotateDrag.angle*10)/10}°`;
          layer.appendChild(label);
        }
      ...
      let seamVisibilityInspectorEye=null;
      ...
      function syncSeamVisibilityUI(){
        if(!seamVisibilityInspectorEye)return;
        const visible=!!state.showSeams;
        seamVisibilityInspectorEye.innerHTML=eyeIcon(visible);
        seamVisibilityInspectorEye.title=visible?'Hide Seams':'Show Seams';
        seamVisibilityInspectorEye.setAttribute('aria-label',seamVisibilityInspectorEye.title);
        seamVisibilityInspectorEye.setAttribute('aria-pressed',String(visible));
        seamVisibilityInspectorEye.style.opacity=visible?'1':'.55';
      }
      ...
      const canvasScrollWrap=canvasCard?.querySelector('.lc-canvas-wrap')||null;
      let zoomWheelCommitTimer=null;
      canvasScrollWrap?.addEventListener('wheel',e=>{
        if(!e.shiftKey||!zoomWheelZHeld||planModalCleanup)return;
        e.preventDefault();
        const rect=canvasScrollWrap.getBoundingClientRect();
        const localX=e.clientX-rect.left;
        const localY=e.clientY-rect.top;
        const oldScale=state.scale;
        const contentX=canvasScrollWrap.scrollLeft+localX;
        const contentY=canvasScrollWrap.scrollTop+localY;
        const worldX=contentX/oldScale;
        const worldY=contentY/oldScale;
        const direction=e.deltaY<0?1:-1;
        const next=clamp(oldScale+direction*.5,zoomMin,zoomMax);
        if(next===oldScale)return;
        setCanvasZoom(next,{save:false,history:false});
        canvasScrollWrap.scrollLeft=Math.max(0,worldX*next-localX);
        canvasScrollWrap.scrollTop=Math.max(0,worldY*next-localY);
        clearTimeout(zoomWheelCommitTimer);
        zoomWheelCommitTimer=setTimeout(()=>{scheduleSave?.();pushHistory?.();},180);
      },{passive:false});
...
if (window.svg2pdf) {
  ...
  async function addRasterPNG() {
    ...
    const serializer = new XMLSerializer();
    const src = serializer.serializeToString(svgForExport(svgEl));
    ...
    sinksUI = initSinksCard({
      ...
      // ------- Utils -------
      const snap  = (n,step) => state.gridSnap===false ? n : Math.round(n/step)*step;
      ...
      function syncToolbarFromLayout(){
        ...
        const planOwnsCanvas=!!currentPlan();
        inCW.disabled=planOwnsCanvas;
        inCH.disabled=planOwnsCanvas;
        inCW.title=planOwnsCanvas?'Canvas size follows the floor plan.':'';
        inCH.title=planOwnsCanvas?'Canvas size follows the floor plan.':'';
        renderPlanNavigator?.();
        syncViewMenuUI?.();
      ...
      function applyPieceFill(el, p){
        ...
        if (noFill){
          ...
          el.setAttribute('fill-opacity', String(getFillOpacity(p)*Math.max(0,Math.min(1,Number(state.pieceFillOpacity ?? 1)))));
        }
      }
      ...
      function drawSlabWorkspace(){
        ...
        [...state.pieces].sort((a,b)=>(a.layer||0)-(b.layer||0)).forEach((p,index)=>{
          ...
          if(showThisPieceDims){
            ...
            const wt=svgEl('text',{x:cx,y:yTop-4,'text-anchor':'middle','font-size':'12',fill:'#111'});
            wt.textContent=fmtCanvasInches(p.w);
            const ht=svgEl('text',{x:xLeft-4,y:cy,'text-anchor':'end','dominant-baseline':'middle','font-size':'12',fill:'#111'});
          ...
          if(state.showEdgeProfiles&&p.edgeProfiles){
            ...
            const addEdgeLabel=(value,xPos,yPos,anchor='middle',rotation=0)=>{
              ...
              const t=svgEl('text',{x:xPos,y:yPos,'text-anchor':anchor,'dominant-baseline':'middle','font-size':symbol?'12':'11','font-weight':symbol?'800':'400',fill:'#111'});
              if(rotation)t.setAttribute('transform',`rotate(${rotation} ${xPos} ${yPos})`);
              t.textContent=edgeProfileDisplayText(profile);
              labelsG.appendChild(t);
            };
            const MARGIN=state.edgeLabelMode==='symbol'?0:8;
          ...
          if(showThisPieceLabel){
            ...
            t.setAttribute('fill',pieceLabelTextColor(p,pickTextColor(p.color||'#ffffff')));
        ...
        raiseEdgeProfileLabels();
        drawPieceRotateHandle();
        drawPieceResizeHandles();
        drawEdgeProfileLegend();
        pieceInspectorPreviewSync?.();
      }
      ...
      function exportJSON(){
        return {
          ...
          pieceFillOpacity: Number(state.pieceFillOpacity ?? 1),
          edgeLabelMode: state.edgeLabelMode==='symbol'?'symbol':'text',
          showEdgeLegend: !!state.showEdgeLegend,
...
function scheduleSave(){
  ...
  saveTimer = setTimeout(()=>{
    try{
      ...
      alert('Autosave failed — project data is too large.\n\nTip: remove unused slab/plan images or crop plans more tightly. Imported plans are compressed automatically.');
    }
  }, 400);
}
...
function restore(){
  try{
    ...
    if (data.layouts && Array.isArray(data.layouts)){
      ...
      if (data.ui){
        if ('gridSnap' in data.ui) state.gridSnap=data.ui.gridSnap!==false;
        ...
        if ('showSeams' in data.ui) state.showSeams = !!data.ui.showSeams;
        if ('showEdgeProfiles' in data.ui) state.showEdgeProfiles = !!data.ui.showEdgeProfiles;
        if ('edgeLabelMode' in data.ui) state.edgeLabelMode = data.ui.edgeLabelMode==='symbol'?'symbol':'text';
        if ('showEdgeLegend' in data.ui) state.showEdgeLegend = !!data.ui.showEdgeLegend;
        if ('showPieceFills' in data.ui) state.showPieceFills = !!data.ui.showPieceFills;
        if ('pieceFillOpacity' in data.ui) state.pieceFillOpacity = Math.max(0,Math.min(1,Number(data.ui.pieceFillOpacity)));
...
(function migrateOverlayToLayouts(){
  ...
  if (state.overlay && (state.overlay.dataURL || state.overlay.visible)){
    ...
    delete state.overlay;
      ...
      function pieceLabelTextColor(piece,fallback){
        return fallback||pickTextColor(piece?.color||'#ffffff');
      }
      ...
      function snappedSplashChildrenFitCanvas(parent){
        if(!parent)return true;
        return state.pieces.every(child=>{
          if(!isLinkedSplash(child)||child.attachment.parentPieceId!==parent.id||!isSplashSnappedToParent(child))return true;
          const place=splashPlacement(parent,child.attachment.sourceEdge,Math.max(.25,Number(child.h)||DEFAULT_SPLASH_HEIGHT));
          return !!(place&&splashPlacementFitsCanvas(child,place));
        });
      }
      ...
      function splashContactEdge(piece){
        const source=piece?.attachment?.sourceEdge;
        if(source==='top'||source==='right')return 'bottom';
        if(source==='bottom'||source==='left')return 'top';
        return null;
      }

      function syncLinkedMiterProfile(piece,side,nextProfile,previousProfile){
        if(!piece||!side)return;
        const next=normalizeEdgeProfile(nextProfile);
        const prev=normalizeEdgeProfile(previousProfile);

        if(isLinkedSplash(piece)){
          const contact=splashContactEdge(piece);
          if(side!==contact)return;
          const parent=state.pieces.find(candidate=>candidate.id===piece.attachment.parentPieceId);
          const parentSide=piece.attachment.sourceEdge;
          if(!parent||!parentSide)return;
          migratePieceGeometry(parent);
          if(next==='miter')parent.edgeProfiles[parentSide]='miter';
          else if(prev==='miter'&&parent.edgeProfiles[parentSide]==='miter')parent.edgeProfiles[parentSide]='none';
          return;
        }

        const child=splashChildForEdge(piece.id,side);
        if(!child)return;
        migratePieceGeometry(child);
        const contact=splashContactEdge(child);
        if(!contact)return;
        if(next==='miter')child.edgeProfiles[contact]='miter';
        else if(prev==='miter'&&child.edgeProfiles[contact]==='miter')child.edgeProfiles[contact]='none';
      }
      ...
      function swapTopBottom(value){
        return value==='top'?'bottom':(value==='bottom'?'top':value);
      }
      function swapLeftRight(value){
        return value==='left'?'right':(value==='right'?'left':value);
      }
      function swapFrontBack(value){
      ...
      // Faucet selections are nine indexed positions (0..8) across the sink's
      // local back edge. Mirroring either piece axis reverses that local
      // left/right order once the sink's reference side/rotation is updated.
      function mirrorSinkFaucetPattern(sink){
        if(!Array.isArray(sink?.faucets))return;
        sink.faucets=sink.faucets
          .map(value=>{
            const idx=Math.max(0,Math.min(8,Math.round(Number(value)||0)));
            return 8-idx;
          })
          .sort((a,b)=>a-b);
      }

      function mirrorPieceLocal(piece,axis){
        if(!piece)return;
        migratePieceGeometry(piece);
        ...
        const e={...piece.edgeProfiles};

        if(axis==='h'){
          // Left ↔ right inside the piece.
          piece.cornerRadii={tl:r.tr,tr:r.tl,br:r.bl,bl:r.br};
          piece.edgeProfiles={top:e.top,right:e.left,bottom:e.bottom,left:e.right};
          piece.pieceSeams.forEach(seam=>{
            if(seam.orientation==='vertical')seam.reference=swapLeftRight(seam.reference);
          });
          piece.sinks.forEach(sink=>{
            const side=['front','back','left','right'].includes(sink.side)?sink.side:'front';
            if(side==='front'||side==='back'){
              sink.centerline=round3(Math.max(0,(Number(piece.w)||0)-(Number(sink.centerline)||0)));
            }else{
              sink.side=swapLeftRight(side);
            }
            sink.rotation=normDeg(-(Number(sink.rotation)||0));
            mirrorSinkFaucetPattern(sink);
          });
          if(isBacksplashPiece(piece)&&piece.attachment?.sourceEdge){
            piece.attachment.sourceEdge=swapLeftRight(piece.attachment.sourceEdge);
          }
        }else{
          // Top ↔ bottom inside the piece.
          piece.cornerRadii={tl:r.bl,tr:r.br,br:r.tr,bl:r.tl};
          piece.edgeProfiles={top:e.bottom,right:e.right,bottom:e.top,left:e.left};
          piece.pieceSeams.forEach(seam=>{
            if(seam.orientation==='horizontal')seam.reference=swapTopBottom(seam.reference);
          });
          piece.sinks.forEach(sink=>{
            const side=['front','back','left','right'].includes(sink.side)?sink.side:'front';
            if(side==='left'||side==='right'){
              sink.centerline=round3(Math.max(0,(Number(piece.h)||0)-(Number(sink.centerline)||0)));
            }else{
              sink.side=swapFrontBack(side);
            }
            sink.rotation=normDeg(-(Number(sink.rotation)||0));
            mirrorSinkFaucetPattern(sink);
          });
          if(isBacksplashPiece(piece)&&piece.attachment?.sourceEdge){
            piece.attachment.sourceEdge=swapTopBottom(piece.attachment.sourceEdge);
          }
        }
        migratePieceGeometry(piece);
      }

      function mirrorSelectedPieces(axis){
        if(state.workspace==='slab')return;
        const selected=Array.isArray(state.selectedIds)&&state.selectedIds.length
          ? [...state.selectedIds]
          : (state.selectedId?[state.selectedId]:[]);
        ...
        while(changed){
          changed=false;
          state.pieces.forEach(piece=>{
            if(ids.has(piece.id)&&isLinkedSplash(piece)&&piece.attachment?.parentPieceId&&!ids.has(piece.attachment.parentPieceId)){
              ids.add(piece.attachment.parentPieceId);changed=true;
            }
            if(!isLinkedSplash(piece)||!piece.attachment?.parentPieceId||!isSplashSnappedToParent(piece))return;
            if(ids.has(piece.attachment.parentPieceId)&&!ids.has(piece.id)){ids.add(piece.id);changed=true;}
          });
        }
        ...
        const poses=pieces.map(piece=>{
          const rs=realSize(piece);
          return {piece,rs,cx:(Number(piece.x)||0)+rs.w/2,cy:(Number(piece.y)||0)+rs.h/2};
        });
        const minX=Math.min(...poses.map(item=>item.cx-item.rs.w/2));
        const maxX=Math.max(...poses.map(item=>item.cx+item.rs.w/2));
        const minY=Math.min(...poses.map(item=>item.cy-item.rs.h/2));
        const maxY=Math.max(...poses.map(item=>item.cy+item.rs.h/2));
        const axisX=(minX+maxX)/2,axisY=(minY+maxY)/2;

        poses.forEach(item=>{
          const piece=item.piece;
          const targetCenter={
            x:axis==='h'?2*axisX-item.cx:item.cx,
            y:axis==='v'?2*axisY-item.cy:item.cy
          };
          piece.rotation=normDeg(-(Number(piece.rotation)||0));
          mirrorPieceLocal(piece,axis);
          const rs=realSize(piece);
          piece.x=round3(targetCenter.x-rs.w/2);
          piece.y=round3(targetCenter.y-rs.h/2);
        });

        syncLinkedSplashRelationships();
        pieces.forEach(piece=>clampToCanvas(piece));
        syncLinkedSplashRelationships();
      ...
      function createBacksplashFromEdge(parent,edge){
        ...
        const childContact=splashContactEdge(child);
        if(childContact&&normalizeEdgeProfile(parent.edgeProfiles?.[edge])==='miter')child.edgeProfiles[childContact]='miter';
        clampToCanvas(child);
      ...
      function edgeProfileDisplayText(value){
        const profile=normalizeEdgeProfile(value);
        if(profile==='none')return '';
        return state.edgeLabelMode==='symbol'
          ? (EDGE_PROFILE_SYMBOLS[profile]||'?')
          : (EDGE_PROFILE_LABELS[profile]||profile);
      }
...
function createSinkRadiusAnnotation(piece,sink,corner){
  ...
  const note={
    ...
    radiusRef:{kind:'sink',pieceId:piece.id,sinkId:sink.id,corner,autoText:true,lastTarget:{x:target.x,y:target.y}}
  };
  const line={
    id:uid(),
    ...
    attachedEnd:'start',
    annotationType:'radius',
    radiusRef:{kind:'sink',pieceId:piece.id,sinkId:sink.id,corner,lastTarget:{x:target.x,y:target.y}}
  };
  L.notes.push(note);
  L.lines.push(line);
  ...
  return note;
      ...
      function positionEdgePainterHUD(){
        const hud=document.querySelector('.lite-cad .lc-edge-painter-hud');
        const wrap=canvasCard?.querySelector('.lc-canvas-wrap');
        if(!hud||hud.hidden||!wrap)return;
        const rect=wrap.getBoundingClientRect();
        requestAnimationFrame(()=>{
          if(hud.hidden)return;
          const width=Math.max(1,hud.offsetWidth||260);
          const height=Math.max(1,hud.offsetHeight||36);
          const minLeft=Math.max(8,rect.left+8);
          const maxLeft=Math.max(minLeft,Math.min(window.innerWidth-width-8,rect.right-width-8));
          const top=Math.max(8,Math.min(window.innerHeight-height-8,rect.top+8));
          hud.style.left=Math.round(maxLeft)+'px';
          hud.style.top=Math.round(top)+'px';
        });
      }

      function syncEdgePainterHUD(tool=activeMomentaryTool()){
        const root=document.querySelector('.lite-cad');
        if(!root)return;
        let hud=root.querySelector(':scope > .lc-edge-painter-hud');
        if(tool!=='edgePainter'||state.workspace==='slab'){
          if(hud)hud.hidden=true;
          setTimeout(()=>{try{syncViewMenuUI?.();syncTopBar?.();}catch(_){}},0);
          return;
        }

        if(!EDGE_PROFILE_VALUES.includes(edgePainterProfile))edgePainterProfile='quarter';
        if(!hud){
          hud=document.createElement('div');
          hud.className='lc-edge-painter-hud';
          const label=document.createElement('span');
          label.className='lc-edge-painter-hud-title';
          label.textContent='Edge Painter';

          const select=document.createElement('select');
          select.className='lc-edge-painter-select';
          select.setAttribute('aria-label','Edge profile to paint');
          EDGE_PROFILE_VALUES.forEach(value=>{
            const option=document.createElement('option');
            option.value=value;
            option.textContent=EDGE_PROFILE_LABELS[value]||value;
            select.appendChild(option);
          });
          select.onchange=()=>setEdgePainterProfile(select.value);

          const hint=document.createElement('span');
          hint.className='lc-edge-painter-hud-hint';
          hint.textContent='Click edges · Shift+E / Enter to exit';

          hud.append(label,select,hint);
          root.appendChild(hud);
        }
        const select=hud.querySelector('.lc-edge-painter-select');
        if(select)select.value=normalizeEdgeProfile(edgePainterProfile);
        hud.hidden=false;
        positionEdgePainterHUD();
        setTimeout(()=>{try{syncViewMenuUI?.();syncTopBar?.();}catch(_){}},0);
      }

      window.addEventListener('resize',()=>{if(activeMomentaryTool()==='edgePainter')positionEdgePainterHUD();});
      window.addEventListener('scroll',()=>{if(activeMomentaryTool()==='edgePainter')positionEdgePainterHUD();},true);

      function drawEdgePainterInteractionLayer(){
        if(activeMomentaryTool()!=='edgePainter'||state.workspace==='slab')return;
        const layer=document.createElementNS(svgNS,'g');
        layer.setAttribute('class','lc-edge-painter-layer');
        layer.setAttribute('data-momentary-layer','edgePainter');

        state.pieces
          .forEach(piece=>{
            migratePieceGeometry(piece);
            const rs=realSize(piece);
            const cx=i2p((Number(piece.x)||0)+rs.w/2);
            const cy=i2p((Number(piece.y)||0)+rs.h/2);
            const W0=i2p(Math.max(.25,Number(piece.w)||.25));
            const H0=i2p(Math.max(.25,Number(piece.h)||.25));
            const rot=((Number(piece.rotation)||0)%360+360)%360;

            const group=document.createElementNS(svgNS,'g');
            group.setAttribute('class','lc-edge-painter-piece');
            group.setAttribute('data-edge-painter-piece-id',piece.id);
            if(rot)group.setAttribute('transform',`rotate(${rot} ${cx} ${cy})`);

            const edges={
              top:[cx-W0/2,cy-H0/2,cx+W0/2,cy-H0/2],
              right:[cx+W0/2,cy-H0/2,cx+W0/2,cy+H0/2],
              bottom:[cx+W0/2,cy+H0/2,cx-W0/2,cy+H0/2],
              left:[cx-W0/2,cy+H0/2,cx-W0/2,cy-H0/2]
            };

            Object.entries(edges).forEach(([side,coords])=>{
              const [x1,y1,x2,y2]=coords;
              const option=document.createElementNS(svgNS,'g');
              option.setAttribute('class','lc-edge-paint-option');
              option.setAttribute('data-edge-side',side);

              const preview=svgEl('line',{
                x1,y1,x2,y2,
                class:'lc-edge-paint-preview',
                'vector-effect':'non-scaling-stroke',
                'pointer-events':'none'
              });
              ...
              hit.setAttribute('class','lc-edge-paint-hit');
              hit.setAttribute('data-edge-paint-hit','1');
              hit.setAttribute('data-edge-side',side);

              const title=document.createElementNS(svgNS,'title');
              title.textContent=`Paint ${EDGE_PROFILE_LABELS[normalizeEdgeProfile(edgePainterProfile)]||edgePainterProfile} on ${side} edge`;
              hit.appendChild(title);
              ...
              hit.addEventListener('pointerdown',ev=>{
                ...
                ev.stopPropagation();
                ev.stopImmediatePropagation?.();
                const next=normalizeEdgeProfile(edgePainterProfile);
                if(piece.edgeProfiles[side]===next)return;
                const previous=normalizeEdgeProfile(piece.edgeProfiles[side]);
                piece.edgeProfiles[side]=next;
                syncLinkedMiterProfile(piece,side,next,previous);
                draw();
                if(isSelected(piece.id))updateInspector();
                scheduleSave();
                pushHistory();
              });
      ...
      function draw(){
        syncCanvasWorkspaceTabs();
        if(activeMomentaryTool()==='edgePainter'&&state.workspace!=='slab')state.showEdgeProfiles=true;
        if(state.workspace==='slab'){
          drawSlabWorkspace();
          return;
        }
        ...
        sorted.forEach((p)=>{
         ...
         // append rotated geometry group to a piece container g (for pointerdown)
          ...
          if (showThisPieceLabel) {
            ...
            text.setAttribute('fill', pieceLabelTextColor(p,fg));
          ...
          if (state.showEdgeProfiles && p.edgeProfiles) {
            ...
            function addEdgeLabel(text, x, y, anchor, baseline, rotation) {
              ...
              t.setAttribute('font-size', symbol?'12':'11');
              if(symbol)t.setAttribute('font-weight','800');
              t.setAttribute('fill', '#111');
              t.textContent = edgeProfileDisplayText(profile);
              labelsG.appendChild(t);
            }

            const MARGIN = state.edgeLabelMode==='symbol'?0:8; // symbols straddle the edge
        ...
        drawToolPreview();
        drawPlanCalibrationOverlay();
        drawOverlayInteractionLayer();
        drawSplashInteractionLayer();
        drawRadiusInteractionLayer();
        drawEdgePainterInteractionLayer();
        raiseEdgeProfileLabels();
        drawPieceRotateHandle();
        drawPieceResizeHandles();
        drawEdgeProfileLegend();
      ...
      function pieceItem(p, i){
        ...
        btnDel.addEventListener('click',e=>{
          e.preventDefault();
          e.stopPropagation();
          // Route navigator deletion through the same canonical path as
          // Inspector Delete / keyboard Delete. A parent therefore deletes its
          // entire linked Splash family; deleting a child still deletes only
          // that child.
          selectOnly(p.id);
          dispatchCadKey('Delete');
        });
...
function syncRadiusAnnotations(L=cur()){
  ...
  L.notes.forEach(note=>{
    ...
    // Keep the entire annotation attached to the geometry. The previous target
    // is persisted with the Note, so any manually adjusted Note offset survives
    // piece/sink moves, rotations and geometry edits.
    const previous=ref.lastTarget;
    const leaders=noteLeadersFor(note.id,L);
    if(previous&&Number.isFinite(Number(previous.x))&&Number.isFinite(Number(previous.y))){
      const dx=round3(target.x-Number(previous.x));
      const dy=round3(target.y-Number(previous.y));
      if(Math.abs(dx)>.0001||Math.abs(dy)>.0001){
        note.x=round3((Number(note.x)||0)+dx);
        note.y=round3((Number(note.y)||0)+dy);
        leaders.forEach(lineObj=>{
          lineObj.x1=round3((Number(lineObj.x1)||0)+dx);
          lineObj.y1=round3((Number(lineObj.y1)||0)+dy);
          lineObj.x2=round3((Number(lineObj.x2)||0)+dx);
          lineObj.y2=round3((Number(lineObj.y2)||0)+dy);
        });
      }
    }

    ref.lastTarget={x:round3(target.x),y:round3(target.y)};
    note.radiusRef=ref;
    if(ref.autoText!==false)note.text=radiusLabelText(target.radius);

    leaders.forEach(lineObj=>{
      lineObj.annotationType='radius';
      lineObj.radiusRef={...ref,lastTarget:{...ref.lastTarget}};
      // Re-pin the geometry end exactly after translating the whole annotation.
      if(lineObj.attachedEnd==='end'){
...
function createRadiusAnnotation(piece,corner){
  ...
  const note={
    ...
    text:radiusLabelText(target.radius),
    annotationType:'radius',
    radiusRef:{kind:'piece',pieceId:piece.id,corner,autoText:true,lastTarget:{x:target.x,y:target.y}}
  };
  const line={
    id:uid(),
    name:'Radius Leader',
    ...
    attachedEnd:'start',
    annotationType:'radius',
    radiusRef:{kind:'piece',pieceId:piece.id,corner,lastTarget:{x:target.x,y:target.y}}
  };
  L.notes.push(note);
  L.lines.push(line);
...
function normalizeNoteRotation(value){
  const n=Number(value);
  if(!Number.isFinite(n))return 0;
  const normalized=((n%360)+360)%360;
  return round3(Math.abs(normalized-360)<0.0001?0:normalized);
}

function noteTextFormat(note){
  const isRadius=note?.annotationType==='radius';
  const sizeRaw=Number(note?.fontSize);
  const align=['left','center','right'].includes(note?.align)?note.align:'left';
  const color=/^#[0-9a-f]{6}$/i.test(String(note?.color||''))?note.color:'#111111';
  return {
    size:clamp(Number.isFinite(sizeRaw)?sizeRaw:12,6,48),
    bold:note?.bold==null?isRadius:!!note.bold,
    italic:!!note?.italic,
    align,
    color,
    halo:!!note?.halo,
    rotation:normalizeNoteRotation(note?.rotation)
  };
}
  ...
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      function inspectorField(labelText,input){
        ...
        lab.className='lc-label lc-inspector-field';
        const cap=document.createElement('span');
        cap.className='lc-inspector-field-label';
        cap.textContent=labelText;
        lab.append(cap,input);
        return lab;
      }

      function inspectorGlyph(name){
        const paths={
          geometry:'<path d="M4 18 18 4M5 14l5 5M14 5l5 5M8 14l2 2M11 11l2 2M14 8l2 2"/>',
          label:'<path d="M4 6h10l6 6-8 8-8-8V6Z"/><circle cx="9" cy="11" r="1.5"/>',
          appearance:'<path d="M4 7h16M4 12h10M4 17h16"/><path d="M17 10v4"/>',
          endpoints:'<path d="M5 12h14"/><circle cx="5" cy="12" r="2"/><path d="m16 9 3 3-3 3"/>',
          size:'<path d="M4 7V4h3M20 7V4h-3M4 17v3h3M20 17v3h-3"/><path d="M7 12h10M9 10l-2 2 2 2M15 10l2 2-2 2"/>',
          position:'<circle cx="12" cy="12" r="6"/><path d="M12 3v4M12 17v4M3 12h4M17 12h4"/>',
          display:'<path d="M2.5 12s3.5-5 9.5-5 9.5 5 9.5 5-3.5 5-9.5 5-9.5-5-9.5-5Z"/><circle cx="12" cy="12" r="2.5"/>',
          source:'<rect x="4" y="5" width="16" height="14" rx="1.5"/><path d="m6.5 16 4-4 3 3 2-2 3 3"/><circle cx="15.5" cy="9" r="1.3"/>'
        };
        const path=paths[name]||paths.geometry;
        return `<svg class="lc-inspector-glyph" viewBox="0 0 24 24" aria-hidden="true">${path}</svg>`;
      }

      function inspectorGroupTitle(text,icon){
        const row=document.createElement('div');
        row.className='lc-inspector-group-title';
        if(icon){
          const glyph=document.createElement('span');
          glyph.className='lc-inspector-group-icon';
          glyph.innerHTML=inspectorGlyph(icon);
      ...
      function inspectorUnitInput(input,unit){
        const wrap=document.createElement('div');
        wrap.className='lc-inspector-unit-input';
      ...
      function renderOverlayInspector(){
        if(state.workspace!=='slab')return false;
        const L=ensureOverlaysOnLayout(activeLayout());
        const o=currentOverlay();
        if(!L||!o)return false;
        L.overlayClip=true;
        ...
        sizeGrid.className='lc-inspector-property-grid lc-slab-size-grid';
        ...
        posGrid.className='lc-inspector-property-grid lc-slab-position-grid';
        ...
        opacityWrap.className='lc-label lc-overlay-opacity-field lc-inspector-field';
        const opacityHead=document.createElement('div');
        opacityHead.className='lc-overlay-opacity-head';
        const opacityLabel=document.createElement('span');opacityLabel.className='lc-inspector-field-label';opacityLabel.textContent='Opacity';
        ...
        visibleBtn.className='lc-btn ghost sm lc-overlay-visible-btn'+(o.visible!==false?' is-active':'');
        visibleBtn.setAttribute('aria-pressed',String(o.visible!==false));
        visibleBtn.innerHTML=inspectorGlyph('display')+'<span>Visible</span>';
        visibleBtn.onclick=()=>{
          o.visible=o.visible===false;
          visibleBtn.classList.toggle('is-active',o.visible!==false);
          visibleBtn.setAttribute('aria-pressed',String(o.visible!==false));
          draw();renderOverlayList();scheduleSave();pushHistory();
        };

        toggles.append(visibleBtn);
        body.appendChild(toggles);

        if(o.natW&&o.natH){
          body.appendChild(inspectorGroupTitle('Source','source'));
          inspectorSummary(body,[['Image',o.natW+' × '+o.natH+' px'],['Slabs',String(L.overlays.length)]]);
        }
      ...
      function renderAnnotationInspector(){
        ...
        if(d){
          ...
          body.appendChild(inspectorField('Offset',inspectorUnitInput(off,'px')));
        ...
        if(lineObj){
          ...
          color.className='lc-input lc-line-color-input';color.type='color';
          ...
          mainGrid.className='lc-inspector-property-grid lc-line-appearance-grid';
          mainGrid.append(
            inspectorField('Style',style),
            inspectorField('Thickness',thickness),
            inspectorField('Color',color)
          );
          ...
          body.appendChild(inspectorGroupTitle('End Caps','endpoints'));
        ...
        if(note){
          ...
          const noteFmt=noteTextFormat(note);

          const noteTools=document.createElement('div');
          noteTools.className='lc-note-tools';

          const makeToolGroup=(label,className='')=>{
            const group=document.createElement('div');
            group.className='lc-note-tool-group'+(className?' '+className:'');
            const caption=document.createElement('div');
            caption.className='lc-note-tool-label';
            caption.textContent=label;
            const controls=document.createElement('div');
            controls.className='lc-note-tool-controls';
            group.append(caption,controls);
            return {group,controls};
          };

          const iconSvg=(paths,viewBox='0 0 24 24')=>`<svg class="lc-note-tool-icon" viewBox="${viewBox}" aria-hidden="true">${paths}</svg>`;
          const alignIcon=(mode)=>{
            const rows=mode==='left'
              ? [[3,21],[3,16],[3,20],[3,14]]
              : mode==='center'
                ? [[3,21],[6,18],[4,20],[7,17]]
                : [[3,21],[8,21],[4,21],[10,21]];
            return iconSvg(rows.map((r,i)=>`<path d="M${r[0]} ${6+i*4}H${r[1]}"/>`).join(''));
          };
          const rotateIcon=(dir)=>iconSvg(dir==='left'
            ? '<path d="M8 7H3V2"/><path d="M3.5 7.5A9 9 0 1 1 4 17"/>'
            : '<path d="M16 7h5V2"/><path d="M20.5 7.5A9 9 0 1 0 20 17"/>');

          const sizeGroup=makeToolGroup('Font Size','lc-note-size-group');
          const sizeWrap=document.createElement('div');
          sizeWrap.className='lc-note-size-wrap';
          const sizeIcon=document.createElement('span');
          sizeIcon.className='lc-note-size-icon';
          sizeIcon.innerHTML=iconSvg('<path d="M3 5h10M8 5v14M16 7h5M18.5 7v10"/><path d="m16.5 9 2-2 2 2M16.5 15l2 2 2-2"/>');
          const size=document.createElement('input');
          size.type='number';
          size.className='lc-input lc-note-size-input';
          size.min='6';size.max='48';size.step='1';
          size.value=String(noteFmt.size);
          size.title='Font size';
          size.setAttribute('aria-label','Font size');
          size.oninput=()=>{note.fontSize=clamp(Number(size.value)||12,6,48);draw();};
          size.onchange=()=>{
            note.fontSize=clamp(Number(size.value)||12,6,48);
            size.value=String(note.fontSize);
            scheduleSave();pushHistory();
          };
          sizeWrap.append(sizeIcon,size);
          sizeGroup.controls.appendChild(sizeWrap);

          const makeFormatToggle=(label,key,title)=>{
            const btn=document.createElement('button');
            btn.type='button';
            btn.className='lc-btn ghost sm lc-note-format-btn'+(noteTextFormat(note)[key]?' is-active':'');
            btn.textContent=label;
            btn.title=title;
            btn.setAttribute('aria-label',title);
            btn.setAttribute('aria-pressed',String(!!noteTextFormat(note)[key]));
            btn.onclick=()=>{
              note[key]=!noteTextFormat(note)[key];
              draw();scheduleSave();pushHistory();updateInspector();
            };
            return btn;
          };

          const styleGroup=makeToolGroup('Style','lc-note-style-group');
          const boldBtn=makeFormatToggle('B','bold','Bold');
          boldBtn.style.fontWeight='800';
          const italicBtn=makeFormatToggle('I','italic','Italic');
          italicBtn.style.fontStyle='italic';
          styleGroup.controls.append(boldBtn,italicBtn);

          const colorGroup=makeToolGroup('Color','lc-note-color-group');
          const textColor=document.createElement('input');
          textColor.type='color';
          textColor.className='lc-input lc-note-color-input';
          textColor.value=noteFmt.color;
          textColor.title='Text color';
          textColor.setAttribute('aria-label','Text color');
          textColor.oninput=()=>{note.color=textColor.value;draw();};
          textColor.onchange=()=>{scheduleSave();pushHistory();};
          colorGroup.controls.appendChild(textColor);

          const haloGroup=makeToolGroup('Halo','lc-note-halo-group');
          const haloBtn=makeFormatToggle('Halo','halo','White text halo');
          haloBtn.classList.add('lc-note-halo-btn');
          haloGroup.controls.appendChild(haloBtn);

          const typographyRow=document.createElement('div');
          typographyRow.className='lc-note-typography-row';
          typographyRow.append(sizeGroup.group,styleGroup.group);
          noteTools.appendChild(typographyRow);

          const colorHaloRow=document.createElement('div');
          colorHaloRow.className='lc-note-color-halo-row';
          colorHaloRow.append(colorGroup.group,haloGroup.group);
          noteTools.appendChild(colorHaloRow);

          const alignmentGroup=makeToolGroup('Alignment','lc-note-alignment-section');
          const alignButtons=document.createElement('div');
          alignButtons.className='lc-note-align-group';
          [
            ['left','Align left'],
            ['center','Align center'],
            ['right','Align right']
            ...
            const btn=document.createElement('button');
            btn.type='button';
            btn.className='lc-btn ghost sm lc-note-align-btn'+(noteFmt.align===value?' is-active':'');
            btn.innerHTML=alignIcon(value);
            btn.title=title;
            btn.setAttribute('aria-label',title);
            btn.setAttribute('aria-pressed',String(noteFmt.align===value));
            btn.onclick=()=>{
              note.align=value;
              draw();scheduleSave();pushHistory();updateInspector();
            };
          ...
          alignmentGroup.controls.appendChild(alignButtons);
          noteTools.appendChild(alignmentGroup.group);

          const rotationGroup=makeToolGroup('Rotation','lc-note-rotation-section');
          const rotationControl=document.createElement('div');
          rotationControl.className='lc-note-rotation-control';

          const rotateLeft=document.createElement('button');
          rotateLeft.type='button';
          rotateLeft.className='lc-btn ghost sm lc-note-rotation-btn lc-note-rotation-icon-btn';
          rotateLeft.innerHTML=rotateIcon('left');
          rotateLeft.title='Rotate note 90° counterclockwise';
          rotateLeft.setAttribute('aria-label','Rotate note 90 degrees counterclockwise');
          rotateLeft.onclick=()=>{
            note.rotation=normalizeNoteRotation(noteTextFormat(note).rotation-90);
            draw();scheduleSave();pushHistory();updateInspector();
          };

          const rotationInputWrap=document.createElement('div');
          rotationInputWrap.className='lc-note-rotation-input-wrap';
          const rotationInput=document.createElement('input');
          rotationInput.type='number';
          rotationInput.className='lc-input lc-note-rotation-input';
          rotationInput.step='1';
          rotationInput.value=String(noteFmt.rotation);
          rotationInput.title='Note rotation in degrees';
          rotationInput.setAttribute('aria-label','Note rotation in degrees');
          ...
          rotationInput.onchange=()=>{
            note.rotation=normalizeNoteRotation(rotationInput.value);
            rotationInput.value=String(note.rotation);
            draw();scheduleSave();pushHistory();
          };
          const degreeMark=document.createElement('span');
          degreeMark.className='lc-note-rotation-degree';
          degreeMark.textContent='°';
          rotationInputWrap.append(rotationInput,degreeMark);

          const resetRotation=document.createElement('button');
          resetRotation.type='button';
          resetRotation.className='lc-btn ghost sm lc-note-rotation-btn lc-note-rotation-reset';
          resetRotation.textContent='0°';
          resetRotation.title='Reset rotation';
          ...
          rotateRight.className='lc-btn ghost sm lc-note-rotation-btn lc-note-rotation-icon-btn';
          rotateRight.innerHTML=rotateIcon('right');
          rotateRight.title='Rotate note 90° clockwise';
          rotateRight.setAttribute('aria-label','Rotate note 90 degrees clockwise');
          rotateRight.onclick=()=>{
            note.rotation=normalizeNoteRotation(noteTextFormat(note).rotation+90);
            draw();scheduleSave();pushHistory();updateInspector();
          };

          rotationControl.append(rotateLeft,rotationInputWrap,resetRotation,rotateRight);
          rotationGroup.controls.appendChild(rotationControl);
          noteTools.appendChild(rotationGroup.group);

          body.appendChild(noteTools);
      ...
      function updateInspector(){
        ...
        seamVisibilityInspectorEye=null;
        pieceInspectorPreviewSync=()=>{};
        ...
        if (!p) {
            ...
            inspector.textContent = state.workspace==='slab'
              ? 'Select a piece or SLAB from the canvas or list.'
              : 'Select a piece, dimension, note, or line.';
            return;
        }
        inspector.className = '';
        inspector.innerHTML = '';
        ...
        pieceVisual.className='lc-piece-visual-card';
        const visualHead=document.createElement('div');
        visualHead.className='lc-piece-visual-head';
        const visualTitle=document.createElement('span');
        visualTitle.className='lc-piece-visual-title';
        visualHead.appendChild(visualTitle);

        const visualSvg=document.createElementNS(svgNS,'svg');
        visualSvg.setAttribute('viewBox','0 0 240 132');
        visualSvg.setAttribute('class','lc-piece-visual-svg');
        visualSvg.setAttribute('aria-label','Selected piece preview');
        visualSvg.setAttribute('pointer-events','none');

        const currentPieceInspectorMode=()=>{
          const key=Object.keys(inspectorSectionOpen).find(k=>inspectorSectionOpen[k]);
          return key||'pieceInfo';
        };
        const modeNames={
          pieceInfo:'PIECE',
          ...
          sinks:'SINKS',
          seams:'SEAMS',
          edgeOptions:'EDGES & CORNERS'
        };

        const renderPieceVisual=()=>{
          const mode=currentPieceInspectorMode();
          visualTitle.textContent=(String(p.name||'Piece').trim()||'Piece')+' · '+(modeNames[mode]||'PIECE');
          visualSvg.replaceChildren();

          const pw=Math.max(.25,Number(p.w)||.25);
          const ph=Math.max(.25,Number(p.h)||.25);
          const maxW=166,maxH=86;
          const scale=Math.min(maxW/pw,maxH/ph);
          const w=Math.max(28,pw*scale),h=Math.max(22,ph*scale);
          const x=(240-w)/2,y=(132-h)/2+3;
          ...
          rect.setAttribute('class','lc-piece-visual-shape');
          rect.setAttribute('fill',p.noFill?'none':(p.color||'#ffffff'));
          rect.setAttribute('fill-opacity',String(p.noFill?0:Math.max(.08,getFillOpacity(p)*Math.max(0,Math.min(1,Number(state.pieceFillOpacity??1))))));
          rect.setAttribute('stroke','#374151');
          rect.setAttribute('stroke-width','1.2');
          visualSvg.appendChild(rect);

          if(mode==='seams'&&Array.isArray(p.pieceSeams)){
            p.pieceSeams.forEach(ps=>{
              let x1,y1,x2,y2;
              if(ps.orientation==='horizontal'){
                const off=ps.reference==='bottom'?ph-(Number(ps.offset)||0):(Number(ps.offset)||0);
                y1=y2=y+off*sy;x1=x;x2=x+w;
              }else{
                const off=ps.reference==='right'?pw-(Number(ps.offset)||0):(Number(ps.offset)||0);
                x1=x2=x+off*sx;y1=y;y2=y+h;
              }
              visualSvg.appendChild(svgEl('line',{
                x1,y1,x2,y2,stroke:'#111','stroke-width':1.4,'stroke-dasharray':'5 3',
                'vector-effect':'non-scaling-stroke'
              }));
            });
          }

          if(mode==='sinks'&&Array.isArray(p.sinks)){
            p.sinks.forEach(sink=>{
              const pose=sinkPoseOnPiece(p,sink);
              const cx=x+pose.cx*sx,cy=y+pose.cy*sy;
              const localAngle=sinkReferenceAngle(sink.side||'front')+(Number(sink.rotation)||0);
              const g=document.createElementNS(svgNS,'g');
              g.setAttribute('transform',`translate(${cx} ${cy}) rotate(${localAngle})`);

              const sw=Math.max(5,(Number(sink.w)||1)*sx);
              const sh=Math.max(5,(Number(sink.h)||1)*sy);
              if(sink.shape==='oval'){
                g.appendChild(svgEl('ellipse',{cx:0,cy:0,rx:sw/2,ry:sh/2,fill:'none',stroke:'#111','stroke-width':1}));
              }else{
                g.appendChild(svgEl('rect',{
                  x:-sw/2,y:-sh/2,width:sw,height:sh,
                  rx:Math.min(5,(Number(sink.cornerR)||0)*Math.min(sx,sy)),
                  fill:'none',stroke:'#111','stroke-width':1
                }));
              }

              if(Array.isArray(sink.faucets)&&sink.faucets.length){
                const spacing=faucetHoleSpacing(sink)*sx;
                const holeY=-(sh/2+holeOffsetFromSinkEdge(sink)*sy);
                const radius=Math.max(1.3,faucetHoleDiameter(sink)*Math.min(sx,sy)/2);
                sink.faucets.forEach(idx=>{
                  g.appendChild(svgEl('circle',{
                    cx:(-4+idx)*spacing,cy:holeY,r:radius,
                    fill:'none',stroke:'#111','stroke-width':1
                  }));
                });
              }
          ...
          if(mode==='edgeOptions'&&p.edgeProfiles){
            ...
            const edgeText=(side,tx,ty,rot=0)=>{
              const value=edgeProfileDisplayText(p.edgeProfiles[side]);
              if(!value)return;
              const t=svgEl('text',{
                x:tx,y:ty,'text-anchor':'middle','dominant-baseline':'middle',
                'font-size':symbol?'11':'9.5',fill:'#174ea6','font-weight':symbol?'800':'700'
              });
              if(rot)t.setAttribute('transform',`rotate(${rot} ${tx} ${ty})`);
              t.textContent=value;
              visualSvg.appendChild(t);
            };
            edgeText('top',x+w/2,y+inset);
            edgeText('bottom',x+w/2,y+h-inset,180);
            edgeText('left',x+inset,y+h/2,-90);
            edgeText('right',x+w-inset,y+h/2,90);
          }
          ...
          const label=svgEl('text',{
            ...
            fill:pieceLabelTextColor(p,pickTextColor(p.color||'#ffffff'))
          });
          label.textContent=String(p.name||'Piece').trim()||'Piece';
          visualSvg.appendChild(label);
        };

        pieceInspectorPreviewSync=renderPieceVisual;
        renderPieceVisual();
        pieceVisual.append(visualHead,visualSvg);
        root.appendChild(pieceVisual);
        ...
        btnDup.onclick = (e) => {
            e.preventDefault();
            e.stopPropagation();
            // Reuse the canonical duplication path so linked splashes and
            // multi-selection relationships behave exactly like Ctrl+D.
            dispatchCadKey('d',{ctrlKey:true});
        };
        ...
        btnDel.onclick = (e) => {
            e.preventDefault();
            e.stopPropagation();
            // Keep Inspector Delete identical to keyboard/menu Delete, including
            // linked splash assemblies.
            dispatchCadKey('Delete');
        };
        ...
        if(state.workspace!=='slab'){
          ...
          mirrorRow.className='lc-piece-mirror-row';
          const mirrorH=document.createElement('button');
          mirrorH.type='button';
          mirrorH.className='lc-btn ghost sm lc-piece-mirror-btn';
          mirrorH.title='Mirror selected piece(s) left ↔ right';
          mirrorH.innerHTML='<svg viewBox="0 0 20 16" aria-hidden="true"><path d="M10 2v12M8 3 3 8l5 5M12 3l5 5-5 5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Mirror H</span>';
          mirrorH.onclick=e=>{e.preventDefault();mirrorSelectedPieces('h');};
          const mirrorV=document.createElement('button');
          mirrorV.type='button';
          mirrorV.className='lc-btn ghost sm lc-piece-mirror-btn';
          mirrorV.title='Mirror selected piece(s) top ↔ bottom';
          mirrorV.innerHTML='<svg viewBox="0 0 20 16" aria-hidden="true"><path d="M3 8h14M5 6l5-4 5 4M5 10l5 4 5-4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Mirror V</span>';
          mirrorV.onclick=e=>{e.preventDefault();mirrorSelectedPieces('v');};
          mirrorRow.append(mirrorH,mirrorV);
          pieceBody.appendChild(mirrorRow);
        }
        ...
        if(state.workspace!=='slab'&&!isBacksplashPiece(p)){
          const linkedChildren=state.pieces.filter(child=>
            isLinkedSplash(child)&&child.attachment?.parentPieceId===p.id
          );

          const splashActions=document.createElement('div');
          splashActions.className='lc-piece-splash-actions';
          ...
          splashActions.appendChild(addSplash);

          if(linkedChildren.length){
            const unlinkAll=document.createElement('button');
            unlinkAll.type='button';
            unlinkAll.className='lc-btn ghost sm lc-inspector-full-action lc-unlink-splashes-action';
            unlinkAll.title='Unlink every splash from this piece without deleting them.';
            unlinkAll.innerHTML='<svg viewBox="0 0 20 16" aria-hidden="true"><path d="M7.5 5.5 5.7 3.7a3 3 0 0 0-4.2 4.2l2.2 2.2a3 3 0 0 0 4.2 0l1.1-1.1M12.5 10.5l1.8 1.8a3 3 0 0 0 4.2-4.2l-2.2-2.2a3 3 0 0 0-4.2 0L11 7M7 8h6" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Unlink All Splashes</span>';
            unlinkAll.onclick=e=>{
              e.preventDefault();
              e.stopPropagation();
            ...
            splashActions.appendChild(unlinkAll);
          }

          pieceBody.appendChild(splashActions);
          syncMomentaryToolUI();
        ...
        if(sinksMountEl){
          ...
          sinkCenterlineInspectorEye.className='lc-btn ghost lc-iconbtn lc-head-eye lc-sinks-dims-eye';
        ...
        const seamArrow=seamHead?.querySelector('.lc-head-arrow');
        if(seamHead&&seamArrow){
          seamArrow.remove();
          seamVisibilityInspectorEye=document.createElement('button');
          seamVisibilityInspectorEye.type='button';
          seamVisibilityInspectorEye.className='lc-btn ghost lc-iconbtn lc-head-eye lc-seams-eye';
          seamVisibilityInspectorEye.onclick=e=>{
            e.preventDefault();e.stopPropagation();
            state.showSeams=!state.showSeams;
            syncSeamVisibilityUI();
            syncViewMenuUI?.();
            draw();scheduleSave?.();pushHistory?.();syncTopBar?.();
          };
          const seamHeadActions=document.createElement('span');
          seamHeadActions.className='lc-sinks-head-actions';
          seamHeadActions.append(seamVisibilityInspectorEye,seamArrow);
          seamHead.appendChild(seamHeadActions);
          syncSeamVisibilityUI();
        }
        clampPieceSeams(p);
    ...
    del.innerHTML='<svg class="lc-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l1 2h4v2H3V5h4l1-2Zm-2 6h12l-1 11H7L6 9Zm3 2v7h2v-7H9Zm4 0v7h2v-7h-2Z" fill="currentColor"/></svg>';
        ...
        const inspectorEdgeMode=document.createElement('div');
        inspectorEdgeMode.className='lc-inspector-edge-mode';
        const inspectorEdgeModeLabel=document.createElement('span');
        inspectorEdgeModeLabel.textContent='Edge Labels';
        inspectorEdgeMode.setAttribute('aria-label','Edge label display');
        const inspectorEdgeModeButtons=document.createElement('div');
        inspectorEdgeModeButtons.className='lc-edge-label-mode-choices';
        const syncInspectorEdgeMode=()=>{
          inspectorEdgeModeButtons.querySelectorAll('[data-edge-label-mode]').forEach(btn=>{
            const on=btn.dataset.edgeLabelMode===(state.edgeLabelMode==='symbol'?'symbol':'text');
            btn.classList.toggle('is-selected',on);
            btn.setAttribute('aria-pressed',String(on));
          });
        ...
        [
          ...
          btn.type='button';
          btn.className='lc-edge-label-mode-btn';
          btn.dataset.edgeLabelMode=mode;
          btn.title=title;
          btn.innerHTML=icon;
          btn.onclick=e=>{
            e.preventDefault();e.stopPropagation();
            ...
            state.showEdgeProfiles=true;
            saveCurrentWorkspaceViewState?.();
            syncInspectorEdgeMode();syncViewMenuUI?.();draw();scheduleSave();pushHistory();
          };
          inspectorEdgeModeButtons.appendChild(btn);
        });
        syncInspectorEdgeMode();
        inspectorEdgeMode.append(inspectorEdgeModeLabel,inspectorEdgeModeButtons);
        ...
        function makeEdgeSelect(side,labelText){
          ...
          edgeOptions.forEach(opt=>{
            ...
            row.onclick=e=>{
              e.preventDefault();
              e.stopPropagation();
              const previous=normalizeEdgeProfile(p.edgeProfiles[side]);
              const next=normalizeEdgeProfile(opt.v);
              p.edgeProfiles[side]=next;
              syncLinkedMiterProfile(p,side,next,previous);
              closePicker();
              sync();
              draw();
              scheduleSave();
        ...
        const edgePainterRow=document.createElement('div');
        edgePainterRow.className='lc-edge-painter-control-row';

        const edgePainterProfileSelect=document.createElement('select');
        edgePainterProfileSelect.className='lc-input lc-edge-painter-profile-select';
        edgePainterProfileSelect.setAttribute('aria-label','Edge profile to paint');
        EDGE_PROFILE_VALUES.forEach(value=>{
          const option=document.createElement('option');
          option.value=value;
          option.textContent=EDGE_PROFILE_LABELS[value]||value;
          edgePainterProfileSelect.appendChild(option);
        });
        edgePainterProfileSelect.value=normalizeEdgeProfile(edgePainterProfile);
        edgePainterProfileSelect.onchange=()=>setEdgePainterProfile(edgePainterProfileSelect.value);

        const edgePainterButton=document.createElement('button');
        edgePainterButton.type='button';
        edgePainterButton.className='lc-edge-manual-radius-action lc-edge-painter-action';
        edgePainterButton.setAttribute('aria-pressed','false');
        edgePainterButton.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 16h8l7-7-4-4-7 7v8H4v-4Zm9-9 4 4" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Paint Edges</span>';
        edgePainterButton.onclick=e=>{
          e.preventDefault();
          e.stopPropagation();
          if(manualRadiusActive)exitManualRadiusMode({commit:true});
          toggleLockedTool('edgePainter');
        };
        edgePainterRow.append(edgePainterProfileSelect,edgePainterButton);

        const edgeHint=document.createElement('div');
        edgeHint.className='lc-small lc-edge-designer-hint';
        edgeHint.textContent='Click a corner to toggle a 1" radius. Manual Radius allows custom values; Label Radius adds callouts; Paint Edges applies profiles canvas-wide.';

        const radiusActions=document.createElement('div');
        radiusActions.className='lc-edge-radius-actions';
        radiusActions.append(manualRadiusButton,radiusLabelButton);

        const edgeTools=document.createElement('div');
        edgeTools.className='lc-edge-tool-stack';
        edgeTools.append(radiusActions,edgePainterRow);

        inspectorEdgeMode.classList.add('lc-edge-display-row');
        edgeDesigner.append(edgeDiagram,edgeTools,inspectorEdgeMode,edgeHint);
        syncMomentaryToolUI();
      ...
      function drawNotes(){
        ...
        L.notes.forEach(note=>{
          ...
          const fmt=noteTextFormat(note);
          const t=document.createElementNS(svgNS,'text');
          t.setAttribute('x',i2p(note.x));
          t.setAttribute('y',i2p(note.y));
          t.setAttribute('font-size',String(fmt.size));
          t.setAttribute('font-weight',fmt.bold?'700':'400');
          t.setAttribute('font-style',fmt.italic?'italic':'normal');
          t.setAttribute('text-anchor',fmt.align==='center'?'middle':(fmt.align==='right'?'end':'start'));
          t.setAttribute('fill',state.selectedNoteId===note.id?'#2563eb':fmt.color);
          if(fmt.rotation){
            t.setAttribute('transform',`rotate(${fmt.rotation} ${i2p(note.x)} ${i2p(note.y)})`);
          }
          if(fmt.halo){
          ...
          g.addEventListener('pointerdown',e=>{
            ...
            svg.querySelectorAll('g[data-note-id]').forEach(group=>{
              const other=L.notes.find(candidate=>candidate.id===group.getAttribute('data-note-id'));
              const textEl=group.querySelector('text');
              if(textEl)textEl.setAttribute('fill',noteTextFormat(other).color);
            });
      ...
      const blockNonMomentaryInteraction=e=>{
        ...
        if(target instanceof Element){
          ...
          if(tool==='edgePainter' && target.closest('[data-edge-paint-hit="1"]'))return;
          ...
          const onCanvasEntity=target.closest(
            'g[data-id], g[data-note-id], [data-overlay-hit="1"], [data-splash-edge-hit="1"], [data-radius-corner-hit="1"], [data-edge-paint-hit="1"], [data-dim-id], [data-line-id]'
          );
          if(!onCanvasEntity){
            cancelPlacementTools({redraw:false});
            return;
      ...
      function activeToolCursor(){
        ...
        if(activeMomentaryTool()==='splash'||activeMomentaryTool()==='splashAll'||activeMomentaryTool()==='radius'||activeMomentaryTool()==='radiusAll'||activeMomentaryTool()==='edgePainter')return 'default';
      ...
      document.addEventListener('pointerdown', (e)=>{
        ...
        const note={
          id:uid(),
          x:round3(p2i(pt.x)),y:round3(p2i(pt.y)),
          text:'Note',
          fontSize:12,
          bold:false,
          italic:false,
      ...
      inCW.onchange = e => { if(currentPlan()){syncToolbarFromLayout();alert('Canvas size follows the floor plan. Crop or recalibrate the plan to change it.');return;} state.cw = Math.max(12, Number(e.target.value||0)); state.pieces.forEach(p=>{clampToCanvas(p);clampSlabPlacement(p);}); draw(); scheduleSave(); pushHistory(); syncTopBar?.(); };
      inCH.onchange = e => { if(currentPlan()){syncToolbarFromLayout();alert('Canvas size follows the floor plan. Crop or recalibrate the plan to change it.');return;} state.ch = Math.max(12, Number(e.target.value||0)); state.pieces.forEach(p=>{clampToCanvas(p);clampSlabPlacement(p);}); draw(); scheduleSave(); pushHistory(); syncTopBar?.(); };
      ...
      btnAdd.onclick = () => {
        ...
        const pieceW=40,pieceH=25.5;
        const normalX=Math.min(spawnInset,Math.max(0,state.cw-pieceW-spawnInset));
        const normalY=Math.min(spawnInset,Math.max(0,state.ch-pieceH-spawnInset));
        const safeMinX=state.cw>=pieceW+splashClearance*2?splashClearance:0;
        const safeMaxX=state.cw>=pieceW+splashClearance*2?state.cw-pieceW-splashClearance:Math.max(0,state.cw-pieceW);
        const safeMinY=state.ch>=pieceH+splashClearance*2?splashClearance:0;
        const safeMaxY=state.ch>=pieceH+splashClearance*2?state.ch-pieceH-splashClearance:Math.max(0,state.ch-pieceH);
        const hover=canvasHoverPoint&&state.workspace!=='slab'?canvasHoverPoint:null;
        const spawnX=hover?clamp(snap(hover.x-pieceW/2,grid),safeMinX,safeMaxX):normalX;
        const spawnY=hover?clamp(snap(hover.y-pieceH/2,grid),safeMinY,safeMaxY):normalY;
        const p = { 
          id: uid(), 
          name: `Piece ${idx+1}`, 
          w:pieceW, 
