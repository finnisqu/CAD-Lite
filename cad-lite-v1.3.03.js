   (function(){
    function init(){
      ...
      window.addEventListener('keydown', (e) => {
        ...
        if(k!=='d'&&k!=='s'&&k!=='l'&&k!=='n')return;
        state.dimTool=k==='d';state.seamTool=(k==='s'||k==='l');state.noteTool=k==='n';
        dimTempStart=null;seamTempStart=null;
        syncDimToolUI?.();syncNoteToolUI?.();syncSeamToolUI?.();
        e.preventDefault();
      });
    ...
    window.addEventListener('keydown', (e) => {
      ...
      function makeLayout(name){
        return { 
          ...
          seams: [],       // free canvas lines (legacy storage key)
          showManualDims: true  
        };
      }
      ...
      btnSeamTool.className='lc-btn ghost big'; btnSeamTool.type='button'; btnSeamTool.id='lc-line-tool';
      if(toolHost)toolHost.appendChild(btnSeamTool); else btnDimTool?.parentElement?.appendChild(btnSeamTool);
      function syncSeamToolUI(){const on=!!state.seamTool;btnSeamTool.textContent=on?'Line Tool: On':'Line Tool: Off';btnSeamTool.classList.toggle('alt',on);btnSeamTool.classList.toggle('ghost',!on);btnSeamTool.style.background=on?'#e5e7eb':'#fff';btnSeamTool.style.color='#111';syncCanvasToolCursor();}
  ...
  "pieces": [
    ...
    { "id": "ycid73l", "name": "Backsplash", "w": 4, "h": 22.5, "x": 159, "y": 10, "rotation": 0, "color": "#d5f0f0", "layer": 8, "rTL": false, "rTR": false, "rBL": false, "rBR": false, "sinks": [] }
      ...
      if(!seamList&&noteList?.closest('.lc-card')){const card=document.createElement('div');card.className='lc-card';const head=document.createElement('div');head.className='lc-card-head';const h3=document.createElement('h3');h3.textContent='Lines';head.appendChild(h3);seamList=document.createElement('div');seamList.id='lc-seam-list';seamList.className='lc-list';card.append(head,seamList);noteList.closest('.lc-card').after(card);}
      const freeLineTitle=seamList?.closest('.lc-card')?.querySelector('.lc-card-head h3');
      ...
      makeListCardToggle(seamList,'Lines');
  ...
  async function addRasterPNG() {
    ...
    sinksUI = initSinksCard({
      ...
      function drawToolPreview(){
        ...
        g.appendChild(svgEl('line',{x1:i2p(start.x),y1:i2p(start.y),x2:i2p(end.x),y2:i2p(end.y),stroke:toolPreview.mode==='seam'?'#111':'#2563eb','stroke-width':2,'stroke-dasharray':toolPreview.mode==='seam'?'none':'5 4','vector-effect':'non-scaling-stroke'}));
...
function renderSeamList(){
  ...
  const L=cur();
  seamList.innerHTML='';
  if(!L||!Array.isArray(L.seams)){renderEmptyState(seamList,'No lines');updateSidebarCounts();return;}

  L.seams.forEach((lineObj,idx)=>{
    const li=document.createElement('div');
    li.className='lc-item nav'+(state.selectedSeamId===lineObj.id?' selected':'');
    ...
    const title=document.createElement('div');
    title.textContent=(String(lineObj.name||'').trim()||`Line ${idx+1}`);
    title.style.whiteSpace='nowrap';
    title.style.overflow='hidden';
    title.style.textOverflow='ellipsis';
    title.title=title.textContent;
    ...
    const select=()=>{
      state.selectedSeamId=lineObj.id;
      state.selectedDimId=null;
      state.selectedNoteId=null;
      clearSelection();
      renderSeamList();
    ...
    rename.onclick=e=>{
      e.stopPropagation();
      beginListRename(title,lineObj,`Line ${idx+1}`,renderSeamList);
    };
    ...
    del.onclick=e=>{
      e.stopPropagation();
      L.seams=L.seams.filter(x=>x.id!==lineObj.id);
      if(state.selectedSeamId===lineObj.id)state.selectedSeamId=null;
      renderSeamList();
      draw();
      scheduleSave();
      pushHistory();
    ...
    if(state.selectedSeamId===lineObj.id){
      ...
      // Legacy free seams had no style property, so preserve their old dashed appearance.
      styleSel.value=lineObj.style==='solid'?'solid':'dashed';
      styleSel.onchange=()=>{
      ...
      controls.append(styleLab,colorLab,thickLab);
  ...
  renderEmptyState(seamList,'No lines');
  updateSidebarCounts();
}
  ...
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      function drawSeams(){
        ...
        L.seams.forEach(lineObj=>{
          const style=lineObj.style==='solid'?'solid':'dashed';
          ...
          const selected=state.selectedSeamId===lineObj.id;

          const g=document.createElementNS(svgNS,'g');
          g.setAttribute('data-seam-id',lineObj.id);
          g.style.cursor=activeToolCursor()||'pointer';

          const hit=svgEl('line',{
            x1:i2p(lineObj.x1),y1:i2p(lineObj.y1),x2:i2p(lineObj.x2),y2:i2p(lineObj.y2),
          ...
          g.addEventListener('pointerdown',ev=>{
            ...
            state.selectedSeamId=lineObj.id;state.selectedDimId=null;state.selectedNoteId=null;clearSelection();renderSeamList();renderDimList();renderNoteList();draw();
          ...
          if(selected){
            const handle=(which,x,y)=>{
              const h=svgEl('circle',{cx:i2p(x),cy:i2p(y),r:5,fill:'#fff',stroke:'#2563eb','stroke-width':2,'vector-effect':'non-scaling-stroke'});
              ...
              h.addEventListener('pointerdown',ev=>{
                ...
                const move=mv=>{
                  const q=svgPoint(mv);
                  let sn=snapSeamPoint({x:p2i(q.x),y:p2i(q.y)});
                ...
                const up=()=>{
                  ...
                  document.removeEventListener('pointerup',up,true);
                  document.removeEventListener('pointercancel',up,true);
                  renderSeamList();scheduleSave();pushHistory();
                };
                document.addEventListener('pointermove',move,true);
                document.addEventListener('pointerup',up,true);
                document.addEventListener('pointercancel',up,true);
            ...
            handle(1,lineObj.x1,lineObj.y1);
            handle(2,lineObj.x2,lineObj.y2);
          }
      ...
      svg.addEventListener('click',e=>{
        ...
        if(!seamTempStart){seamTempStart=snapped;state.selectedSeamId=null;}
          ...
          const seam={id:uid(),x1:seamTempStart.x,y1:seamTempStart.y,x2:end.x,y2:end.y,style:'solid',color:'#111111',thickness:2};
