   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      function syncNoteToolUI(){btnNoteTool.textContent=state.noteTool?'Note Tool: On':'Note Tool: Off';btnNoteTool.classList.toggle('alt',!!state.noteTool);btnNoteTool.classList.toggle('ghost',!state.noteTool);if(svg)svg.style.cursor=state.noteTool?'text':(state.dimTool?'crosshair':'');}
      btnNoteTool.onclick=()=>{const turningOn=!state.noteTool;state.noteTool=turningOn;if(turningOn&&state.dimTool){state.dimTool=false;syncDimToolUI?.();}syncNoteToolUI();}; syncNoteToolUI();
  ...
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      btnDimTool && (btnDimTool.onclick = () => {
        const turningOn = !state.dimTool;
        state.dimTool = turningOn;
        if (turningOn && state.noteTool) {
          state.noteTool = false;
          syncNoteToolUI();
        }
        if (!state.dimTool) dimTempStart = null;
        syncDimToolUI();
      });
