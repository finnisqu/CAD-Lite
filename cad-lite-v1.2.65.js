  ...
  if (state.overlay && (state.overlay.dataURL || state.overlay.visible)){
    ...
    delete state.overlay;
      ...
      function pieceItem(p, i){
        ...
        // Double-click the piece label to rename it inline.
        line.title='Double-click to rename';
        line.addEventListener('dblclick',(e)=>{
          e.preventDefault();e.stopPropagation();
          const input=document.createElement('input');
          input.className='lc-input';input.value=p.name||'Piece';input.style.width='100%';
          line.replaceWith(input);input.focus();input.select();
          let finished=false;
          const finish=(save)=>{
            if(finished)return;finished=true;
            if(save){const v=input.value.trim();if(v)p.name=v;scheduleSave();pushHistory();}
            renderList();draw();updateInspector();
          };
