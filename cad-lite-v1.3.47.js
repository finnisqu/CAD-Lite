   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      addToolbarMenuAction(quickLayoutsMenu,'Galley Kitchen',()=>insertQuickLayout('galley'),{icon:ICON_MENU.piece});
      addToolbarMenuAction(quickLayoutsMenu,'L-Kitchen',()=>insertQuickLayout('lKitchen'),{icon:ICON_MENU.piece});
      addToolbarMenuAction(quickLayoutsMenu,'U-Kitchen',()=>insertQuickLayout('uKitchen'),{icon:ICON_MENU.piece});
      addToolbarMenuAction(quickLayoutsMenu,'Kitchen Island',()=>insertQuickLayout('island'),{icon:ICON_MENU.piece});
      addToolbarMenuSeparator(quickLayoutsMenu);
      addToolbarMenuAction(quickLayoutsMenu,'Single Vanity',()=>insertQuickLayout('singleVanity'),{icon:ICON_MENU.piece});
      addToolbarMenuAction(quickLayoutsMenu,'Double Vanity',()=>insertQuickLayout('doubleVanity'),{icon:ICON_MENU.piece});
      addToolbarMenuSeparator(insertDropdown.menu);
      ...
      addToolbarMenuAction(exportDropdown.menu,'Current Layout PDF',()=>btnExportPDF?.click(),{icon:ICON_MENU.pdf});
      addToolbarMenuAction(exportDropdown.menu,'All Layouts PDF',()=>btnExportPDFAll?.click(),{icon:ICON_MENU.pdf});
        ...
        function render(){
          ...
          if ((piece.sinks?.length || 0) > 0 && piece.sinks.length < MAX_SINKS_PER_PIECE) {
            ...
            add.onclick = () => {
              ...
              openIndex.set(piece.id,piece.sinks.length-1);
              onStateChange?.();
            };
          ...
          const open = openIndex.has(piece.id) ? openIndex.get(piece.id) : -1;
          ...
          piece.sinks.forEach((sink, idx)=>{
            ...
            row.addEventListener('click', (e)=>{
              if(e.target.closest('button')) return;
              if(open===idx)openIndex.delete(piece.id);
              else openIndex.set(piece.id,idx);
              render();
            });
            list.appendChild(row);
...
function installSidebarReorder(listEl,{rowSelector='.lc-item.nav',startSelector=null,getArray,onMoved}={}){
  ...
  listEl.addEventListener('pointerup',()=>{
    ...
    const rows=Array.from(listEl.querySelectorAll(rowSelector));
    const after=marker.nextElementSibling&&marker.nextElementSibling.matches?.(rowSelector)
...
function renderLayouts(){
  ...
  installSidebarReorder(layoutsEl,{
    ...
    startSelector:'.lc-layout-title-row',
    getArray:()=>{
