   (function(){
    function init(){
      ...
      function syncMomentaryToolUI(){
        ...
        root.classList.toggle('lc-momentary-splash',tool==='splash'||tool==='splashAll');
          ...
          if(overlaySelected){
            ...
            if(originals.length){
              ...
              originals.forEach(p=>{
                ...
                np.name=sourceWasSplash?'Piece Copy':(p.name||'Piece')+' Copy';
                if(sourceWasSplash){
                  np.tags=Array.isArray(np.tags)?np.tags.filter(tag=>tag!=='backsplash'):[];
                  delete np.pieceType; delete np.splashKind; delete np.splashHeight; delete np.attachment;
                }
    ...
    window.addEventListener('keydown', (e) => {
      ...
      const ICON_MENU={
        ...
        starter:'M5 5h14v14H5zM8 8h8v8H8z',
      ...
      addToolbarMenuAction(insertDropdown.menu,'Note',()=>activateNoteTool(),{icon:ICON_MENU.note,shortcut:'N'});
      ...
      addToolbarMenuAction(importDropdown.menu,'Load Starter Layout',()=>btnLoadStarter?.click(),{icon:ICON_MENU.starter});
        ...
        function snapshotState(){
          try{
            return JSON.stringify({
              ...
              showGrid: !!state.showGrid, showDims: !!state.showDims, showSplashDims: !!state.showSplashDims, showSinkCenterlines: !!state.showSinkCenterlines, showManualDims: !!state.showManualDims, showEdgeProfiles: !!state.showEdgeProfiles, showPieceFills: !!state.showPieceFills, showNotes: !!state.showNotes, showLines: !!state.showLines, showLabels: !!state.showLabels, showSplashLabels: !!state.showSplashLabels, showLabelDims: !!state.showLabelDims, dimPrecision: state.dimPrecision, dimFormat: state.dimFormat,
  ...
  if (state.overlay && (state.overlay.dataURL || state.overlay.visible)){
    ...
    delete state.overlay;
      ...
      function drawOverlayInteractionLayer(){
        ...
        if(!L||L.showOverlays===false||!arr.length)return;
      ...
      function draw(){
        ...
        sorted.forEach((p)=>{
         ...
         // append rotated geometry group to a piece container g (for pointerdown)
          ...
          // Splash-tagged pieces have independent label visibility.
          const showThisPieceLabel=isBacksplashPiece(p)?!!state.showSplashLabels:!!state.showLabels;
          ...
          const showThisPieceDims=isBacksplashPiece(p)?!!state.showSplashDims:!!state.showDims;
  ...
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      function updateInspector(){
        ...
        pieceBody.appendChild(splashTagRow);
    ...
    del.innerHTML='<svg class="lc-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l1 2h4v2H3V5h4l1-2Zm-2 6h12l-1 11H7L6 9Zm3 2v7h2v-7H9Zm4 0v7h2v-7h-2Z" fill="currentColor"/></svg>';
      ...
      function duplicatePiece(p){
        ...
        const rs = realSize(p);
        const sourceWasSplash=isBacksplashPiece(p);
        const d={...p, pieceSeams:Array.isArray(p.pieceSeams)?p.pieceSeams.map(ps=>({...ps,id:uid()})):[], id: uid(), name: sourceWasSplash?'Piece Copy':p.name+' Copy', x:clamp(p.x+state.grid,0,state.cw-rs.w), y:clamp(p.y+state.grid,0,state.ch-rs.h), layer:top};
        if(sourceWasSplash){
          d.tags=Array.isArray(d.tags)?d.tags.filter(tag=>tag!=='backsplash'):[];
          delete d.pieceType; delete d.splashKind; delete d.splashHeight; delete d.attachment;
        }
        state.pieces.push(d); 
