...
function renderDimList(){if(!dimList)return;const L=cur();dimList.innerHTML='';if(!L||!Array.isArray(L.dims)){renderEmptyState(dimList,'No dimensions');updateSidebarCounts();return;}L.dims.forEach((d,idx)=>{const li=document.createElement('div');li.className='lc-item nav'+(state.selectedDimId===d.id?' selected':'');li.style.display='flex';li.style.alignItems='center';li.style.gap='8px';const dx=d.x2-d.x1,dy=d.y2-d.y1,dist=Math.sqrt(dx*dx+dy*dy),ang=(Math.atan2(dy,dx)*180/Math.PI+360)%180;const label=document.createElement('div');label.style.flex='1';label.textContent=`Dim ${idx+1}: ${dist.toFixed(2)}" (${ang<45||ang>135?'Horiz':'Vert'})`;label.onclick=()=>{state.selectedDimId=d.id;state.selectedId=null;renderDimList();updateInspector();draw();};const del=document.createElement('button');del.type='button';del.className='lc-btn ghost';del.title='Delete dimension';del.textContent='🗑';del.onclick=e=>{e.stopPropagation();L.dims=L.dims.filter(x=>x.id!==d.id);if(state.selectedDimId===d.id)state.selectedDimId=null;renderDimList();draw();scheduleSave();pushHistory();};li.append(label,del);dimList.appendChild(li);});renderEmptyState(dimList,'No dimensions');updateSidebarCounts();}
