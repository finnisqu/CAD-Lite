   (function(){
    function init(){
      ...
      function selectOnly(id){ state.selectedDimId=null; state.selectedLineId=null; state.selectedNoteId=null; setSelection([id]); }
  ...
  "pieces": [
    ...
    { "id": "ycid73l", "name": "Backsplash", "w": 4, "h": 22.5, "x": 159, "y": 10, "rotation": 0, "color": "#d5f0f0", "layer": 8, "rTL": false, "rTR": false, "rBL": false, "rBR": false, "sinks": [] }
      ...
      makeListCardToggle(lineList,'Lines');
...
function renderLineList(){
  ...
  L.lines.forEach((lineObj,idx)=>{
    ...
    if(state.selectedLineId===lineObj.id){
      ...
      // Lines without explicit style data default to dashed.
      ...
      color.oninput=()=>{
        ...
        const visible=svg.querySelector(`g[data-line-id="${lineObj.id}"] line.lc-free-line-visible`);
    ...
    pushHistory();
      ...
      function drawLines(){
        ...
        L.lines.forEach(lineObj=>{
          ...
          g.addEventListener('pointerdown',ev=>{
            ...
            state.selectedLineId=lineObj.id;state.selectedDimId=null;state.selectedNoteId=null;clearSelection();renderLineList();renderDimList();renderNoteList();draw();
      ...
      svg.addEventListener('click',e=>{
        ...
        if(!lineTempStart){lineTempStart=snapped;state.selectedLineId=null;}
          ...
          const lineObj={id:uid(),x1:lineTempStart.x,y1:lineTempStart.y,x2:end.x,y2:end.y,style:'solid',color:'#111111',thickness:2};
          L.lines.push(lineObj);state.selectedLineId=lineObj.id;state.selectedDimId=null;state.selectedNoteId=null;clearSelection();lineTempStart=null;toolPreview=null;state.lineTool=false;syncLineToolUI();draw();renderLineList();scheduleSave();pushHistory();
        }
        e.stopPropagation();
      });
      ...
      window.addEventListener('keydown',e=>{
        ...
        L.lines.splice(idx,1);state.selectedLineId=null;draw();renderLineList();scheduleSave();pushHistory();e.preventDefault();
      });
      ...
      btnClearSel && (btnClearSel.onclick = ()=>{
        clearSelection();
        state.selectedDimId=null; state.selectedLineId=null; state.selectedNoteId=null;
        renderList(); renderDimList(); renderLineList(); renderNoteList(); updateInspector(); sinksUI?.refresh(); draw();
      });
