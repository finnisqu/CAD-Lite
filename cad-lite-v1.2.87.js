        ...
        function snapshotState(){
          try{
            return JSON.stringify({
              ...
              showGrid: !!state.showGrid, showDims: !!state.showDims, showManualDims: !!state.showManualDims, showEdgeProfiles: !!state.showEdgeProfiles, showNotes: !!state.showNotes, showLabels: !!state.showLabels, showLabelDims: !!state.showLabelDims, dimPrecision: state.dimPrecision,
        ...
        if (L && Array.isArray(L.dims) && L.dims.length && state.showManualDims) {
          ...
          const offPx = 12; // perpendicular offset in px from the measured segment
            ...
            L.dims.forEach(d => {
              ...
              // Keep normal dimensions centered. If the segment is too short to
              // comfortably contain its text, move the label beyond endpoint 2.
              // This changes display position only; geometry remains untouched.
              const estimatedLabelWidthPx = Math.max(28, label.length * 8);
              const SHORT_DIM_PAD_PX = 16;
              const shortDim = lenPx < estimatedLabelWidthPx + SHORT_DIM_PAD_PX;
              const alongPx = shortDim ? (lenPx / 2 + estimatedLabelWidthPx / 2 + 8) : 0;
              const lx = midx + ox + nx * LABEL_GAP_PX * labelSide + ux * alongPx;
              const ly = midy + oy + ny * LABEL_GAP_PX * labelSide + uy * alongPx;
