   (function(){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      // Put the note tool in the visible 2x2 toggle grid, replacing the unused spacer.
      const toggleGrid = togGrid?.closest('.lc-2x2');
      const toggleSpacer = toggleGrid?.querySelector('.lc-ctl');
      if (toggleSpacer) toggleSpacer.replaceWith(btnNoteTool);
      else if (toggleGrid) toggleGrid.appendChild(btnNoteTool);
      else btnDimTool?.parentElement?.insertBefore(btnNoteTool, btnDimTool);
      function syncNoteToolUI(){btnNoteTool.textContent=state.noteTool?'Add Note: Click Canvas':'Add Note';btnNoteTool.classList.toggle('alt',!!state.noteTool);btnNoteTool.classList.toggle('ghost',!state.noteTool);}
      btnNoteTool.onclick=()=>{state.noteTool=!state.noteTool;if(state.noteTool&&state.dimTool){state.dimTool=false;syncDimToolUI?.();}syncNoteToolUI();}; syncNoteToolUI();
      ...
      const dimToolCtl = btnDimTool?.closest('.lc-ctl');
      if (dimToolCtl) dimToolCtl.appendChild(btnPieceSnap);
...
if(btnAddLayout){
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      function drawNotes(){
        const L=cur(); if(!L||!Array.isArray(L.notes)) return;
        L.notes.forEach(note=>{
          const g=document.createElementNS(svgNS,'g'); g.setAttribute('data-note-id',note.id); g.style.cursor='move';
          const t=document.createElementNS(svgNS,'text'); t.setAttribute('x',i2p(note.x));t.setAttribute('y',i2p(note.y));t.setAttribute('font-size','13');t.setAttribute('font-weight','600');t.setAttribute('fill','#111');t.setAttribute('paint-order','stroke');t.setAttribute('stroke','#fff');t.setAttribute('stroke-width','3');t.textContent=note.text||'Note';g.appendChild(t);
          t.addEventListener('dblclick',e=>{e.preventDefault();e.stopPropagation();openInlineDimEditor(t,note.text||'',raw=>{const v=String(raw??'').trim();if(!v)return false;note.text=v;draw();scheduleSave();pushHistory();return true;});});
          g.addEventListener('pointerdown',e=>{
            if(state.noteTool)return;e.preventDefault();e.stopPropagation();
            const p0=svgPoint(e),sx=p2i(p0.x),sy=p2i(p0.y),nx=note.x,ny=note.y;
            const move=ev=>{const q=svgPoint(ev);note.x=clamp(nx+p2i(q.x)-sx,0,state.cw);note.y=clamp(ny+p2i(q.y)-sy,0,state.ch);draw();};
            const up=()=>{svg.removeEventListener('pointermove',move);svg.removeEventListener('pointerup',up);svg.removeEventListener('pointercancel',up);scheduleSave();pushHistory();};
      ...
      if (state.noteTool) {
        if (e.target.closest('g[data-id]') || e.target.closest('g[data-note-id]')) return;
        e.preventDefault(); e.stopPropagation();
        const pt=svgPoint(e), note={id:uid(),x:round3(p2i(pt.x)),y:round3(p2i(pt.y)),text:'Note'};
        cur().notes.push(note); state.noteTool=false; syncNoteToolUI(); draw(); scheduleSave(); pushHistory();
        requestAnimationFrame(()=>{const el=svg.querySelector(`g[data-note-id="${note.id}"] text`);if(el)openInlineDimEditor(el,'',raw=>{const v=String(raw??'').trim();if(!v){cur().notes=cur().notes.filter(n=>n.id!==note.id);draw();scheduleSave();return true;}note.text=v;draw();scheduleSave();pushHistory();return true;});});
        blankDown=null; return;
      }
