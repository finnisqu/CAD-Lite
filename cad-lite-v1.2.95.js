   (function(){
    function init(){
      ...
      function selectOnly(id){ state.selectedDimId=null; setSelection([id]); }
      ...
      window.addEventListener('keydown', (e) => {
        ...
        if(tag==='input'||tag==='textarea'||tag==='select'||(ael&&ael.isContentEditable))return;
        if(e.key!=='Delete'&&e.key!=='Backspace')return;
        // Let selected dimensions/seams use their own delete handlers.
        if(state.selectedDimId||state.selectedSeamId)return;
      ...
      window.addEventListener('keydown', (e) => {
        ...
        if(e.key==='Escape'){
          const hadTool=state.dimTool||state.noteTool||state.seamTool;
          state.dimTool=false;state.noteTool=false;state.seamTool=false;
          dimTempStart=null;seamTempStart=null;
          syncDimToolUI?.();syncNoteToolUI?.();syncSeamToolUI?.();
          if(!hadTool){
            clearSelection();state.selectedDimId=null;state.selectedSeamId=null;
            renderList();renderDimList();renderNoteList();renderSeamList();updateInspector();sinksUI?.refresh?.();draw();
          }
          e.preventDefault();return;
        }

        const k=(e.key||'').toLowerCase();
        if(k!=='d'&&k!=='s'&&k!=='n')return;
        state.dimTool=k==='d';state.seamTool=k==='s';state.noteTool=k==='n';
        dimTempStart=null;seamTempStart=null;
        syncDimToolUI?.();syncNoteToolUI?.();syncSeamToolUI?.();
        e.preventDefault();
      });

      // Ctrl/Cmd+D duplicates selected countertop piece(s).
      window.addEventListener('keydown',(e)=>{
        ...
        const ids=state.selectedIds.length?[...state.selectedIds]:(state.selectedId?[state.selectedId]:[]);
        if(!ids.length)return;
        const originals=state.pieces.filter(p=>ids.includes(p.id));if(!originals.length)return;
        const newIds=[];
        originals.forEach(p=>{
          ...
          state.pieces.push(np);newIds.push(np.id);
        });
        setSelection(newIds);state.lastSelIndex=state.pieces.length-1;
        renderList();updateInspector();sinksUI?.refresh?.();draw();scheduleSave();pushHistory();
        e.preventDefault();
      });
    ...
    window.addEventListener('keydown', (e) => {
      ...
      const cur = () => {
        ...
        if (!Array.isArray(L.seams)) L.seams = [];
        return L;
      };
      ...
      btnNoteTool.onclick=()=>{state.noteTool=!state.noteTool;if(state.noteTool){state.dimTool=false;state.seamTool=false;syncDimToolUI?.();syncSeamToolUI?.();}syncNoteToolUI();}; syncNoteToolUI();
      const btnSeamTool=document.createElement('button');
      btnSeamTool.className='lc-btn ghost big'; btnSeamTool.type='button'; btnSeamTool.id='lc-seam-tool';
      if(toolHost)toolHost.appendChild(btnSeamTool); else btnDimTool?.parentElement?.appendChild(btnSeamTool);
      function syncSeamToolUI(){const on=!!state.seamTool;btnSeamTool.textContent=on?'Seam Tool: On':'Seam Tool: Off';btnSeamTool.classList.toggle('alt',on);btnSeamTool.classList.toggle('ghost',!on);btnSeamTool.style.background=on?'#e5e7eb':'#fff';btnSeamTool.style.color='#111';syncCanvasToolCursor();}
      btnSeamTool.onclick=()=>{state.seamTool=!state.seamTool;if(state.seamTool){state.dimTool=false;state.noteTool=false;syncDimToolUI?.();syncNoteToolUI();}if(!state.seamTool)seamTempStart=null;syncSeamToolUI();}; syncSeamToolUI();

      const btnShowNotes=document.createElement('button');
      btnShowNotes.className='lc-btn alt big';btnShowNotes.type='button';btnShowNotes.id='lc-toggle-notes';
      const showNotesHost=togGrid?.closest('.lc-2x2');
      if(showNotesHost)showNotesHost.appendChild(btnShowNotes);
      function syncShowNotesUI(){btnShowNotes.textContent=state.showNotes?'Hide Notes':'Show Notes';btnShowNotes.classList.toggle('alt',!!state.showNotes);btnShowNotes.classList.toggle('ghost',!state.showNotes);}
      btnShowNotes.onclick=()=>{state.showNotes=!state.showNotes;syncShowNotesUI();draw();scheduleSave?.();};
      syncShowNotesUI();
      const btnLabelDims=document.createElement('button');
      btnLabelDims.className='lc-btn alt big';btnLabelDims.type='button';btnLabelDims.id='lc-toggle-label-dims';
      if(showNotesHost)showNotesHost.appendChild(btnLabelDims);
      function syncLabelDimsUI(){btnLabelDims.textContent=state.showLabelDims?'Hide Label Dims':'Show Label Dims';btnLabelDims.classList.toggle('alt',!!state.showLabelDims);btnLabelDims.classList.toggle('ghost',!state.showLabelDims);}
      btnLabelDims.onclick=()=>{state.showLabelDims=!state.showLabelDims;syncLabelDimsUI();draw();scheduleSave?.();pushHistory();};
      syncLabelDimsUI();
      const formatWrap=document.createElement('div');
      formatWrap.className='lc-ctl';
      const formatLabel=document.createElement('label');
      formatLabel.textContent='Dim Format';
      const formatSelect=document.createElement('select');
      formatSelect.id='lc-dim-format';
      [['fraction','Fraction'],['decimal','Decimal']].forEach(([value,label])=>{const o=document.createElement('option');o.value=value;o.textContent=label;formatSelect.appendChild(o);});
      formatSelect.value=state.dimFormat;
      formatSelect.onchange=()=>{state.dimFormat=formatSelect.value==='decimal'?'decimal':'fraction';draw();scheduleSave?.();pushHistory();};
      formatLabel.appendChild(formatSelect);formatWrap.appendChild(formatLabel);
      if(showNotesHost)showNotesHost.appendChild(formatWrap);
      const precisionWrap=document.createElement('div');
      precisionWrap.className='lc-ctl';
      const precisionLabel=document.createElement('label');
      precisionLabel.textContent='Dim Precision';
      const precisionSelect=document.createElement('select');
      precisionSelect.id='lc-dim-precision';
      [[1,'1"'],[2,'1/2"'],[4,'1/4"'],[8,'1/8"'],[16,'1/16"']].forEach(([value,label])=>{const o=document.createElement('option');o.value=String(value);o.textContent=label;precisionSelect.appendChild(o);});
      precisionSelect.value=String(state.dimPrecision);
      precisionSelect.onchange=()=>{state.dimPrecision=Number(precisionSelect.value)||16;draw();scheduleSave?.();pushHistory();};
      precisionLabel.appendChild(precisionSelect);precisionWrap.appendChild(precisionLabel);
      if(showNotesHost)showNotesHost.appendChild(precisionWrap);
      const btnPieceSnap = document.createElement('button');
      btnPieceSnap.className = 'lc-btn alt big';
      btnPieceSnap.type = 'button';
      btnPieceSnap.id = 'lc-piece-snap';
      const pieceSnapHost=togGrid?.closest('.lc-2x2');
      if(pieceSnapHost)pieceSnapHost.appendChild(btnPieceSnap);
      function syncPieceSnapUI(){
        btnPieceSnap.textContent = state.pieceSnap ? 'Piece Snap: On' : 'Piece Snap: Off';
        ...
        function snapshotState(){
          try{
            return JSON.stringify({
              ...
              showGrid: !!state.showGrid, showDims: !!state.showDims, showManualDims: !!state.showManualDims, showEdgeProfiles: !!state.showEdgeProfiles, showNotes: !!state.showNotes, showLabels: !!state.showLabels, showLabelDims: !!state.showLabelDims, dimPrecision: state.dimPrecision, dimFormat: state.dimFormat,
  ...
  "pieces": [
    ...
    { "id": "ycid73l", "name": "Backsplash", "w": 4, "h": 22.5, "x": 159, "y": 10, "rotation": 0, "color": "#d5f0f0", "layer": 8, "rTL": false, "rTR": false, "rBL": false, "rBR": false, "sinks": [] }
      ...
      let seamList=document.getElementById('lc-seam-list');
      if(!seamList&&noteList?.closest('.lc-card')){const card=document.createElement('div');card.className='lc-card';const head=document.createElement('div');head.className='lc-card-head';const h3=document.createElement('h3');h3.textContent='Seams';head.appendChild(h3);seamList=document.createElement('div');seamList.id='lc-seam-list';seamList.className='lc-list';card.append(head,seamList);noteList.closest('.lc-card').after(card);}
      // Sidebar order: Notes, Dimensions, Seams
      const noteCard=noteList?.closest('.lc-card'),dimCard=dimList?.closest('.lc-card'),seamCard=seamList?.closest('.lc-card');
      if(noteCard&&dimCard&&seamCard){dimCard.before(noteCard);dimCard.after(seamCard);}
      function makeListCardToggle(listEl,label){
        ...
        const title=head.querySelector('h3'); if(title){title.dataset.baseLabel=label;}
        const arrow=document.createElement('span');arrow.textContent='▸';arrow.style.marginLeft='auto';arrow.style.fontSize='14px';arrow.setAttribute('aria-hidden','true');
        head.style.display='flex';head.style.alignItems='center';head.appendChild(arrow);
        let open=false;listEl.style.display='none';
        const sync=()=>{listEl.style.display=open?'':'none';arrow.textContent=open?'▾':'▸';head.setAttribute('aria-expanded',String(open));};
        head.addEventListener('click',e=>{if(e.target.closest('button,input,select,textarea'))return;open=!open;sync();});
        head.setAttribute('role','button');head.setAttribute('tabindex','0');
        head.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open=!open;sync();}});
        sync();
      }
      makeListCardToggle(dimList,'Dimensions');
      makeListCardToggle(noteList,'Notes');
      makeListCardToggle(seamList,'Seams');
      // Pieces uses a dedicated full-header toggle because its header also contains Add Piece.
      ...
      if(piecesCard&&piecesHead&&!piecesHead.dataset.toggleReady){
        if(piecesHead.querySelector('h3'))piecesHead.querySelector('h3').dataset.baseLabel='Pieces';
        piecesHead.dataset.toggleReady='1';
        piecesHead.style.cursor='pointer';piecesHead.style.userSelect='none';
        ...
        piecesArrow.textContent='▸';piecesArrow.style.marginLeft='auto';piecesArrow.style.fontSize='14px';piecesArrow.style.flex='0 0 auto';piecesArrow.setAttribute('aria-hidden','true');
        // Order: Pieces title, Add Piece, arrow. Arrow remains at far right like the other lists.
        if(addPieceBtn){
          addPieceBtn.style.marginLeft='auto';
          addPieceBtn.insertAdjacentElement('afterend',piecesArrow);
          piecesArrow.style.marginLeft='8px';
        }else piecesHead.appendChild(piecesArrow);
        let piecesOpen=false;
        const syncPieces=()=>{list.style.display=piecesOpen?'':'none';piecesArrow.textContent=piecesOpen?'▾':'▸';piecesHead.setAttribute('aria-expanded',String(piecesOpen));};
        piecesHead.addEventListener('click',e=>{
          if(e.target.closest('#lc-add,input,select,textarea'))return;
          piecesOpen=!piecesOpen;syncPieces();
        });
        piecesHead.setAttribute('role','button');piecesHead.setAttribute('tabindex','0');
        piecesHead.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();piecesOpen=!piecesOpen;syncPieces();}});
        syncPieces();
      }
  ...
  async function addRasterPNG() {
    ...
    sinksUI = initSinksCard({
      ...
      function previewSnapInfo(raw,mode){
        const snapped=mode==='seam'?snapSeamPoint(raw):snapDimPoint(raw);
      ...
      function drawToolPreview(){
        ...
        g.appendChild(svgEl('line',{x1:i2p(start.x),y1:i2p(start.y),x2:i2p(end.x),y2:i2p(end.y),stroke:toolPreview.mode==='seam'?'#111':'#2563eb','stroke-width':2,'stroke-dasharray':toolPreview.mode==='seam'?'8 4':'5 4','vector-effect':'non-scaling-stroke'}));
  ...
  if (state.overlay && (state.overlay.dataURL || state.overlay.visible)){
    ...
    delete state.overlay;
      ...
      function enablePieceDimEdit(textEl, piece, prop, promptLabel){
        ...
        textEl.addEventListener('pointerdown', e => { if(!(state.dimTool||state.seamTool||state.noteTool)) e.stopPropagation(); });
      ...
      function draw(){
        ...
        sorted.forEach((p)=>{
         ...
         // append rotated geometry group to a piece container g (for pointerdown)
          ...
          if(!(state.dimTool||state.seamTool||state.noteTool))gPiece.style.cursor='move';
          ...
          if (state.showEdgeProfiles && p.edgeProfiles) {
            ...
            const MARGIN = 16; // px inside from piece edge
          ...
          gPiece.addEventListener('pointerdown', (e)=>{
            if(state.seamTool || state.dimTool || state.noteTool) return; // active drawing tool owns the canvas
...
function updateSidebarCounts(){
  ...
  setCount(seamList,Array.isArray(L?.seams)?L.seams.length:0);
}
...
function renderDimList(){if(!dimList)return;const L=cur();dimList.innerHTML='';if(!L||!Array.isArray(L.dims)){renderEmptyState(dimList,'No dimensions');updateSidebarCounts();return;}L.dims.forEach((d,idx)=>{const li=document.createElement('div');li.className='lc-item nav'+(state.selectedDimId===d.id?' selected':'');li.style.display='flex';li.style.alignItems='center';li.style.gap='8px';li.style.cursor='pointer';const dx=d.x2-d.x1,dy=d.y2-d.y1,dist=Math.sqrt(dx*dx+dy*dy),ang=(Math.atan2(dy,dx)*180/Math.PI+360)%180;const orient=ang<=3||ang>=177?'Horizontal':(Math.abs(ang-90)<=3?'Vertical':`${ang.toFixed(1)}°`);const label=document.createElement('div');label.style.flex='1';label.innerHTML=`<div>Dim ${idx+1}</div><div style="font-size:12px;opacity:.7">${fmtCanvasInches(dist)} · ${orient}</div>`;const select=()=>{state.selectedDimId=d.id;state.selectedSeamId=null;state.selectedId=null;renderDimList();renderSeamList();updateInspector();draw();};li.onclick=e=>{if(e.target.closest('button'))return;select();};const del=document.createElement('button');del.type='button';del.className='lc-btn ghost';del.title='Delete dimension';del.textContent='🗑';del.onclick=e=>{e.stopPropagation();L.dims=L.dims.filter(x=>x.id!==d.id);if(state.selectedDimId===d.id)state.selectedDimId=null;renderDimList();draw();scheduleSave();pushHistory();};li.append(label,del);dimList.appendChild(li);});renderEmptyState(dimList,'No dimensions');updateSidebarCounts();}

function renderSeamList(){if(!seamList)return;const L=cur();seamList.innerHTML='';if(!L||!Array.isArray(L.seams)){renderEmptyState(seamList,'No seams');updateSidebarCounts();return;}L.seams.forEach((s,idx)=>{const li=document.createElement('div');li.className='lc-item nav'+(state.selectedSeamId===s.id?' selected':'');li.style.display='flex';li.style.alignItems='center';li.style.gap='8px';li.style.cursor='pointer';const dx=s.x2-s.x1,dy=s.y2-s.y1,len=Math.sqrt(dx*dx+dy*dy),ang=(Math.atan2(dy,dx)*180/Math.PI+360)%180;const orient=ang<=3||ang>=177?'Horizontal':(Math.abs(ang-90)<=3?'Vertical':`${ang.toFixed(1)}°`);const label=document.createElement('div');label.style.flex='1';label.innerHTML=`<div>Seam ${idx+1}</div><div style="font-size:12px;opacity:.7">${fmtCanvasInches(len)} · ${orient}</div>`;const select=()=>{state.selectedSeamId=s.id;state.selectedDimId=null;clearSelection();renderSeamList();draw();};li.onclick=e=>{if(e.target.closest('button'))return;select();};const del=document.createElement('button');del.type='button';del.className='lc-btn ghost';del.title='Delete seam';del.textContent='🗑';del.onclick=e=>{e.stopPropagation();L.seams=L.seams.filter(x=>x.id!==s.id);if(state.selectedSeamId===s.id)state.selectedSeamId=null;renderSeamList();draw();scheduleSave();pushHistory();};li.append(label,del);seamList.appendChild(li);});renderEmptyState(seamList,'No seams');updateSidebarCounts();}

function renderNoteList(){if(!noteList)return;const L=cur();noteList.innerHTML='';if(!L||!Array.isArray(L.notes)){renderEmptyState(noteList,'No notes');updateSidebarCounts();return;}L.notes.forEach((n,idx)=>{const li=document.createElement('div');li.className='lc-item nav';li.style.display='flex';li.style.alignItems='center';li.style.gap='8px';const label=document.createElement('div');label.style.flex='1';label.style.whiteSpace='pre-wrap';label.textContent=n.text||`Note ${idx+1}`;label.onclick=()=>{const el=svg.querySelector(`g[data-note-id="${n.id}"] text`);if(el)openInlineNoteEditor(el,n.text||'',raw=>{const v=String(raw??'').trim();if(!v)return false;n.text=v;draw();renderNoteList();scheduleSave();pushHistory();return true;});};const del=document.createElement('button');del.type='button';del.className='lc-btn ghost';del.title='Delete note';del.textContent='🗑';del.onclick=e=>{e.stopPropagation();L.notes=L.notes.filter(x=>x.id!==n.id);draw();renderNoteList();scheduleSave();pushHistory();};li.append(label,del);noteList.appendChild(li);});renderEmptyState(noteList,'No notes');updateSidebarCounts();}
...
if(btnAddLayout){
  btnAddLayout.onclick = ()=>{
    ...
    pushHistory();
      ...
      function drawSeams(){
        const L=cur(); if(!L||!Array.isArray(L.seams)) return;
        L.seams.forEach(seam=>{
          const g=document.createElementNS(svgNS,'g');g.setAttribute('data-seam-id',seam.id);g.style.cursor=activeToolCursor()||'pointer';
          const hit=svgEl('line',{x1:i2p(seam.x1),y1:i2p(seam.y1),x2:i2p(seam.x2),y2:i2p(seam.y2),stroke:'rgba(0,0,0,0.001)','stroke-width':14,'vector-effect':'non-scaling-stroke'});
          const line=svgEl('line',{x1:i2p(seam.x1),y1:i2p(seam.y1),x2:i2p(seam.x2),y2:i2p(seam.y2),stroke:'#111','stroke-width':state.selectedSeamId===seam.id?3:2,'stroke-dasharray':'8 4','vector-effect':'non-scaling-stroke','pointer-events':'none'});
          g.append(hit,line);
          g.addEventListener('pointerdown',ev=>{
            if(state.dimTool||state.seamTool)return;
            ev.preventDefault();ev.stopPropagation();
            state.selectedSeamId=seam.id;state.selectedDimId=null;clearSelection();renderSeamList();draw();
            ...
            const move=mv=>{
              ...
              moved=true;
              // Move both endpoints together so seam length and angle are preserved.
              // Clamp the whole seam to the canvas without changing its geometry.
              const minX=Math.min(x1,x2),maxX=Math.max(x1,x2),minY=Math.min(y1,y2),maxY=Math.max(y1,y2);
              const cdx=clamp(dx,-minX,state.cw-maxX),cdy=clamp(dy,-minY,state.ch-maxY);
              seam.x1=round3(x1+cdx);seam.y1=round3(y1+cdy);
              seam.x2=round3(x2+cdx);seam.y2=round3(y2+cdy);
              draw();
            };
            const up=()=>{
              ...
              if(moved){renderSeamList();scheduleSave();pushHistory();}
            };
          ...
          if(state.selectedSeamId===seam.id){
            const handle=(which,x,y)=>{const h=svgEl('circle',{cx:i2p(x),cy:i2p(y),r:5,fill:'#fff',stroke:'#2563eb','stroke-width':2,'vector-effect':'non-scaling-stroke'});h.style.cursor='crosshair';h.addEventListener('pointerdown',ev=>{if(state.dimTool||state.seamTool)return;ev.preventDefault();ev.stopPropagation();const move=mv=>{const q=svgPoint(mv);let sn=snapSeamPoint({x:p2i(q.x),y:p2i(q.y)});const fixed=which===1?{x:seam.x2,y:seam.y2}:{x:seam.x1,y:seam.y1};sn=constrainDrawPoint(fixed,sn,!!mv.shiftKey||drawShiftHeld);if(which===1){seam.x1=round3(sn.x);seam.y1=round3(sn.y);}else{seam.x2=round3(sn.x);seam.y2=round3(sn.y);}draw();};const up=()=>{document.removeEventListener('pointermove',move,true);document.removeEventListener('pointerup',up,true);document.removeEventListener('pointercancel',up,true);renderSeamList();scheduleSave();pushHistory();};document.addEventListener('pointermove',move,true);document.addEventListener('pointerup',up,true);document.addEventListener('pointercancel',up,true);});g.appendChild(h);};
            handle(1,seam.x1,seam.y1);handle(2,seam.x2,seam.y2);
          }
          svg.appendChild(g);
        });
      }

      function drawNotes(){const L=cur();if(!state.showNotes||!L||!Array.isArray(L.notes))return;L.notes.forEach(note=>{const g=document.createElementNS(svgNS,'g');g.setAttribute('data-note-id',note.id);g.style.cursor='move';const t=document.createElementNS(svgNS,'text');t.setAttribute('x',i2p(note.x));t.setAttribute('y',i2p(note.y));t.setAttribute('font-size','13');t.setAttribute('font-weight','600');t.setAttribute('fill','#111');t.setAttribute('paint-order','stroke');t.setAttribute('stroke','#fff');t.setAttribute('stroke-width','3');String(note.text||'Note').split('\n').forEach((row,i)=>{const sp=document.createElementNS(svgNS,'tspan');sp.setAttribute('x',i2p(note.x));sp.setAttribute('dy',i?'1.25em':'0');sp.textContent=row||' ';t.appendChild(sp);});g.appendChild(t);t.addEventListener('dblclick',e=>{e.preventDefault();e.stopPropagation();openInlineNoteEditor(t,note.text||'',raw=>{const v=String(raw??'').trim();if(!v)return false;note.text=v;draw();renderNoteList();scheduleSave();pushHistory();return true;});});g.addEventListener('pointerdown',e=>{if(state.noteTool)return;e.preventDefault();e.stopPropagation();const p0=svgPoint(e),sx=p2i(p0.x),sy=p2i(p0.y),nx=note.x,ny=note.y;const move=ev=>{const q=svgPoint(ev);note.x=clamp(nx+p2i(q.x)-sx,0,state.cw);note.y=clamp(ny+p2i(q.y)-sy,0,state.ch);draw();};const up=()=>{svg.removeEventListener('pointermove',move);svg.removeEventListener('pointerup',up);svg.removeEventListener('pointercancel',up);renderNoteList();scheduleSave();pushHistory();};svg.addEventListener('pointermove',move);svg.addEventListener('pointerup',up);svg.addEventListener('pointercancel',up);});svg.appendChild(g);});}
      ...
      function activeToolCursor(){
        return state.noteTool?'text':((state.dimTool||state.seamTool)?'crosshair':'');
      }
      ...
      btnDimTool && (btnDimTool.onclick=()=>{state.dimTool=!state.dimTool;if(state.dimTool){state.noteTool=false;state.seamTool=false;syncNoteToolUI();syncSeamToolUI?.();}if(!state.dimTool)dimTempStart=null;syncDimToolUI();});
      ...
      // --- Seam Tool: click two snapped points to place a seam ---
      let seamTempStart=null;
      svg.addEventListener('click',e=>{
        if(!state.seamTool) return;
        const L=cur(); if(!L)return; if(!Array.isArray(L.seams))L.seams=[];
        const pt=svgPoint(e); if(!pt)return;
        const raw={x:p2i(pt.x),y:p2i(pt.y)}; const snapped=snapSeamPoint(raw)||raw;
        if(!seamTempStart){seamTempStart=snapped;state.selectedSeamId=null;}
        else{
          const end=constrainDrawPoint(seamTempStart,snapped,!!e.shiftKey||drawShiftHeld);
      ...
      // Delete selected seam with Delete/Backspace.
      window.addEventListener('keydown',e=>{
        const ael=document.activeElement,tag=(ael&&ael.tagName||'').toLowerCase();
        if(tag==='input'||tag==='textarea'||tag==='select'||(ael&&ael.isContentEditable))return;
        if(e.key!=='Delete'&&e.key!=='Backspace')return;
        const L=cur();if(!L||!state.selectedSeamId)return;
        const idx=L.seams.findIndex(s=>s.id===state.selectedSeamId);if(idx<0)return;
        L.seams.splice(idx,1);state.selectedSeamId=null;draw();renderSeamList();scheduleSave();pushHistory();e.preventDefault();
      });
      ...
      svg.addEventListener('pointermove',e=>{
        const mode=state.dimTool?'dim':(state.seamTool?'seam':null);
        const start=mode==='dim'?dimTempStart:(mode==='seam'?seamTempStart:null);
      ...
      document.addEventListener('pointerdown', (e)=>{
        ...
        requestAnimationFrame(()=>{
          ...
          openInlineNoteEditor(el,'',raw=>{
            ...
            if(!v){L.notes=L.notes.filter(n=>n.id!==note.id);draw();scheduleSave();return true;}
      ...
      if (state.dimTool || state.seamTool) { blankDown = null; return; }
