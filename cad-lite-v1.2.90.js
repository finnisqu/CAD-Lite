  ...
  async function addRasterPNG() {
    ...
    sinksUI = initSinksCard({
      ...
      function constrainDrawPoint(start,end,force=false){
        ...
        const AUTO_TAN=Math.tan(15*Math.PI/180);
      ...
      function drawSeams(){
        ...
        L.seams.forEach(seam=>{
          ...
          if(state.selectedSeamId===seam.id){
            const handle=(which,x,y)=>{const h=svgEl('circle',{cx:i2p(x),cy:i2p(y),r:5,fill:'#fff',stroke:'#2563eb','stroke-width':2,'vector-effect':'non-scaling-stroke'});h.style.cursor='crosshair';h.addEventListener('pointerdown',ev=>{if(state.dimTool||state.seamTool)return;ev.preventDefault();ev.stopPropagation();const move=mv=>{const q=svgPoint(mv),sn=snapSeamPoint({x:p2i(q.x),y:p2i(q.y)});if(which===1){seam.x1=round3(sn.x);seam.y1=round3(sn.y);}else{seam.x2=round3(sn.x);seam.y2=round3(sn.y);}draw();};const up=()=>{document.removeEventListener('pointermove',move,true);document.removeEventListener('pointerup',up,true);document.removeEventListener('pointercancel',up,true);renderSeamList();scheduleSave();pushHistory();};document.addEventListener('pointermove',move,true);document.addEventListener('pointerup',up,true);document.addEventListener('pointercancel',up,true);});g.appendChild(h);};
